...
## 24. Phase3-C History-Only Control Result: Window Index Concern Reduced

[Fact] `history_only_h96_h720` control 已完成并同步：

- raw artifacts:
  `analysis/phase3_regime_segment_operator_history_only_20260624/raw/`;
- report:
  `analysis/phase3_regime_segment_operator_history_only_20260624/phase3_regime_segment_operator_history_only_report.md`;
- summary:
  `analysis/phase3_regime_segment_operator_history_only_20260624/phase3_regime_segment_operator_history_only_summary.json`。

[Evidence] History-only vs R.3:

| Quantity | Window-index run | History-only run |
| --- | ---: | ---: |
| MSE wins vs R.3 | `5/6` | `5/6` |
| mean relative MSE vs R.3 | `-0.39%` | `-0.51%` |
| observed aggregate-gap wins | `1/2` | `1/2` |
| observed H720 segment-gap wins | `2/3` | `2/3` |
| non-gap mean relative MSE vs R.3 | `-0.66%` | `-0.64%` |
| max prefix mismatch MSE | `4.925e-14` | `4.861e-14` |
| `window_index_norm` feature std | `0.288733` | `0.0` |

[Decision] `window_index_norm` 质疑被显著削弱：去掉 split-position 后，整体收益没有消失，甚至
mean relative MSE 从 `-0.39%` 变为 `-0.51%`。因此当前 positive signal 不能主要归因于
split-position shortcut。

[Remaining Confound] 当前 control 仍只使用 `TARGET_HORIZONS=96,720`，而 R.3 使用
`96,192,336,720`。因此 horizon-set/objective-pressure confound 仍未解除，不能最终判定机制 pass。

[11-Step Loop]

- `current_step`: Step 9-10 for `history_only_h96_h720`，并回到 Step 8 for full horizon-set control。
- `problem`: Regime-conditioned operator 的收益是否来自真正结构，而非 window index 或 horizon-set 改动。
- `existence_evidence`: window-index ablation 通过；horizon-set confound 未解除。
- `idea`: history-only regime token 条件化 target-side operator。
- `theory_check`: 去掉 window index 后收益保留，支持 history-regime signal；但完整 mixed-horizon 压力仍需验证。
- `design`: 下一轮 `history_only_h96_h192_h336_h720`。
