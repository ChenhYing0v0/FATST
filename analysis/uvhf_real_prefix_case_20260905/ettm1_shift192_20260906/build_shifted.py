"""Render the user-specified forecast origin, shifted forward exactly 192 steps."""
...
def main() -> None:
    ...
    selected = table[(table.origin == 8752 + 192) & (table.channel == 4)]
    ...
    np.testing.assert_allclose(
        new.loc[-191:0, "history"], old.loc[1:192, "ground_truth"]
    )
    np.testing.assert_allclose(
        new.loc[1:528, "ground_truth"], old.loc[193:720, "ground_truth"]
    )
    rows = []
    ...
    data.update(
        ...
        shift_steps=192,
        new_origin=8944,
        new_zero_equals_previous_future_step=192,
        zero_timestamp=str(raw.iloc[34560 + 8944 - 1, 0]),
        first_forecast_timestamp=str(raw.iloc[34560 + 8944, 0]),
        last_forecast_timestamp=str(raw.iloc[34560 + 8944 + 719, 0]),
