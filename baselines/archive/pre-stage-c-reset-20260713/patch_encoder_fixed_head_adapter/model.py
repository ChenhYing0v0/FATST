...
class PatchEncoderFixedHeadAdapter(nn.Module):
    def __init__(
        self,
        seq_len: int,
        pred_len: int,
    ...
    def _segment_adapter(self, z: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        return gamma[:, : self.pred_len], beta[:, : self.pred_len]
    ...
    def forward(self, x: torch.Tensor, return_components: bool = False) -> torch.Tensor | dict[str, torch.Tensor]:
        ...
        gamma, beta = self._segment_adapter(z)
