...
from model import PatchEncoderFixedHeadAdapter
...
def segment_state_similarity(model: PatchEncoderFixedHeadAdapter) -> list[dict[str, float]]:
    ...
    with torch.no_grad():
        for batch_index, (x, y) in enumerate(loader, start=1):
            ...
            if not isinstance(output, dict):
                raise TypeError("PatchEncoderFixedHeadAdapter must return component dict.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train PatchEncoderFixedHeadAdapter Phase 1 baseline.")
    ...
    parser.add_argument("--run-name", default="PatchEncoderFixedHeadAdapter")
...
def main() -> None:
    ...
    model = PatchEncoderFixedHeadAdapter(
        args.seq_len,
        args.pred_len,
