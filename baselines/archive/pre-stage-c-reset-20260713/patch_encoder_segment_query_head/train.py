...
from model import PatchEncoderSegmentQueryHead
...
def segment_state_similarity(model: PatchEncoderSegmentQueryHead) -> list[dict[str, float]]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train PatchEncoderSegmentQueryHead Phase 1 baseline.")
    ...
    parser.add_argument("--decoder-d-ff", type=int, default=256)
    parser.add_argument("--batch-size", type=int, default=128)
    parser.add_argument("--epochs", type=int, default=1)
    parser.add_argument("--learning-rate", type=float, default=1e-4)
    parser.add_argument("--patience", type=int, default=10)
    parser.add_argument("--seed", type=int, default=2021)
    parser.add_argument("--run-name", default="PatchEncoderSegmentQueryHead")
...
def main() -> None:
    ...
    model = PatchEncoderSegmentQueryHead(
        args.seq_len,
        args.pred_len,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_metrics, _, _ = evaluate(model, val_loader, device, args.max_eval_batches)
    ...
    test_metrics, pred_np, true_np = evaluate(model, test_loader, device, args.max_eval_batches)
    ...
    write_csv(run_dir / "segment_query_similarity.csv", segment_state_similarity(model))
