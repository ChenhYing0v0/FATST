...
from model import PatchEncoderStepSpecificStateAdapter
...
def segment_query_similarity(model: PatchEncoderStepSpecificStateAdapter) -> list[dict[str, float]]:
...
def segment_activation_similarity(components: dict[str, np.ndarray]) -> list[dict[str, float]]:
    states = components["segment_states"]
    if states.shape[2] < 2:
        return [{"segment_a": 0, "segment_b": 0, "cosine": 1.0}]
    ...
    with torch.no_grad():
        for batch_index, (x, y) in enumerate(loader, start=1):
            ...
            if not isinstance(output, dict):
                raise TypeError("PatchEncoderStepSpecificStateAdapter must return component dict.")
...
def state_modulation_stats(components: dict[str, np.ndarray]) -> list[dict[str, float]]:
    gamma = components["gamma"]
    beta = components["beta"]
    states = components["segment_states"]
    rows = [
        {
            ...
            "mean_abs_gamma": float(np.mean(np.abs(gamma))),
            "mean_abs_beta": float(np.mean(np.abs(beta))),
            "mean_segment_state_norm": float(np.mean(np.linalg.norm(states, axis=-1))),
            "std_segment_state_norm": float(np.std(np.linalg.norm(states, axis=-1))),
        }
    ]
    for segment_index in range(gamma.shape[2]):
        rows.append(
            {
                ...
                "mean_segment_state_norm": float(
                    np.mean(np.linalg.norm(states[:, :, segment_index, :], axis=-1))
                ),
                "std_segment_state_norm": float(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train PatchEncoderStepSpecificStateAdapter Phase 1 baseline.")
    ...
    parser.add_argument("--run-name", default="PatchEncoderStepSpecificStateAdapter")
...
def main() -> None:
    ...
    model = PatchEncoderStepSpecificStateAdapter(
        args.seq_len,
        args.pred_len,
    ...
    write_csv(run_dir / "adapter_query_similarity.csv", segment_query_similarity(model))
    write_csv(run_dir / "segment_state_activation_similarity.csv", segment_activation_similarity(components))
    write_csv(run_dir / "adapter_delta_stats.csv", adapter_delta_stats(pred_np, components))
    write_csv(run_dir / "state_modulation_stats.csv", state_modulation_stats(components))
