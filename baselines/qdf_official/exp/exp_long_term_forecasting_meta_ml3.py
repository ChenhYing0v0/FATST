...
class Exp_Long_Term_Forecast_META_ML3(Exp_Basic):
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch_cycle) in enumerate(vali_loader):
                ...
                if self.args.max_eval_batches > 0 and i + 1 >= self.args.max_eval_batches:
                    break
    ...
    def meta_test(self, train_loader, vali_data, vali_loader, criterion, path):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch_cycle) in enumerate(train_loader):
                ...
                if self.args.max_train_batches > 0 and i + 1 >= self.args.max_train_batches:
                    break
    ...
    def train(self, setting, prof=None):
        ...
        best_A_path = os.path.join(path, 'A.pth')
        self.A = torch.load(best_A_path, weights_only=False)
    ...
    def test(self, setting, prof=None, test=0):
        ...
        if test:
            ...
            self.model.load_state_dict(torch.load(os.path.join(ckpt_dir, 'checkpoint.pth'), weights_only=True))
            self.A = torch.load(os.path.join(ckpt_dir, 'A.pth'), weights_only=False)
