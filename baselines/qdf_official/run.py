...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_train_batches', type=int, default=0,
                        help='FATST resource-smoke cap; 0 preserves the upstream full loop')
    parser.add_argument('--max_eval_batches', type=int, default=0,
                        help='FATST resource-smoke cap; 0 preserves the upstream full loop')
    parser.add_argument('--final_evaluation_split', type=str, default='test', choices=['test', 'none'],
                        help='FATST test-hygiene adapter; upstream behavior is test')
        ...
        for ii in range(args.itr):
            ...
            if args.use_profiler:
                ...
                if args.final_evaluation_split == 'test':
                    ...
                    print('>>>>>>>final evaluation skipped by adapter<<<<<<<<<<<<<<<<<<<<<<<<')
            else:
                print('>>>>>>>start training : {}>>>>>>>>>>>>>>>>>>>>>>>>>>'.format(setting))
                exp.train(setting)
