...
[Remote runner boundary] `scripts/remote/run_phase2_region_balanced_gate.sh` 对 `GPU_IDS`
使用 bounded parallelism。当前版本使用 `wait -n` 动态填充队列：任一数据集完成后，下一
个 queued dataset 会立即启动到释放出的 GPU，而不是等待同一批次所有数据集都结束。这一
点对默认顺序 `ETTm1 Weather ETTh2` 很重要，因为 `Weather` 通常远慢于 `ETTm1`，不能让
`ETTh2` 等到 `Weather` 结束后才开始。
