...
8. `scripts/analyze_main_ii_h720_prefix_results.py` 合并 7 个既有 ISCF evaluations、42 个 reused-baseline tests 与 21 个 newly trained tests，硬检查 70 checkpoints/280 raw rows/224 aggregate cells，并将 35 个 local H720 anchors 与 frozen Main I 做 `max(1e-8, four float32 ULPs at anchor)` continuity audit；iTransformer/PatchTST/DLinear 的 21 个 H720 values 仅记录相对 published three-run means 的 signed deviations。
