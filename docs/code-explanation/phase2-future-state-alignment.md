...
`training_log.csv` 在每个 epoch 结束后刷新一次，而不是等整个训练结束后才写入。训练脚本
同时向 stdout 输出结构化 `epoch_progress` 行：

```text
epoch_progress run_name=... dataset=... epoch=3/100 val_mean_mse=... elapsed_sec=... eta_sec=...
```

远程进度检查脚本据此汇报当前 dataset 矩阵位置、epoch/total 和预计结束时间。
