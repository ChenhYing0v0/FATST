...
## Phase2-E2 Implementation Status

更新时间：2026-06-23

[Fact] Phase2-E2 residual/loss alignment diagnostic 已实现：
...
在同一批 R.3 residual 上比较：

1. `identity`;
2. `prefix_risk`;
3. `static_train_target_offdiag`;
4. `qdf_diag_precision`;
5. `qdf_off_diag_precision`;
6. `qdf_all_precision`。

核心输出是 `ratio_to_residual_mse`。它把每个 matrix 的 normalized quadratic loss 除以同一
setting 的 plain residual MSE，用来观察不同 objective matrix 是否对 hard horizons /
specialist gaps 施加不同 pressure。
...
- `analysis/phase2_qdf_alignment_diagnostic_20260623/phase2_qdf_residual_alignment_summary.json`。
...
- missing R.3 predictions: `12/12`。

### Next Action

commit/push 后，在 3090 上启动 artifact collection run：

```bash
GPU_IDS="1 2" bash scripts/remote/run_phase2_qdf_alignment_r3_predictions.sh
```
