...
`current_step`: 11-step loop 的 Step 11 已完成；R4.3 remote gate 和 R4.4 decision
report 已完成。当前结论是回退到 Step 4/6，进入 Phase4-S 重新设计 conditioned
...
- sync results into `analysis/phase4_horizon_decoupled_gate_20260624`。

[Fact] raw artifacts 位于：

- remote: `/home/yingch/exp_outputs/r-2026-fatst/phase4_horizon_decoupled`
- local: `analysis/phase4_horizon_decoupled_gate_20260624/raw`

[Fact] `.gitignore` 保持 `analysis/**/raw/` 不跟踪；保留聚合表和决策报告作为 repo artifact。
...
- `scripts/analyze_phase4_horizon_decoupled_gate.py`
- `analysis/phase4_horizon_decoupled_gate_20260624/phase4_horizon_decoupled_main_metrics.csv`
- `analysis/phase4_horizon_decoupled_gate_20260624/phase4_horizon_decoupled_strategy_summary.csv`
- `analysis/phase4_horizon_decoupled_gate_20260624/phase4_horizon_decoupled_dataset_summary.csv`
- `analysis/phase4_horizon_decoupled_gate_20260624/phase4_horizon_decoupled_horizon_summary.csv`
- `analysis/phase4_horizon_decoupled_gate_20260624/phase4_horizon_decoupled_segment_summary.csv`
- `analysis/phase4_horizon_decoupled_gate_20260624/phase4_horizon_decoupled_prefix_summary.csv`
- `analysis/phase4_horizon_decoupled_gate_20260624/phase4_horizon_decoupled_trace_summary.csv`
- `analysis/phase4_horizon_decoupled_gate_20260624/phase4_horizon_decoupled_decision_report.md`

[Fact] 关键结果：

| Strategy | MSE wins vs R.3 | Mean relative MSE | Worst dataset mean degradation |
| --- | ---: | ---: | ---: |
...
[Strong Evidence] 没有任何 horizon-decoupled candidate 通过 mean relative MSE < 0、
MSE wins >= 7/12、dataset degradation <= +0.5% 的 paper-core gate。

[Strong Evidence] `D3_interval_supervision` 在 `ETTh2` 有局部收益，但跨 dataset 不稳；
`D2_random_future_mask` 是整体最接近 R.3 的策略，但仍是 +3.51% mean relative MSE。

[Fact] prefix mismatch 保持 numerical-zero，说明失败不是 prefix consistency 或 evaluation
interface 错误。

[Decision] 当前失败的是 `D2-D6` 这种静态、全局、替换式 horizon-decoupled supervision。
HSS 研究问题保留，但回退到 Step 4/6：重新设计 conditioned schedule，而不是继续调当前参数。
