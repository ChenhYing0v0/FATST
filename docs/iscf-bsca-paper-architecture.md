...
| `current_review_cursor` | Section 6 v0.4 temporarily frozen usable；Section 7 v0.1 concise Conclusion draft待author review |
...
| `provisional_content` | Section 5 v0.13（5.6 narrative待author review）；Section 7 v0.1 two-paragraph prose；Method Figure 4 stable vector-asset synchronization |
| `authorization_source` | 2026-08-24用户确认Section 7两段功能与内容设定，并要求扩写为精简英文Conclusion初稿；Sections 1--6 evidence与Figures 5--7不变 |
...
Section 7 v0.1 prose amendment：Author确认两段式结构后，已将其扩写为204词英文Conclusion。P1收束varied-horizon problem、CHPC与ISCF-BSCA principle；P2压缩system-level、accuracy--cost、ablation、diagnostic与two-backbone evidence，并以output-side sharing granularity对unified forecasting的意义收尾。Conclusion不重复Section 6 limitations，不新增citations、metrics、mechanisms或future claims。Canonical draft=`docs/paper-drafts/iscf-bsca-conclusion-initial-draft.md`。Decision=`section7_v0p1_concise_initial_draft_author_review_candidate`。
...
Figure 5 v5.3 freeze / Section 5.6 v0.11 amendment：author已将Figure 5 v5.3固定为论文可用图片。Section 5.6按`5.5 matched utility -> unchanged Full-model behavior -> selected sample/allocation -> scope-signal diversity -> CHPC and boundary`重组。Figure 5a/c共同说明selected probe中的region-dependent soft reweighting，Figure 5b说明Scope-conditioned Forecasts并非重复轨迹；CHPC只在段末作为construction property与20/20 numerical audit报告。Figure与正文均不声称population prevalence、sparse/hard routing、oracle recovery或causal specialization。Decision=`figure5_v5p3_temporarily_fixed_section5_6_v0p11_author_review_candidate`。
...
Section 5 figure-only transfer amendment：manuscript更新为v0.8。5.7不再插入或引用Decoder-Transfer Table 4，Figure 7成为该节唯一main-text evidence carrier；冻结LaTeX table继续作为Figure 7的numerical/audit source。由于Figure 7只显示four-horizon mean MSE，正文同步删除不可由图直接读取的MAE结果，仅报告DLinear-style与PatchTST-style的MSE变化。当前visible evidence chain为Table 1、Table 2、Figure 6、Table 3与Figure 7；artifact values、figure assets与new implementation/training/formal test均未改变。
...
H5D prelaunch amendment：基于115个completed ETTh1 profiles，H5D只搜索`dropout=0`
与batch/LR、p19/p21 geometry及mode rank的48个new interactions；capacity、standalone
weight decay和budget extension不再扩张。Architecture/objective/scopes/inference graph保持不变，
因此本轮仍是paper-facing HPO而非method redesign。Remote train/validation已授权；formal test
必须等待48/48 manifest及新授权。
...
正文采用三步短叙事：首先一句说明ISCF由同一full trajectory返回不同horizon prefixes，因此CHPC by construction，并报告20/20 numerical checks的maximum absolute CHPD=0；其次说明sample-conditioned allocation不宜只用dataset mean表征，并通过Figure 5a/c展示同一selected probe上的regional dominant-scope变化与完整per-step soft probabilities；最后通过Figure 5b的scope-relative deviation profiles说明Scope-conditioned Forecasts不是重复轨迹。结论严格限定为selected-example capability，不估计prevalence。

Figure 5 v5 author-review candidate固定为：Panel a=ETTm1 validation probe 113的真实fused forecast与regional dominant-scope colour/strip；Panel b=五条Scope-conditioned Forecast相对fused forecast的deviation small multiples；Panel c=同一probe的$5\times720$ per-step Scope Probability heatmap。Selection rule为先最大化distinct region-dominant scopes，再最大化mean pairwise forecast disagreement。Caption必须披露validation-only、seed 2021、selection pool/rule、soft rather than hard allocation与selected-example boundary。Realized allocation value与独立failure-case panel仍不进入当前正文。

原始diagnostic audit中的aggregate probability range和allocation--competence alignment继续作为internal claim boundary保留。Figure 5 v5直接显示selected probe的per-step probability range并以colorbar透明呈现，不授权更强机制解释：只支持selected-example scope forecast diversity与region-dependent soft reweighting，不支持hard routing、oracle recovery或causal specialization。Canonical figure=`paper-figures/figure_5_scope_allocation_behavior.*`；QA=`analysis/iscf_bsca_section5_6_figure5_v5_sample_specific_behavior_20260821/figure_contract_and_qa.md`。
