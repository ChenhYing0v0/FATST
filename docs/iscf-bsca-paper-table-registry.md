...
| `Efficiency` | 正文 supporting result | `complete_accuracy_params_epoch_accuracy_parameter_advantage_compute_tradeoff` | 3 systems；84/84 accuracy cells、21/21 parameter units、63/63 timing logs | `analysis/iscf_bsca_paper_experiment_consolidation_20260731/efficiency_accuracy_params_epoch_20260817/table/table_iscf_bsca_efficiency.tex` | 支持one-model accuracy与four-horizon total-parameter consolidation；ISCF的logged one-epoch cycle更慢，不支持uniform efficiency或lower training-time claim |
...
- 比较集合冻结为ISCF-BSCA、TimeAlign与QDF；完整性为84/84 Main-I accuracy cells、21/21 four-horizon parameter service units与63/63 native timing logs；new training/formal test=`0/0`；
- 七dataset macro结果：ISCF-BSCA=`0.261/0.306 MSE/MAE, 2.926M params, 133.1 s/epoch`，TimeAlign=`0.274/0.314, 10.741M, 104.6 s`，QDF=`0.288/0.331, 5.337M, 55.4 s`；
- 相对TimeAlign/QDF，ISCF-BSCA分别改善MSE `4.936%/9.320%`、MAE `2.536%/7.639%`，并减少four-horizon total parameters `72.760%/45.181%`；
- 负向边界：ISCF-BSCA one-epoch cycle分别是TimeAlign/QDF的`1.272×/2.404×`。Timing为完成的RTX 3090 native runs中每checkpoint `train + scheduled validation, no test` epoch-cycle中位数，再按four-model service求和；不是新的exclusive-GPU microbenchmark；
- DLinear/PatchTST H720-prefix属于one-model services，不满足本表“四个horizon-specific models求和”的定义；其余Main-I systems缺少与accuracy配置匹配的完整7×4 local checkpoint/timing evidence，因此不混入主表；
- canonical audit=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/efficiency_accuracy_params_epoch_20260817/result_and_table_audit.md`；review PDF=`output/pdf/iscf_bsca_efficiency_accuracy_params_epoch_20260817.pdf`。旧2026-08-14 latency/memory/storage profiler完整保留为historical supplementary audit。
