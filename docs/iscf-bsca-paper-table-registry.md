...
| `Efficiency` | 正文 supporting result | `complete_five_system_accuracy_advantage_resource_mixed` | 5 systems；140/140 accuracy cells、35/35 memory/storage units、119 table-role checkpoints | `analysis/iscf_bsca_paper_experiment_consolidation_20260731/efficiency_accuracy_memory_storage_20260817/table/table_iscf_bsca_efficiency.tex` | 支持one-model macro accuracy与service consolidation；相对TimeAlign/AMD有memory/storage优势，但SimpleTM更轻，不支持uniform resource advantage |
...
- 比较集合冻结为ISCF-BSCA、TimeAlign、QDF、AMD与SimpleTM；完整性为140/140 Main-I accuracy cells、35/35 four-horizon memory/storage service units与119/119 table-role checkpoints；new training/formal test=`0/0`；
- 七dataset macro结果（MSE/MAE, peak MiB, checkpoints MiB）：ISCF-BSCA=`0.261/0.306, 38.817, 17.677`，TimeAlign=`0.274/0.314, 109.102, 95.433`，QDF=`0.288/0.331, 31.939, 20.381`，AMD=`0.282/0.328, 238.035, 221.150`，SimpleTM=`0.293/0.333, 14.841, 2.835`；
- ISCF-BSCA在五系统中macro MSE/MAE均最佳，并相对TimeAlign/AMD显著减少peak memory与checkpoint storage；相对QDF只保留storage优势，SimpleTM在两项resource指标上均更轻；
...
- SimpleTM accuracy沿用Main I multi-run summary，resource使用预冻结repeat 0作为一次非ensemble deployment；
- iTransformer/PatchTST/DLinear当前只有Main II H720 checkpoints，TVNet只有ETTh2 local correction，均不满足完整7×4 horizon-specific checkpoint协议；
