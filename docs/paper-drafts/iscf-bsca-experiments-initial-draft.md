...
| `version` | `v0.10-section5-6-sample-specific-behavior` |
| `date` | `2026-08-21` |
| `review_status` | `Section 5.6 and Figure 5 v5 revised for author review; remaining Section 5 text unchanged` |
...
Because every horizon request is returned as a prefix of the same predicted trajectory, ISCF satisfies CHPC by construction; the frozen implementation also yields a maximum absolute CHPD of 0 across all 20 dataset--horizon checks. Rather than visualizing this deterministic property, we examine two internal behaviors that must be established empirically: whether the Scope-conditioned Forecasts retain distinct predictive signals and whether the learned allocation reduces every future region to the same scope preference.

Scope Probabilities depend on the sample-variable history and future coordinate, so averaging them over an entire dataset can obscure the sample-specific behavior that the allocation path is designed to express. Figure 5 therefore examines one ETTm1 validation probe selected from 1,280 candidates by first maximizing the number of distinct region-dominant scopes and then maximizing scope-forecast disagreement. Figure 5a plots the actual fused forecast and colours each future region by the scope receiving the highest mean probability in that region. The eight regions collectively involve all five scopes, showing that the allocation does not reduce this example to one fixed scope. The colours identify the dominant component of a soft allocation and do not represent hard routing.

Figure 5b makes the corresponding forecast diversity visible by plotting each Scope-conditioned Forecast relative to the fused trajectory. The five deviation profiles are non-zero and differently structured, with a mean pairwise absolute disagreement of 0.0812 on the normalized scale, showing that the scope field has not degenerated into repeated copies of one forecast. Figure 5c reports the complete per-step Scope Probabilities for the same probe and shows how the relative weights evolve within and across future regions. Together, these sample-specific diagnostics illustrate distinct scope signals and region-dependent soft reweighting in the trained model. As a selected validation example, however, the figure does not estimate how frequently this behavior occurs or establish sparse routing, oracle scope selection or causal specialization.
...
**Figure 5 | Sample-specific scope diversity and allocation behavior in ISCF-BSCA.** The ETTm1 validation probe was selected from 1,280 candidates by first maximizing the number of distinct region-dominant scopes under the mean soft allocation and then maximizing mean pairwise scope-forecast disagreement. **a**, The actual fused forecast, with each segment and the lower strip coloured by the highest-weight scope after averaging Scope Probabilities within that future region. The colour indicates the dominant component of a soft allocation rather than a hard route. **b**, Deviations of the five Scope-conditioned Forecasts from the fused forecast, shown on a shared scale. **c**, Per-step Scope Probabilities for the same probe; white lines mark future-region boundaries. All panels are validation-only diagnostics from seed 2021 and illustrate one selected example rather than population prevalence or oracle scope recovery.
