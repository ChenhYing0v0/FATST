...
ISCF-BSCA attains the lowest macro MSE/MAE, 0.261/0.306, among the five fully audited systems. Its complete service uses 38.817 MiB peak allocated GPU memory and 17.677 MiB of checkpoint storage. Relative to TimeAlign and AMD, the unified service reduces peak memory by 64.42% and 83.69% and checkpoint storage by 81.48% and 92.01%, respectively. Relative to QDF, it retains a 13.27% storage reduction but uses 21.54% more peak memory.

The expanded comparison also exposes an important boundary. SimpleTM is substantially lighter, requiring only 14.841 MiB peak memory and 2.835 MiB of checkpoint storage for its four native models, although its macro MSE/MAE is worse at 0.293/0.333. Peak memory is measured in a fresh process on an exclusive RTX 3090 in FP32 with batch size 1, synthetic standardized inputs and all service checkpoints resident. SimpleTM resource cost uses one non-ensemble repetition per horizon, while its frozen Main-I accuracy follows the multi-run summary. The evidence therefore supports a better accuracy--resource trade-off than the stronger TimeAlign and AMD families and a one-checkpoint consolidation benefit, but not a uniform memory or storage advantage over every lightweight baseline.
...
| 5.4 Efficiency | 140/140 Main-I accuracy cells, 35/35 memory/storage service units and 119 table-role checkpoints | Best five-system macro accuracy and one-checkpoint consolidation; lower memory/storage than TimeAlign and AMD | Uniform memory/storage advantage over all baselines or a training-time claim |
