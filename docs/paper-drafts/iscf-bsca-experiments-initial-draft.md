...
| `version` | `v0.11-section5-6-figure-aligned-narrative` |
| `date` | `2026-08-22` |
| `review_status` | `Figure 5 v5.3 temporarily fixed as manuscript-usable; Section 5.6 v0.11 revised for author review; remaining Section 5 text unchanged` |
...
The controlled ablations establish the aggregate utility of the proposed components, but they do not reveal how the unchanged Full model organizes an individual forecast. We therefore examine whether the Scope-conditioned Forecasts retain distinct signals and whether scope preference changes across future regions instead of remaining fixed.

Scope Probabilities depend on the sample-variable history and Future Coordinate, so dataset-level averaging can obscure the conditional behavior of the Target-Adaptive Allocation Path. Figure 5 therefore uses ETTm1 validation probe 113, selected from 1,280 candidates by maximizing regional dominant-scope diversity and then mean pairwise scope-forecast disagreement. Figure 5a shows the actual fused forecast, with each region coloured by the scope receiving the highest mean probability in that region. All five scopes become the highest-weight component in at least one of the eight regions. The allocation therefore does not collapse to one fixed regional preference in this selected example. The colours and lower strip summarize the dominant component of a soft allocation rather than a hard routing decision.

Figure 5b examines whether the scope lines provide distinguishable forecast signals. It plots each Scope-conditioned Forecast relative to the fused trajectory using one shared scale. The five profiles have different temporal structures, and their mean pairwise absolute disagreement is 0.0812 on the normalized scale. Thus, the scope-indexed forecast field retains non-identical predictions for this probe rather than repeating one forecast across all scopes.

Figure 5c displays the complete per-step Scope Probability profiles for the same probe. Their relative weights vary within and across future regions, complementing the regional summary in Figure 5a. These behaviors concern how ISCF constructs one shared trajectory, whereas consistency across horizon requests follows from prefix-based inference. Because every request returns a prefix of that trajectory, ISCF satisfies CHPC by construction; the frozen implementation yields a maximum absolute CHPD of 0 in all 20 dataset--horizon checks. Together, the three panels illustrate distinct scope signals and region-dependent soft reweighting in the selected probe. They do not estimate population prevalence or establish sparse routing, oracle scope selection or causal specialization.
