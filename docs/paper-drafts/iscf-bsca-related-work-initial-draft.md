# HoriScope Section 2: Related Work
...
| --- | --- | --- |
| 2.1 Fixed-horizon multi-step forecasting | Distinguish recursive, direct, MIMO and block multi-output strategies before contrasting them with UVHF | Existing strategies organize predictions within one prespecified horizon; UVHF serves multiple request endpoints with one trajectory |
| 2.2 Unified varied-horizon forecasting | Position the paper against foundation models and ElasTST | ElasTST already establishes horizon-invariant varied-horizon forecasting; the complementary gap is output-side region-dependent state sharing |
...
| 2.4 Multi-scale forecasting and adaptive allocation | Separate output-side sharing scopes from history-side scales and generic experts | HoriScope allocates explicit state-reuse granularities rather than selecting complete predictors or frequency experts |
...
| HoriScope distinction | Architecture-level interpretation | Effectiveness, component value and transferability remain contingent on Sections 5.2--5.7 |
