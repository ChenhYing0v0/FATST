...
| Efficiency and system cost | Main-I accuracy, model count, four-horizon total parameters and normalized one-epoch cycle | collapsing parameter consolidation into a uniform training-efficiency claim |
...
| 5.4 Efficiency and system cost | What accuracy and training-system cost changes when one model replaces a four-model horizon-specific family? | Main-I macro MSE/MAE; model count; four-horizon total parameters; one-epoch cycle; timing-log protocol | Table 3 / `Efficiency` | Accuracy and parameter consolidation versus four-model TimeAlign/QDF, with the slower one-epoch-cycle boundary retained | Complete; 84/84 accuracy cells, 21/21 parameter units and 63/63 timing logs |
...
| Table 3 | Efficiency | accuracy, four-horizon parameters and one-epoch-cycle trade-off | complete; accuracy/parameter advantage with slower epoch cycle |
...
| ISCF-BSCA consolidates a four-horizon service while improving Main-I accuracy and reducing total parameters | Efficiency | supported as a trade-off | report accuracy/parameter reductions together with the slower logged one-epoch cycle; do not claim uniform efficiency or lower training time |
