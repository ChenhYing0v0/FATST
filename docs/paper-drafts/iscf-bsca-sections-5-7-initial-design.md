...
| `version` | `v0.22-section5-6-author-outline` |
| `date` | `2026-08-24 Section 5.6 author-outline reconstruction` |
| `review_status` | `Figure 5 v5.3 remains temporarily fixed usable; Section 5.6 v0.12 pending author review; remaining structural design unchanged` |
...
| `manuscript_prose` | Section 5 v0.12 drafted at `docs/paper-drafts/iscf-bsca-experiments-initial-draft.md`; Figure 5 v5.3 temporarily fixed usable，Section 5.6正文待author review；Sections 6--7 remain structural only |
...
| 5.6 Scope diversity and allocation behavior | Can Full ISCF-BSCA form distinct scope signals and sample-specific regional reweighting? | selected actual fused trajectory with regional dominant-scope colours；five scope-relative deviation profiles；same-probe $5\times720$ probability heatmap；20/20 CHPC numerical audit | Figure 5 v5.3 | In the selected probe, scope forecasts are non-identical and all five scopes appear as regional dominant components；the farthest region's $s=720$ preference is sample-specific；no prevalence、monotonic distance--scope law、sparse/hard routing、oracle recovery或causal specialization | Figure fixed；Section 5 v0.12 prose under author review |
