...
| `evidence_snapshot` | Main-I, corrected Main-II, Core-Ablation, Decoder-Transfer and Efficiency complete; mechanism analysis remains pending under the current table registry |
...
| 5.7 Backbone transferability | Does the decoder remain useful beyond its current Encoder realization? | Weather、ETTm1、ETTm2；DLinear-style与PatchTST-style；Original Decoder、+ISCF、+ISCF-BSCA；end-to-end training | Table 5 / `Decoder-Transfer` | 在作者后置收窄的三数据集范围内，best-config PatchTST相对Original为+1.268%/+0.192%，但Weather/ETTm1仍缺同profile matched +ISCF controls；five-dataset negative audit保留 | three-dataset paper candidate complete；2 matched controls pending |
...
| Table 5 | Decoder-Transfer | end-to-end portability across backbone families | author-refined three-dataset candidate；best-config performance positive；2 matched +ISCF controls pending |
...
| The decoder transfers across forecasting backbones | Decoder-Transfer | provisional on author-refined three-dataset scope | DLinear-style与PatchTST best-config均在三数据集macro MSE/MAE正向；在两个missing matched +ISCF controls闭合前只写performance portability，并披露five-dataset negative boundary |
