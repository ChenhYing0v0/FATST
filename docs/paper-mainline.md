...
| `current_11_step` | Phase5 Stage A re-evaluation：Step 2/3 已完成，Stage A standalone method route 暂停 |
| `active_carrier` | official-source TimeAlign |
| `active_stage_ledger` | `docs/stage-ledgers/phase5-timealign-interface.md` |
| `active_question` | 在 capacity-preserving prefix-aware carrier 已被控制的前提下，future supervision 的可靠性应如何决定 gradient path |
| `current_gate` | Stage A 重评估完成：A4S 否定 existing-path selector；A2/A3D 证明 interface 是必要控制但不足以作为 standalone method |
| `paper_core_status` | 论文贡献边界重构为 `Problem Formulation and Interface-Controlled Evaluation` + `Reliability-Aware Future Supervision Routing`；下一步建立 Stage B diagnostic plan |
...
Stage A 已经证明 interface 是 material confounder，但还没有证明一个可作为 standalone
method contribution 的 new interface。此时继续堆 head 或 existing-path selector 会被审稿人
看成 ad hoc search。更合理的主线是：保留 interface-controlled evaluation 作为论文必要贡献
边界，把 paper-core method 转向 `future supervision gradient-path scheduling`。
...
[Claim Version 0.6] Horizon-Agnostic Supervision Scheduling treats benchmark
horizons as evaluation probes rather than training units. It schedules future
supervision through an interface-controlled carrier and a reliability-aware
...
> paths 作为监督调度对象。Stage A 的作用是证明并控制 unified prediction interface 这个
> confounder，避免方法收益来自 full-720 crop 或 head capacity；论文主方法则必须回答
> future supervision 的可靠性如何决定 future branch 的梯度进入模型的位置。
...
2. **Interface-Controlled Evaluation Protocol**
   证明 unified multi-horizon 结果会被 prediction interface 显著影响，并建立后续 HSS 方法必须
   使用的 capacity-preserving prefix-aware carrier/control。该贡献不再声称已有 head 是最终方法，
   而是把 interface 作为 paper-core method 的必要控制条件，避免收益来自 full-720 crop、
   head capacity 或 validation artifact。

3. **Reliability-Aware Future Supervision Routing**
   在 interface-controlled carrier 上，根据 future unit 的可重构性、alignment consistency、
   residual volatility、gradient conflict 或预测误差结构，决定 future supervision 的强度与
   gradient path。
...
- H1 `target_set_decoder_multiprefix`、H1C `row_gated_dense_head_multiprefix` 与 A3D
  `teacher_preserved_nested` 是当前 interface controls / carrier candidates；后续方法必须说明
  收益不是由这些 head/control 的选择单独造成。
- A4S 只否定 existing-path selector，不否定 interface 问题本身。interface 保留为 problem
  evidence 与 carrier/control constraint。
- Reliability routing 必须在 interface-controlled carrier 上评估。若 Stage B 收益高度依赖某个
  head choice，必须回滚 Stage A 重新设计 first-principles interface，而不能把 head choice 写成方法。
...
Stage A final decision after A4S：
...
- 因此 Stage A standalone interface method route 暂停。interface 保留为
  `Problem Formulation and Interface-Controlled Evaluation`，下一步主方法进入
  `Reliability-Aware Future Supervision Routing`；
- 若 Stage B 收益高度依赖某个 head/control，必须回滚 Stage A，重新设计 first-principles
  interface，而不是把 head choice 写成方法。
...
- Stage A contribution re-evaluation 已完成：standalone interface method route 暂停，interface 保留为
  problem evidence 与 carrier/control constraint；下一步进入 Stage B。

### Stage B：Future Supervision Reliability Diagnostic

目标：在 interface-controlled carrier 上证明 future supervision 的 useful/harmful 差异真实存在，
并判断这种差异是否应通过 gradient-path routing 处理。Stage B 是当前 active next route，不再只是
Stage A2/A3 的并行准备。
...
- 比 interface-controlled carrier 稳定更好，至少包含 H1 `target_set_decoder_multiprefix` 与
  A3D `teacher_preserved_nested` controls；必要时保留 H1C `row_gated_dense_head_multiprefix`；
