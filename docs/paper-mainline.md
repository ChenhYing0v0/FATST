...
| `current_11_step` | Phase5-A3E：Step 9/10/11，A3E 已评估并回滚到 interface reliability diagnostic |
...
| `current_gate` | A3E effectiveness_gate 未通过；target conditioning 进入 primary nested head 的增量不足，ETTm1 上 A3C 仍是最强候选 |
| `paper_core_status` | Stage A interface 主线进入 reliability diagnostic：暂不直接叠加 A3F，先判断 capacity-preserving path 的 dataset/horizon reliability 是否能重构贡献 1 |
