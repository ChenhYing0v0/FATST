...
| `current_11_step` | Phase5-A4S：Step 6/7/8，validation-prefix signal export 已实现，准备远程 diagnostic-only run |
...
| `paper_core_status` | Stage A interface 主线暂定重构为 `Reliability-Aware Capacity-Preserving Interface`；A4S 将验证 prefix-wise validation signals 是否足以支撑该路线，若失败则回 Step 2/3 重审 interface contribution |
