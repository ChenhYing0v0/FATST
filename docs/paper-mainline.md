...
| `current_11_step` | paper consolidation；writing=Section 4 v0.1 pending author review；experiments=QDF Main I L336 8×4 reproduction authorized，prelaunch gate active |
...
[QDF Main I L336 Full Reproduction Prelaunch, 2026-08-06] 用户要求把QDF从现有L96 mixed-source block改为`seq_len=336`下8 datasets × four H × seed2023的全本地复现。六个official-script datasets仅改变lookback，保留released逐Hprofiles；Solar和Exchange分别固定为ECL-derived与ETTh1-derived source-informed presets，不冒充official scripts。Formal matrix=32 fixed-H systems，validation early stopping后每run一次test；32/32 artifact/hash audit前当前L96表保持不变，禁止混用partial L336 rows。通过后7-dataset dense table全部替换为本地QDF值，Exchange companion增加QDF。Canonical prelaunch=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/qdf_main_i_l336_20260806/design_and_launch_gate.md`。Decision=`QDF_L336_8x4_prelaunch_pass_remote_smoke_then_background_formal_authorized`。
