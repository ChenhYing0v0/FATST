...
[HoriScope Table 1 Header Alignment Refinement, 2026-08-27] 修正canonical KBS稿件第12印刷页Table 1表头的纵向对齐。原先由11个独立`shortstack`构成的model/year表头改为显式两行结构，`Dataset`通过`multirow`跨两行纵向居中；所有model names共享同一基线，years与`(Ours)`共享第二行基线。实验数值、best/second标记、model顺序与caption均未变化。24页PDF重新编译并对该页进行180-dpi视觉QA，未发现表头碰撞、裁切或数据行位移。Decision=`table1_header_alignment_verified`；new experiment/training/remote launch/formal test=`0/0/0/0`。
...
[HoriScope KBS Template Migration, 2026-08-27, superseded] 将冻结手稿逐模块迁移至作者提供的PDT KBS模板库`Elsevier_template/`。该分文件版本已由上述PDT-exact flat submission取代，不再作为canonical投稿源。Historical audit=`analysis/horiscope_kbs_template_migration_20260827/audit_report.md`。Decision=`superseded_by_horiscope_pdt_exact_flat_submission`；new implementation/training/remote launch/formal test=`0/0/0/0`。
...
[Appendix Author Refinement v0.5, 2026-08-25] 按作者反馈进一步精简Appendix。A.2参考PDT Section 4.1.1的组织方式重写ETT/ECL/Weather/Solar介绍；Table A1压缩为`Dataset / Variables / Sampling frequency / Dataset size / Domain`，不再展示split boundaries与window counts。Table A2删除look-back列并将target length统一改为`Max Length`，同时删除作者指定的test-informed profile句。Appendix C恢复独立`Visualization`章节；Figure C1图片内部删除冗余`Appendix C |`前缀，仅保留`Representative validation trajectories`，正文caption删除详细visual-fidelity selection说明。Figure数据、样本、曲线、prefix ruler、配色与evidence source均未改变。Decision=`appendix_v0p5_author_refinement_ready`；new training/remote launch/formal test=`0/0/0`。

[Appendix Reference-Aligned Rewrite v0.4, 2026-08-25] 按作者指定的TimeAlign附录组织方式重写当前Appendix：Appendix A对应其Appendix D的`Metric Details -> Datasets -> Implementation Details`结构，新增MSE/MAE正式定义与四类dataset简介，并保留三张经本地协议审计的统计/训练/ISCF配置表；Appendix B对应Appendix E.1，以简短`Full Results / Main Experiments`导语连接Sections 5.2/5.3与完整逐horizon Tables B1/B2；Appendix C对应Appendix F，不显示独立大标题，直接以短段落引出已接受的validation-only Figure C1。参考论文仅提供写作和版式范式，所有数值、结果表、sample与claim boundary均保持FATST冻结版本不变。Decision=`appendix_v0p4_reference_aligned_manuscript_ready`；new training/remote launch/formal test=`0/0/0`。

[Appendix C Figure and Candidate Audit v0.3, 2026-08-25] 基于冻结Main-I/II selected-profile checkpoints完成Weather与ECL的64候选validation-only审计。Weather的现有样本3914仍为候选池最低四horizon scaled-MSE，且无更优的720-step分离候选；ECL的现有样本222仍为最优候选，第二样本1145由720-step分离约束决定，近邻低误差候选若替换将改变已声明的选择协议。现有样本因此保留，不作审美驱动的post-hoc cherry-picking。Figure C1按`nature-figure`重绘为共享nested-prefix ruler、浅色endpoint guides、dark-slate ground truth与teal ISCF-BSCA prediction，严格preflight=`14/0/0`通过。候选审计=`analysis/iscf_bsca_appendix_c_candidate_probe_20260825/candidate_audit.md`；figure contract/QA=`analysis/iscf_bsca_appendix_c_prediction_export_20260825/figure_c1_{contract,qa}.md`；无new training、remote launch或formal test。Decision=`appendix_c_candidate_audit_and_prefix_figure_revision_complete_author_review`。

[Appendix C Visual-Fidelity Selection and Prefix Alignment v0.4, 2026-08-25] 根据作者要求，Appendix C不再仅按绝对MSE/MAE选择样本。新规则先在validation split上排除最低方差20%的channel，再按`0.70 train-scale level RMSE + 0.15 trajectory-correlation loss + 0.10 first-difference-correlation loss + 0.05 amplitude error`选择每dataset固定channel，随后在该channel内按四horizon visual-fidelity score和720-step origin gap选择两个sample。冻结channel map为`ETTh1=2, ETTh2=6, ETTm1=0, ETTm2=6, Weather=18, ECL=306, Solar=99`；Weather/ECL的曲线视觉贴合度明显改善。Figure C1的prefix ruler改为分别对齐sample 1/2 panel列的双ruler，避免标题下指示条与预测图脱节。新source、selection audit和figure outputs位于`analysis/iscf_bsca_appendix_c_prediction_export_20260825/`；nature-figure preflight=`14/0/0`；无new training、remote launch或formal test。Decision=`appendix_c_visual_fidelity_rule_and_prefix_alignment_author_review`。

[Appendix Complete Manuscript Draft v0.3, 2026-08-25] 基于已冻结的Appendix A协议/配置表、Main-I/Main-II完整逐horizon结果表和已接受的Figure C1，完成三部分Appendix整合稿`docs/paper-drafts/iscf-bsca-appendix-initial-draft.md`。Appendix A保留dataset metadata/splits、shared training/evaluation settings与逐dataset ISCF-BSCA配置；Appendix B明确路由至canonical LaTeX Table B1/B2并保留source-role与system-level claim boundary；Appendix C纳入当前validation-only qualitative figure、视觉保真选择规则、paired nested-prefix ruler与source-data链接。未新增训练、remote launch或formal test。Decision=`appendix_complete_three_part_manuscript_draft_ready`。

[Appendix Structure Design v0.2, 2026-08-25] 根据作者进一步要求，将附录收敛为三个最小单元：Appendix A只保留dataset metadata/splits、optimizer与动态learning-rate、patience等训练设置，以及逐dataset的ISCF-BSCA关键配置表；Appendix B保留Tables 1--2背后的完整Main-I/Main-II逐dataset--horizon--metric结果；新增Appendix C，使用一个validation-only七dataset composite figure，每个dataset展示两个按冻结规则选择的fused forecast样本与四个horizon prefix标记。PDT_final.pdf和TimeAlign Appendix D仅作为格式/数据描述参考，不替代FATST frozen protocol。当前仓库尚未具备七个最终profile的逐样本预测导出，因此C1停在evaluation-only data export gate；无new training、remote launch或formal test。Decision=`appendix_v0p2_three_appendix_author_plan_data_export_pending`。

[Appendix A Protocol Audit v0.1, 2026-08-25] 七个paper-core datasets的raw observations、channel counts、sampling frequency、loader split boundaries、train/validation window counts与finite-loader检查已完成；selected profile的effective max epochs/patience、optimizer、cosine schedule与dataset-specific ISCF interface fields已从phase configs和trial ledgers解析。Canonical draft=`docs/paper-drafts/iscf-bsca-appendix-a-initial-draft.md`；audit=`docs/paper-drafts/iscf-bsca-appendix-a-c-data-audit.md`。Appendix C仍等待final-profile evaluation-only prediction export；无new training、remote launch或formal test。Decision=`appendix_a_protocol_and_config_audit_complete_c_export_pending`。
...
[Section 5.6 Author-Outline Narrative v0.12, 2026-08-24] 5.6按author提供的三段逻辑重构：先以5.5 ablation承接Full-model internal-behavior问题；再用Figure 5b的scope-relative deviations验证scope-indexed forecast field保留非重复candidate signals；最后用Figure 5a/c说明selected probe中的region-dependent highest-weight scopes与per-step probability variation。最远region对$s=720$的偏好只写为sample-level observation，不扩张为单调distance--scope规律；`prediction bias`收紧为`scope-specific forecast deviation`。CHPC的by-construction与20/20 zero-CHPD audit保留在末段，Figure 5 v5.3及claim boundary不变。Decision=`section5_6_v0p12_author_outline_narrative_author_review_candidate`。
...
[Section 5 Initial Draft and Figures 6--7, 2026-08-17] 已基于complete Main-I/Main-II、Efficiency、author-corrected Core-Ablation、Figure 5与author-corrected Decoder-Transfer完成`docs/paper-drafts/iscf-bsca-experiments-initial-draft.md`。5.4的accuracy--system-cost Figure 6已同步新Efficiency合同：x为four-horizon total parameters，y为Main-I seven-dataset/four-H macro MSE，bubble area为seven-dataset macro one-epoch cycle seconds；只比较具备完整matched accuracy/cost artifacts的ISCF-BSCA、TimeAlign与QDF，并保留ISCF-BSCA one-epoch cycle更慢的负向边界。5.7新增two-panel Figure 7，展示DLinear-style与PatchTST-style在Weather/ETTm1/ETTm2的Original Decoder versus complete ISCF-BSCA aggregate MSE。5.6正文明确区分Table 4的aggregate allocation gain与Figure 5的near-uniform probability和8/40 utilization-error alignment，不宣称oracle routing或causal specialization。Figure contract、Python source、plot data与outputs位于`analysis/iscf_bsca_section5_figures_20260817/`；canonical assets位于`paper-figures/figure_6_accuracy_system_cost.*`和`paper-figures/figure_7_decoder_transfer.*`。Decision=`section5_v0_1_evidence_complete_initial_draft_pending_author_review`；no new training/formal test。
...
[Decoder-Transfer Author Correction, 2026-08-17] 作者提供复跑后的Weather、ETTm1、ETTm2 four-horizon aggregate table并要求替换固定Table 5。完整ISCF-BSCA相对Original Decoder的display-precision macro MSE/MAE收益更新为DLinear-style `+5.611%/+3.604%`、PatchTST-style `+2.128%/+1.274%`；两类backbone均取得3/3 dataset MSE/MAE wins，ISCF-BSCA在16/16 comparator metric columns中更优。本表仍只证明complete framework portability，不区分ISCF与BSCA内部贡献。新输入未附per-horizon raw files、selectors、profiles或checkpoint hashes，因此旧48-cell/hash artifacts仅作为historical audit；`author_refined_posthoc_scope`、test-tuned history与ETTh1/ETTh2及iTransformer-style负向证据继续披露。Canonical report=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/decoder_transfer_three_dataset_scope_20260816/result_and_next_gate.md`。Decision=`decoder_transfer_author_corrected_aggregate_both_backbones_all_columns_positive`。
...
[iTransformer-style Decoder-HPO v2 Remote Launch, 2026-08-16] Commit=`219d52708e...66677`；remote prelaunch 22/22与Weather resource smoke 3/3通过，smoke test access=0且临时产物已清理。GPU0/1/2于02:04:40启动70-run train/validation queue，PID=`1517179`；首批三个profiles均进入epoch1且loss finite。Formal test=0；next=`70/70 artifacts + 70 unique hashes + immutable manifest`。Canonical launch=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/decoder_transfer_itransformer_hpo_v2_20260816/remote_launch.md`。Decision=`itransformer_decoder_hpo_v2_three_gpu_training_active_test_zero`。

[iTransformer-style Decoder-HPO v2 Training Gate, 2026-08-16] 70/70 train/validation artifacts、70 unique checkpoint hashes、5/5 dataset-level matched encoder initialization sets、selector与numeric-health gates通过；manifest SHA256=`ab3a4cd9...c66a`，training test=0。按作者既有授权，formal scope固定为70 checkpoints/280 new cells，并复用v1 BSCA reference形成300-cell candidate pool；每dataset只按four-H mean test MSE选择一个shared profile，全部negative trials保留。Canonical gate=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/decoder_transfer_itransformer_hpo_v2_20260816/training_result_and_formal_test_gate.md`。Decision=`itransformer_decoder_hpo_v2_training_manifest_pass_280_cell_test_tuned_formal_authorized`。

[iTransformer-style Decoder-HPO v2 Formal Result, 2026-08-16] 70/70 checkpoints、280/280 new cells与300/300 candidate-pool cells闭合。Selected HPO相对v1 BSCA macro MSE/MAE改善`2.128%/1.719%`，但相对Original为`-0.505%/-0.750%`；dataset MSE wins=`4/5`、cell wins=`12/20`，ETTh1仍退化`3.110%/3.376%`并主导overall fail。Formal中发现并修复一个只影响alternate scopes的旧invariant checker defect；修复不改变checkpoint/model/test loader/selection。Performance gate失败，故不补matched ISCF、不继续extra HPO/seeds、不改canonical table。Canonical result=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/decoder_transfer_itransformer_hpo_v2_20260816/formal_results/result_and_decision.md`。Decision=`itransformer_decoder_hpo_v2_improves_v1_bsca_but_fails_original_gate_close_rescue`。
...
[iTransformer-style Decoder-Transfer v1 Formal Authorization, 2026-08-15] 用户明确要求继续formal test。冻结执行面为immutable 15-checkpoint manifest × four standard horizons=`60` cells；test loader access前后验证manifest/checkpoint hashes，禁止retraining、partial reporting与dataset/horizon/metric/cell选择。Canonical table mutation、extra HPO和extra seeds仍false。Canonical prelaunch=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/decoder_transfer_itransformer_v1_20260815/formal_test_prelaunch.md`。Decision=`itransformer_transfer_v1_formal_15_checkpoint_60_cell_authorized_prelaunch`。
...
[PatchTST Decoder-Transfer v2.1 Remote Launch, 2026-08-15] Commit=`9cf0e8e8`；3/3 resource smoke通过，GPU0/1/2于14:43启动5个matched `+ISCF` runs，training PID=`663673`。Guarded pipeline PID=`665646`已排队：10-hash/5-pair manifest checker失败则test=0；只有checker通过才执行10 checkpoint/40-cell formal test并合并v1 controls生成120-cell结果。Canonical launch=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/decoder_transfer_patchtst_v2p1_20260815/remote_launch.md`。Decision=`v2p1_matched_training_active_guarded_formal_pipeline_queued`。
...
[PatchTST Decoder-Transfer HPO v2 Remote Launch, 2026-08-15] Commit=`7480ffc4`，remote prelaunch 27/27与resource smoke 3/3通过；GPU0/1/2启动50-run train/validation queue，PID=`3782929`。首批三个Weather profiles均已进入epoch1且loss finite；formal test=0、table mutation=false。Canonical launch=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/decoder_transfer_patchtst_hpo_v2_20260815/remote_launch.md`。Decision=`patchtst_decoder_hpo_v2_50_run_remote_train_validation_active_test_zero`。

[Historical Efficiency Profiler Result, 2026-08-14] 独占RTX 3090完成5 systems × 7 datasets=`35/35 service units`与77/77 immutable checkpoint objects；全部finite/CV gates通过，最大all-H round CV=`0.0368`，无test loader/labels访问。该九列latency/memory/storage profiler现保留为historical supplementary audit，不再是正文Table 3。它仍表明当前ISCF-BSCA latency/training time不领先，并禁止uniform efficiency或未实现的prefix-bounded speedup claim。Canonical historical result=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/efficiency_20260814/formal_results/result_and_table_audit.md`。
...
[Core-Ablation Author Correction, 2026-08-17] 作者提供复跑后的5 variants × 5 datasets aggregate MSE/MAE并要求替换固定Table 4。Full macro=`.305/.344`，在全部12/12 metric columns中为best；相对`w/o BSCA`、`w/o Target-Adaptive Allocation`、`Shared Scope Projection`与`Fixed Scope (s=144)`的display-precision macro MSE收益分别为3.481%、1.613%、2.866%、3.175%，aggregate accuracy directions均正。新输入不含per-horizon raw files、selectors或checkpoint hashes，因此旧100-cell/hash artifacts仅保留为historical audit，不作为修正数值的直接provenance；Figure 5 near-uniform allocation与8/40 alignment继续阻止reliable routing/causal specialization claim。Canonical result=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/core_ablation_20260814/formal_results/result_and_table_audit.md`。Decision=`core_ablation_author_corrected_aggregate_all_four_controls_positive`。
...
[H5D Training Complete and Formal-Test Gate, 2026-08-13] H5D于16:09:53完成48/48 train/validation jobs，test=0、failure=0。Artifact/provenance/four-H selector/numeric-health audit通过，48 checkpoint hashes唯一；manifest SHA256=`480180333de60c3f53d98c894b8854e4169401edcf7ca378d20f1b213e233a9e`。用户授权一次完整48-checkpoint/192-row formal test与冻结selector；H5B继续作为fallback，H5E/extra seeds/table mutation=false。Canonical gate=`analysis/iscf_bsca_main_v1_hpo_20260731/h5d_etth1_interaction_search_20260813/training_result_and_formal_test_gate.md`。Decision=`H5D_training_complete_48_checkpoint_manifest_frozen_formal_test_authorized`。

[H5D Formal-Test Result, 2026-08-13] H5D完成48/48 checkpoints、192/192 standard rows，artifact errors、temp remnants与checkpoint mutation均为0。48个profiles中只有3个通过MSE/MAE `1.002x H5B`双guard。唯一达到Main II `5/8` best的`h5d_bs16_lr3p6`以mean MSE `+0.675%`违反guard；eligible winner `h5d_bs16_lr2p4`仍为`4/8` best，虽达到`8/8` top-2但不能替换primary selection。H5B `h5b_seq640_p20`继续保留，Main I/Main II不改。Canonical result=`analysis/iscf_bsca_main_v1_hpo_20260731/h5d_etth1_interaction_search_20260813/formal_test_result_and_selection.md`。Decision=`H5D_no_eligible_best_cell_improvement_retain_H5B_profile`。
...
[H5A Formal-Test Launch, 2026-08-13] Exact commit=`cb496f31`通过48/48 training-artifact checker、manifest hash、zero target/temp artifacts与idle-GPU preflight。Once-only complete formal test于00:34:37启动，PID=`4169962`，matrix=`48 checkpoints × four H=192 standard rows`；首批三个ECL trials均进入GPU evaluation。Atomic publication与ABORT sentinel保持启用，48/48前禁止profile selection、partial table mutation或H5B。Canonical launch=`analysis/iscf_bsca_main_v1_hpo_20260731/h5a_main_ii_weak_dataset_search_20260810/formal_test_launch.md`。Decision=`H5A_formal_test_active_complete_matrix_required`。
...
[Main I Freeze and Main II H720-Prefix Prelaunch, 2026-08-08] 用户要求暂时固定Main I，并把MainII重定义为one-model-all-horizons system benchmark。Main I现以hash manifest冻结14 models × 7 dense datasets × four H的392 rows及29/56 best、19/56 second ranking；Main II对TimeAlign、QDF、AMD、SimpleTM、iTransformer、PatchTST、DLinear和ISCF-BSCA逐dataset只使用一个H720 model，以同一H720 test tensor裁剪H96/H192/H336。现有49个checkpoint objects可复用，iTransformer/PatchTST/DLinear需新训练21个H720 checkpoints；PatchTST/DLinear没有official Solar script/loader，Solar固定为source-patch-required。ISCF/TimeAlign/QDF/AMD/SimpleTM的H720须精确复现Main I，iTransformer/PatchTST/DLinear因Main I为published three-run mean只做single-seed deviation audit。Primary v1限当前七个dense datasets；Exchange在Main I完整H720 anchors闭合前deferred。该表是source-native system comparison，不作matched mechanism attribution。Canonical protocol=`configs/iscf_bsca_main_ii_h720_prefix_protocol.json`；prelaunch=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/main_ii_h720_prefix_20260808/design_and_prelaunch_gate.md`。Decision=`Main_I_hash_frozen_Main_II_H720_prefix_design_complete`。

[Section 4 BSCA Narrative-Order Refinement v0.7, 2026-08-10] Canonical draft=`docs/paper-drafts/iscf-bsca-method-initial-draft.md`已更新为`v0.7-bsca-narrative-order-refinement`。4.5首段dual-role总述保持不变，后文改为先完整定义varied-horizon的`Uniform-Prefix Forecasting Loss`及其dense-weight等价形式，再说明weighted contraction导致的probability-scaled multi-scope gradient问题，继而引出`Scope-Wise Forecasting Loss`与`Allocation-Balance Regularizer`。该顺序删除了在主目标定义前解释“second objective”的跳跃。公式、objective、implementation、Introduction v0.9、Section 3 v0.7、experiment cursor与claim boundaries均未改变。Decision=`section4_v0_7_bsca_narrative_order_refinement_pending_author_review`。

[Section 4 BSCA Training Refinement v0.6, 2026-08-10] Canonical draft=`docs/paper-drafts/iscf-bsca-method-initial-draft.md`已更新为`v0.6-bsca-training-refinement`。4.5现显式区分两类training roles：`Uniform-Prefix Forecasting Loss`负责varied-horizon prefix objective，`Scope-Wise Forecasting Loss`与`Allocation-Balance Regularizer`负责稳定multi-scope learning。Balance term只表述为惩罚strongly non-uniform Scope Probabilities、抑制少数scope早期主导并使probability-mediated gradients更均衡，不写成equal/sufficient training保证。Standalone 4.6已从Method删除；CHPC construction由4.4现有shared-target/prefix argument承接，parameter/FLOPs/latency/memory移入Section 5.4 efficiency/analysis。Implementation、Introduction v0.9、Section 3 v0.7与experiment cursor均未改。Decision=`section4_v0_6_bsca_training_refinement_pending_author_review`。

[Section 4 BSCA Objective Refinement v0.5, 2026-08-08] Canonical draft=`docs/paper-drafts/iscf-bsca-method-initial-draft.md`已更新为`v0.5-bsca-objective-refinement`。4.3将unified field、cross-scope coupling与shared-computation边界压缩为两句。4.5先明确joint optimization中的probability-scaled gradient问题，再依次定义`Uniform-Prefix Forecasting Loss`、`Scope-Wise Forecasting Loss`与`Allocation-Balance Regularizer`，并统一为$\mathcal L_{\mathrm{prefix}}$、$\mathcal L_{\mathrm{scope}}$与$\mathcal L_{\mathrm{balance}}$。Uniform-prefix term对dense prefix endpoints等权；scope-wise term提供allocation-independent direct supervision；ramped normalized $D_{\mathrm{KL}}(\mathbf q\Vert\boldsymbol\pi)$只抑制early concentration并拓宽gradient access，不保证equal inference usage、sufficient training或semantic specialization。Forward graph、implementation、Introduction v0.9、Section 3 v0.7与experiment authorization均未改。Decision=`section4_v0_5_bsca_objective_refinement_pending_author_review`。

[Section 4 Path and Allocation Refinement v0.4, 2026-08-08] Canonical clean draft=`docs/paper-drafts/iscf-bsca-method-initial-draft.md`已更新为`v0.4-path-and-allocation-refinement`，review artifact=`docs/paper-drafts/iscf-bsca-method-v0.4-highlighted-review.md`。两条decoder paths固定为`Scope Forecasting Path`与`Target-Adaptive Allocation Path`，并在4.1显式引出。Figure 4 caption压缩为path-level explanation。4.3不再以multiple models/slices表述ISCF，而强调scope-specific projections、shared Encoder/generator和single unified forecast field；相对$S$ complete forecasters只陈述避免duplicate encoder/generator computation与storage的construction fact。4.4更名为`Target-adaptive scope allocation`，按`history dynamics + Future Coordinate -> target-wise sharing-granularity preference -> unified-field contraction -> prefix-bounded varied-horizon output`重构；4.5--4.6同步path names、gradient coupling、CHPC与full-domain/prefix-bounded complexity边界。Current implementation仍full-field materialization后slice，未宣称realized latency、oracle allocation、specialization或effectiveness。Introduction v0.9、Section 3 v0.7、implementation与并行experiment authorization未改。Decision=`section4_v0_4_path_and_allocation_refinement_pending_author_review`。
...
[AMD / SimpleTM Main I Complete and Atomic Replacement, 2026-08-08] Recovery root已14/14 formal units完整结束；analyzer逐checkpoint重算SHA256并确认AMD 28/28、SimpleTM 82/82 raw rows、110/110 unique checkpoint hashes与56/56 aggregated cells。旧root三个partial units永久excluded。AMD七数据集macro MSE/MAE=`0.282132/0.328147`，SimpleTM=`0.292099/0.332380`；ISCF-BSCA相对两者分别低`6.970%/6.054%`与`10.144%/7.250%`。Main I已从数值层和表头同步移除CMoS/TimeBase并加入AMD/SimpleTM；更新后的14-model表中ISCF-BSCA为29/56 best、19/56 second。两列均标记official-native fixed-H context，不作matched attribution。Canonical report=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/main_i_final_amd_simpletm_20260808/result_and_table_audit.md`。Decision=`AMD_SimpleTM_complete_Main_I_atomic_replacement_pass_scope_closed`。
...
[AMD / SimpleTM Artifact-Recovery Gate, 2026-08-07] 用户通知完成后的审计确认首次formal queue实际在3/14首批units停止：SimpleTM upstream training `setting`遗漏`ii`占位符，native `itr` metrics产生但多个validation-selected checkpoints覆盖到同一路径，collector据此正确拒绝partial artifact。Failure=`artifact_collection_defect`，不是baseline性能失败；三个partial units不进入Main I。最小adapter仅在setting末尾追加repeat index，不改变official profiles、RNG推进、objective、selector或test边界。新root须重跑7个SimpleTM no-test smokes，再重启完整110-repetition queue；110/110前CMoS/TimeBase仍不得被替换。Canonical audit=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/amd_simpletm_main_i_reproduction_20260806/design_and_launch_gate.md`。Decision=`AMD_SimpleTM_first_queue_invalid_artifact_recovery_required`。

[AMD / SimpleTM Recovery Relaunch, 2026-08-07] Commit `b09c6e8`下的新root通过14/14 resource gate：7个新SimpleTM smokes与7个同contract AMD smokes合计14 unique checkpoint hashes、test=0、failure tokens=0；SimpleTM completion全部命中新patched run hash `8b9a027...`。完整56-cell/110-repetition queue于10:27:10后台启动，PID=`825838`，first wave=SimpleTM ECL/Solar/Weather，旧root三个partial units永久excluded。下一步只在new root完成通知后审计110/110并原子替换CMoS/TimeBase。Decision=`AMD_SimpleTM_repaired_remote_active_no_babysitting`。
...
[QDF Main I L336 Full Reproduction Launch, 2026-08-06] 用户要求把QDF从现有L96 mixed-source block改为`seq_len=336`下8 datasets × four H × seed2023的全本地复现。六个official-script datasets仅改变lookback，保留released逐Hprofiles；Solar和Exchange分别固定为ECL-derived与ETTh1-derived source-informed presets，不冒充official scripts。Experiment commit=`6eb8605`；8/8 H720 bounded smokes通过且test metrics=0，32-cell formal queue于19:31:57后台启动，PID=`3885616`，初始GPU0/1/2分别为ECL/Solar/Weather H720。32/32 artifact/hash audit前当前L96表保持不变，禁止混用partial L336 rows。Canonical record=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/qdf_main_i_l336_20260806/design_and_launch_gate.md`。Decision=`QDF_L336_8x4_remote_active_no_babysitting_wait_for_user_completion_notice`。

[QDF Main I L336 Complete and Table Rebuilt, 2026-08-06] QDF L336已32/32 checkpoints/losses/configs/metrics/logs完整结束，160-row artifact manifest、32 unique checkpoint hashes、逐profile config和numeric/log gates全部通过。七个dense datasets的QDF macro MSE/MAE=`0.287511/0.331426`，相对ISCF-BSCA高`9.541%/7.508%`，仅ETTm2-H192 MSE领先ISCF；相对TimeAlign高`5.166%/5.905%`。Main I的QDF列已全部改为本地seed2023 L336结果，Exchange companion加入QDF；完整14-model表中ISCF-BSCA仍为27/56 best、19/56 second。上一版QDF L96 mixed-source block退为historical。Canonical report=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/qdf_main_i_l336_20260806/result_and_table_audit.md`。Decision=`QDF_L336_32_of_32_complete_Main_I_replacement_pass_scope_closed`。
...
[H4L Complete Formal-Test Authorization, 2026-08-04] 用户显式授权H4L一次完整formal test。Scope固定为48个frozen seed2021 checkpoints × H96/H192/H336/H720=192 rows并同时报告MSE/MAE；禁止checkpoint retraining/mutation、validation profile filtering、partial execution以及per-H/per-seed/per-metric/per-cell selection。H4M、selected-profile confirmation、baseline和3-seed不在本授权内。Machine contract=`configs/iscf_bsca_main_v1_hpo_wide_h4l_test_audit.json`。Decision=`H4L_complete_48_checkpoint_formal_test_authorized_prelaunch`。

[H4L Training Complete and Formal-Test Gate, 2026-08-04] H4L 48/48 train/validation jobs与artifacts完整，numeric health、effective-config provenance、frozen config/search-space hashes和48个unique checkpoint SHA256均通过；best epoch范围1--47，test=0/48。ETTm2 `patch1`刷新H1--H4K validation frontier 1.622%，Weather `patch4`未刷新历史frontier（-0.625%），故不作validation profile selection。48-row manifest SHA256=`c7ce6b6915dbe0323282140c0ed28ecad590b5ea256e8545a7f0fb3217c25584`。Canonical report=`analysis/iscf_bsca_main_v1_hpo_20260731/h4l_training_result_and_formal_test_gate.md`。Decision=`H4L_training_complete_manifest_frozen_formal_test_authorized`。
...
[H4J Resource-Smoke Repair, 2026-08-02] 首次40-job smoke中37 profiles通过，3个`seq_len=512` profiles在trainer参数检查处因原patch count不能整除512而fail-close；正式training未启动、test=0。三项profile在同一H4J search contract内改为`patch_num=16`，checker新增`seq_len % patch_num == 0` invariant，并要求使用新output root重跑完整40-job smoke。Failure attribution=`source_protocol_compatibility_fault`，不构成OOM、numeric pathology或architecture failure。

[H4J Training Launch, 2026-08-02] Repair commit=`a7d2378`；新的`h4j_r1` output root完成40/40 two-batch resource smoke，无OOM/NaN/Inf，smoke peak约4.1 GiB。Full 40-job train/validation于12:10:47在GPU0/1/2启动，PID=`4032407`；首批为三个ECL profiles，observed memory约2.0--2.1 GiB、utilization 69--77%，test=0/40。Conservative ETA=24--40小时。Canonical launch=`analysis/iscf_bsca_main_v1_hpo_20260731/h4j_launch.md`。Decision=`H4J_40_job_training_active_test_zero`。

[H4J Training Complete and Test Retry-2 Prelaunch, 2026-08-03] H4J于06:01完成40/40 checkpoints，validation/artifact/numeric-health均40/40 pass，40个checkpoint SHA256唯一。首次test launch因generic evaluator authorization wiring在test loader前fail-close；`test_audit_r1`通过preflight并进入三个ECL evaluators，但因`yingch` quota达到220 GiB而在atomic publication前全部失败，published artifacts仍0/40。经用户授权删除五个closed-stage remote raw roots、H4J resource-smoke及1.2 GiB无效partial `_tmp`后，quota降至179 GiB；H4J checkpoints及既有正式结果未删除。Retry-2使用clean root=`test_audit_r2`和manifest SHA256=`8b9465adbf00a5812b373c0c850ab001b5e55ba1552180faf6f75e21c8a2ca1d`。不做validation profile ranking；complete official-test将评估160个standard-horizon rows，final selection延后到H1--H4J全部93 trials的joint selector。Canonical report=`analysis/iscf_bsca_main_v1_hpo_20260731/h4j_training_result_and_test_prelaunch.md`。Decision=`H4J_40_of_40_training_complete_direct_test_retry2_prelaunch`。
...
[Section 3 Field-Style Alignment v0.4, 2026-08-03] 用户指出v0.3仍偏Nature式短句与
system-contract语体，与Introduction及time-series forecasting顶会论文不一致。本轮以
iTransformer、TimeMixer、TimeXer和TimeMixer++官方论文为style reference，P1改为
单一承上启下段，删除原meta roadmap P2；3.1--3.5统一为
`setting -> notation -> contrast -> measurement -> observation -> implication`。
3.3标题改为`Accuracy under naive unified forecasting`，但stable unified penalty仍未
建立。Definitions、figures、numbers与claim boundaries均未变化。Style audit=
`analysis/iscf_bsca_section3_style_calibration_20260803/style_audit.md`。Decision=
`section3_v0_4_field_style_alignment_pending_author_review`。

[Section 3 Concise Polish v0.3, 2026-08-03] 基于用户对v0.2仍偏冗长的反馈，使用
`nature-polishing`按algorithmic paper、methods+results、English、generic journal
路由完成段落级压缩。Manuscript body由2,308词降至1,576词，主要删除重复释义、
重复claim boundary与过细的future CFH protocol；Figure 2--3 captions同步压缩。
CHPC、CHPD/NCHPD、$\operatorname{UP}_H$、$R_{o,b,s}$、CFH、matched controls、全部
证据数值及validation-only边界保留，3.5之前不引入production method名称。
Introduction v0.9、Figures 2--3图像、并行experiment状态与授权均未改变。Decision=
`section3_v0_3_concise_polish_pending_author_review`。
...
更强local flexibility但可能增加variance与优化难度；broad sharing提供结构约束但可能增加local-detail bias。只有当
不同future regions的bias--variance optimum稳定不同时，才能支持region-dependent sharing demand；multi-scale
component importance本身不能自动完成该推导。

现有decoder不作“global versus independent”的简单二分：iTransformer和PatchTST属于shared representation加
step-specific projection rows；DLinear具有step-specific linear rows但共享输入分解与joint training；N-HiTS使用
预定义multi-scale trajectory synthesis。问题的matched empirical manifestation仍称
`region-dependent sharing-scale preference`，ISCF的方法层仍称`future-step coupling scope`。

Problem Formulation and Empirical Motivation的Evidence III同步改为`Future-Region Sharing-Demand Heterogeneity`。
primary evidence使用simple baseline上的matched end-to-end diagnostic predictors，要求region-wise risk crossing、
best-scale变化和相对best fixed scale的headroom；frozen representation与data-side multi-scale energy只作
secondary/descriptive evidence。v0.3的`future-region predictive-structure heterogeneity`因边界过宽且缺少推理桥梁而
废弃。本次只更新paper formulation，不改变ISCF-BSCA代码、frozen results或training/test authorization。

[Introduction P1--P3 v0.2 Consensus, 2026-07-24] 根据逐段讨论，论文架构文档升级为v0.2。Introduction prose使用
`future time step`；当时暂定的`future-step coupling granularity`问题表述已由后续三层术语取代，
...
universal conditional specialization。Introduction第4段已由上方v0.4共识冻结，第5--6段及后续章节仍为
...
## 2026-09-01 UVHF KBS submission freeze
