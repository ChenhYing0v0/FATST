...
| `current_11_step` | Phase5-A4R：Step 3/4/11，现有 training-log signal 诊断完成，下一步设计 A4S validation-prefix signal export |
...
| `current_gate` | A4R 证明现有 training-log signals 太粗：ALL 最强 Spearman 只有 `0.321`，不能支撑 learned routing |
| `paper_core_status` | Stage A interface 主线暂定重构为 `Reliability-Aware Capacity-Preserving Interface`，但必须先通过 A4S 的 prefix-wise validation signal gate；若失败，回 Step 2/3 重审 interface contribution |
