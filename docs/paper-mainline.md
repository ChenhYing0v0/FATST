...
| `current_11_step` | Phase5-A3E：Step 6/7/8，验证 target-conditioned nested primary interface |
...
| `current_gate` | Stage A3E narrative_gate 已通过；effectiveness_gate 要求 target conditioning 相对 A3C/A3D 有增量；本轮用 ETTm1 替换 ETTm2，检验 minute-level failure 是否可泛化 |
| `paper_core_status` | A3D 为 partial pass；teacher preservation 有效但不足以作为最终 paper-core，A3E 是当前 primary nested interface 主线 |
...
- 若两者都失败，则优先评估 A3F 最小组合或重审 Stage A interface family。
