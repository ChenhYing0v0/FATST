...
| `current_11_step` | Phase5-A4S：Step 9/10/11，validation-prefix signal diagnostic 已完成，Stage A 回 Step 2/3 重评估 |
...
| `current_gate` | A4S 未通过 signal-existence gate：ALL 最强 `teacher_student_mae` Spearman 仅 `0.388`，dataset-level signals 方向不一致 |
| `paper_core_status` | Stage A interface/routing-from-existing-paths 路线不能继续推进；贡献 1 必须回 Step 2/3 重评估，不能简单降级成 diagnostic，也不能直接删除而不重构论文主线 |
...
2. **Reliability-Aware Capacity-Preserving Interface**
   设计一个比 H1 target-set 和 H1C row-gated 更强的 unified prediction interface。它不能只是
   full-720 crop、prefix loss 或 post-hoc row calibration；也不能写成按 dataset/horizon
   手工切换 head。当前证据更支持：不同 capacity-preserving paths 的可靠性随 future context
   变化，因此必须先找到训练期或 validation-time 可观测的 reliability signal，再决定是否做
   learned/estimated interface routing。
...
- Stage A 必须回 Step 2/3 做贡献重评估：如果保留 interface 贡献，需要重新定义问题和机制；
  如果放弃 interface 贡献，必须同步重构 paper-mainline，使论文仍满足高水平 SCI 的创新性和工作量要求。
