...
3. **Capacity preservation**
   ...
   preservation 必须来自 decoder/operator class、参数容量对照、function-preserving constraint
   或明确的 teacher/control 证据；不能再把随机 row-copy 或 pretrained dense anchor 当作 paper-core
   机制。
...
A5 narrative gate result after review：

- `A5_pcf_prefix_consistent_function_preserving_decoder` 的 `narrative_ready` 结论已撤回；
- 复评判断：active trained dense anchor + cumulative correction 更像旧阶段机制混合与 residual repair，
  不能支撑“重新设计 unified head”的 paper-core 叙事；
- 预训练 anchor 可以作为 diagnostic/control 讨论，但不能作为 paper-core architecture 的必要组成；
- A5 已回 Step 4/5 并重新提出 prefix request 进入主生成路径、且不依赖 pretrained full-head anchor
  的 unified head；
- 复评说明见 `docs/experiments/phase5-a5-capacity-preserving-prefix-consistent-decoder.md`。

A5 first-principles candidates：

- `A5-Q_elastic_causal_target_query_decoder`：future position queries + prefix-causal / structured mask；
- `A5-B_continuous_forecast_basis_operator`：continuous forecast function/operator；
- `A5-S_step_specific_hypernetwork_head`：coordinate-conditioned generated readout weights；
- `A5-I_cumulative_innovation_process_decoder`：trajectory innovation process；
- `A5-M_masked_future_placeholder_head`：future placeholders + structured mask。

A5-Q/A5-B narrative gate result：

- `A5-Q_elastic_causal_target_query_decoder` 通过 narrative gate：贡献边界是
