...
Current cursor=`Section 6 v0.4 temporarily frozen usable`。Author确认当前三段Discussion and Limitations可作为论文可用版本；正文、paragraph structure、interpretation与methodological limitations均暂时固定，只有Section 7或全文组装产生明确矛盾且author批准时才解冻。Decision=`section6_v0p4_temporarily_frozen_usable`。
...
Current cursor=`new Efficiency contract complete -> Table 3 and Figure 6 synchronized -> author review`。正文只保留ISCF-BSCA、TimeAlign与QDF，因为三者同时具备完整Main-I accuracy、与checkpoint配置匹配的parameter counts和native timing logs。DLinear/PatchTST H720-prefix是另一类one-model service，其余Main-I systems缺少完整7×4 local cost evidence，均不混入本表。

Accuracy为seven datasets × four horizons的Main-I macro MSE/MAE；parameters按每dataset完整four-horizon service统计，ISCF计一个unified model，TimeAlign/QDF分别加和四个fixed-H models；one-epoch cycle取每checkpoint completed native log中的`train + scheduled validation, no test`中位数，baseline再加和四个fixed-H models。完整性=`84/84 accuracy cells + 21/21 parameter units + 63/63 timing logs`，new training/formal test=`0/0`。

ISCF-BSCA为`0.261/0.306 MSE/MAE, 2.926M params, 133.1 s/epoch`；TimeAlign为`0.274/0.314, 10.741M, 104.6 s`；QDF为`0.288/0.331, 5.337M, 55.4 s`。因此ISCF-BSCA同时获得更高accuracy和更低four-horizon parameter count，但one-epoch cycle更慢，Decision=`efficiency_accuracy_params_epoch_complete_accuracy_parameter_advantage_compute_tradeoff`。Canonical report=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/efficiency_accuracy_params_epoch_20260817/result_and_table_audit.md`；旧2026-08-14 profiler降为historical supplementary audit。
...
Figure 6已同步新Efficiency合同，把Main-I macro MSE与four-horizon total parameters、seven-dataset macro one-epoch cycle seconds编码为accuracy--system-cost plot；caption与正文同时披露当前ISCF-BSCA one-epoch cycle不领先。Figure 7以paired bars呈现two backbones × Weather/ETTm1/ETTm2 aggregate MSE，并披露author-refined post-hoc scope、historical negative blocks与unsynchronized per-H provenance。Figure source/QA bundle=`analysis/iscf_bsca_section5_figures_20260817/`，canonical assets=`paper-figures/figure_6_accuracy_system_cost.*`与`paper-figures/figure_7_decoder_transfer.*`。Decision=`section5_v0_1_evidence_complete_initial_draft_pending_author_review`；next=author逐节审阅，之后再起草Sections 6--7。
...
Formal结果：70/70 jobs、280/280 new cells、300/300 candidate cells完整；selected HPO相对v1 BSCA改善`2.128%/1.719%`，相对Original仍为`-0.505%/-0.750%`。MSE赢4/5 datasets，但ETTh1退化`3.110%`使macro gate失败。一个variable-scope invariant compatibility defect被exact repair，不改变模型或metrics。Rollback=Step 4--6并关闭该rescue；不补matched ISCF、不再追加test-informed HPO。Canonical result=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/decoder_transfer_itransformer_hpo_v2_20260816/formal_results/result_and_decision.md`；Decision=`itransformer_decoder_hpo_v2_improves_v1_bsca_but_fails_original_gate_close_rescue`。
...
Current cursor=`Core-Ablation author-corrected aggregate complete -> paper consolidation`。2026-08-17作者提供复跑后的5-variant × 5-dataset four-H aggregate表。Full macro MSE/MAE=`.305/.344`，12/12 metric columns为best；相对prefix-only `w/o BSCA`、equal-fusion `w/o Target-Adaptive Allocation`、capacity-matched `Shared Scope Projection`与`Fixed Scope (s=144)`，Full的display-precision macro MSE收益分别为3.481%、1.613%、2.866%、3.175%，五数据集MSE/MAE方向均5/5。新输入不含per-horizon raw files、selectors或checkpoint hashes；旧100-cell/hash artifacts保留为historical audit，不冒充修正表provenance。

Four-layer decision：paper-facing dataset-aggregate table支持4/4 accuracy directions；per-horizon/checkpoint rerun provenance未闭合；Figure 5仍显示near-uniform utilization与8/40 alignment，因此不能从accuracy gain推出reliable region-best routing或causal specialization。Canonical report=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/core_ablation_20260814/formal_results/result_and_table_audit.md`。Decision=`core_ablation_author_corrected_aggregate_all_four_controls_positive`；next=`Sections 5--7 prose with explicit provenance and internal-health boundary`。
...
Formal-test exact-commit/manifest/zero-artifact/GPU preflight已通过；完整48-checkpoint queue于
2026-08-13 00:34:37启动，PID=`4169962`。Current cursor=`Step 9 formal test active`；
48/48前不执行selector或table mutation。Canonical launch=
`analysis/iscf_bsca_main_v1_hpo_20260731/h5a_main_ii_weak_dataset_search_20260810/formal_test_launch.md`。
...
四个subsection标题同步改为`Fixed-horizon multi-step forecasting`、`Unified and varied-horizon forecasting`、`Forecast generation and output-side modeling`与`Multi-scale forecasting and adaptive allocation`。Introduction v0.9、Section 3 v0.7、Section 4 v0.7及其冻结状态不变；main/ablation/transfer claims仍由Section 5 tables兑现；并行experiment cursor与authorization不变。Decision=`related_work_v0_2_author_structure_refinement_pending_review`。
...
Exact commit=`7544f76d`的48/48 resource smoke已通过，test=0且48 checkpoint hashes唯一；
full three-GPU queue于2026-08-10 15:13:32启动，PID=`2375625`。Current cursor=
`Step 8 remote training active`；下一步是48/48 artifact audit与immutable manifest，而不是
中途validation筛选或提前test。Canonical launch=`analysis/iscf_bsca_main_v1_hpo_20260731/h5a_main_ii_weak_dataset_search_20260810/remote_launch.md`。
...
Final resource gate=`14/14 checkpoints, test=0, 2× storage projection=4.470 GiB`。Formal queue于22:14:19在commit `014b068`上启动，PID=`4100426`，GPU0/1/2首批为SimpleTM ECL/Solar/Weather，initial failure tokens=0。Current cursor=`Step 8 remote active / no babysitting`；下一步等待用户完成通知后执行110/110 audit与Main I原子替换。
...
## H4N Weather-only Wide HPO Prelaunch (2026-08-05)

Current experiment cursor=`Step 7 local protocol gate -> Step 8 remote smoke/training`。H1--H4M共189 trials、其中56个Weather profiles的复审确认：`L640/p20/lr5e-5`是four-H MSE frontier=0.214752，`L512/p16/lr2e-5`是MAE/joint frontier=0.246084；Weather已在H192/H336/H720 MSE与H720 MAE领先，剩余问题是H96--H336 MAE。用户授权扩大Weather超参数范围后，H4N冻结40个与历史零重复的seed2021 profiles：16 context×LR、8 LR boundary、8 patch geometry、5 mode-rank、3 capacity。主selector改为dataset four-H MSE/MAE relative mean优先，仅0.1% near-tie用lead cells；仍禁止per-H/per-metric/per-cell selection。训练120 epochs/patience24，test=0；40/40 manifest后一次完整formal test已授权。Remote preflight为GPU0--2各18 MiB/0%，quota169G/200G soft。Canonical prelaunch=`analysis/iscf_bsca_main_v1_hpo_20260731/h4n_weather_wide_matrix_and_prelaunch_20260805.md`。Decision=`H4N_frozen_local_gate_then_remote_launch_no_automatic_H4O`。

Exact commit=`ba17fc9`已完成40/40 resource smoke且test=0；full queue于10:47:16在GPU0--2启动，PID=`1397808`，初始jobs进入epoch2，memory约1.6--1.7 GiB。Current cursor=`Step 8 H4N train/validation active`；40/40 manifest前不进行formal test。Canonical launch=`analysis/iscf_bsca_main_v1_hpo_20260731/h4n_weather_remote_launch_20260805.md`。
...
3.3撤销`region-wise MSE`专有命名。当前manuscript正式定义future-region prediction risk $R_{o,b,s}$为给定forecast origin、future region和sharing extent后，在该region全部future steps与variables上聚合的empirical squared-error loss；MSE只说明计算形式，risk承载extent--region匹配关系，并明确区别于population expectation。Figure 3a同步改为`Excess prediction risk`。数据、数值、layout、validation-only evidence role、oracle boundary、Introduction v0.9与parallel experiment authorization均不变。Decision=`section3_v0_7_author_risk_definition_refinement_pending_review`。
...
原3.3 naive-unified accuracy与原3.5 Design Requirements已删除，future-region sharing renumber为3.3。该节从CHPC过渡到decoder information organization，matched predictors只保留“除sharing extent外完全一致”的正文表述；Figure 3a解释region-wise excess risk与winner diversity，Figure 3b单独解释相对best fixed $s=720$的8.112% descriptive oracle headroom。为避免claim promotion，Figure 2 caption保留aggregate-CHPD selection disclosure，Figure 3正文保留一句validation-label oracle定性。新增sharing-extent schematic未进入本版，因为Figure 3a已经直接显示extent×region关系，额外图会与planned Method Figure 4重复。Introduction v0.9、existing figure assets、method identity与experiment authorization均未改变。Decision=`section3_v0_5_author_structure_refinement_pending_review`。
