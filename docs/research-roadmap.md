...
[Diagnostic Update] Existing-residual projection audit 已完成：
...
- specialist gap rows:
  `4/12`;
- gap mean residual top16 energy share:
  `0.789`;
- non-gap mean residual top16 energy share:
  `0.828`;
- gap minus non-gap top16-over-label ratio:
  `-0.054`;
- H720 segment gap vs non-gap top16 reconstruction share:
  `0.716` vs `0.714`。

[Decision] Component-space residual structure 存在，但 known gaps 不集中在 dominant top
components。因此不采用 top-only TransDF-style loss 作为第一实现。下一步进入
`Component-Balanced Objective` 设计：
