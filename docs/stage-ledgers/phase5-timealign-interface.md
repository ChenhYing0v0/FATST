...
| `B6-PLO` | `protocol_ready` | A6-LBF 的 architecture 已在 learned-basis space 中建模，但当前 supervision 仍主要是 time-domain point loss；可能缺少 prefix-native label/basis objective | not evaluated：requires train-only label-autocorrelation and coefficient-residual diagnostic | not evaluated | Run offline diagnostic before any objective implementation | `docs/experiments/phase5-stage-b-prefix-native-label-objective-diagnostic.md` |
