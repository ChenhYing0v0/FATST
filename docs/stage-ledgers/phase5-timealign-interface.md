...
| `latest_decision` | B11-BCF small gate is blocked by required controls: B11 vs A6 `-0.10%` mean MSE, but B11 vs no-basis is only `-0.0012%` with `2/12` wins and B11 is worse than constant-slot by `+0.03%` |
| `next_required_action` | Do not promote B11-BCF; decide whether to redesign B11 at Step 4 with a stronger intervention point or rollback StageB to Step 2/3 |
| `rollback_point` | Current B11-BCF implementation is blocked by `capacity_control_explains`; broader emergent-basis direction is not fully rejected, but any continuation must redesign before launch |
...
| Decide B11 redesign or rollback | Codex | B11-BCF small gate is explained by no-basis/constant-slot controls | `pending` | Choose Step 4 redesign with stronger intervention, or rollback StageB to Step 2/3 architecture search |
