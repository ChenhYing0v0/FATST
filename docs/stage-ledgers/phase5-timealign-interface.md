...
| `latest_decision` | full contextual encoder replacement failed (`+4.14%/+4.80%`), but exact hierarchical repair passes 3/3 datasets with zero output/metric diff and canonical memory `[B,C,30,48]` |
...
| `B14-PRE-HPM` | `hierarchical_patch_memory_ready` | B14 requires a common local patch axis without sacrificing accepted A6 performance | full CPE replacement failed；parameter-free hierarchical repair has exact A6 state/parameter/output/metric equivalence and `[B,C,30,48]` memory | prerequisite only；not a retrieval method | Release B14-FURD Step 3；keep contextual replacement closed | `docs/experiments/phase5-stage-b-b14-prerequisite-patchwise-encoder.md`; `analysis/phase5_stage_b_b14_prerequisite_patchwise_encoder_20260710/hierarchical_patch_memory_gate_report.md` |
...
| B14 hierarchical patch-memory gate | `B14-PRE-HPM` | exact carrier-preservation repair | 3/3 datasets have equal state keys/parameters, output and full metrics max diff `0.0`, memory `[B,C,30,48]` | `hierarchical_patch_memory_ready`; release B14 Step 3 | `analysis/phase5_stage_b_b14_prerequisite_patchwise_encoder_20260710/hierarchical_patch_memory_gate_report.md` |
...
| Run B14-FURD demand mismatch diagnostic | Codex | HPM prerequisite exact pass | `pending` | Rewrite analyzer for 30 overlapping patch supports and run U180/U240 checkpoint diagnostic |
