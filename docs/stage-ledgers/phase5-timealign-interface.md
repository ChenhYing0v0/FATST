...
| `next_required_action` | Step 8/9：A6 capacity-native remote gate 已在 3090 启动，等待 artifacts 返回后分析 A6-DER ceiling 与 A6-LBF-r256/r512 |
...
| `A6-DER_prefix_native_dense_equivalent_row_bank` | `remote_running` | 若完全保留 dense row capacity 且只改成 prefix-native invocation，应该能判断 Stage A bottleneck 是否真在 head capacity | not_required：作为 capacity ceiling/control，不单独宣称 paper-core | local smoke passed；remote gate running | 等返回后判断 DER 是否恢复 dense/full-head capacity；若 DER 仍失败，回 Step 2/3 重审 multi-prefix objective / representation conflict | `docs/experiments/phase5-a6-capacity-native-unified-head-mechanisms.md`; `docs/code-explanation/phase5-a6-capacity-native-heads.md` |
| `A6-LBF_learned_basis_forecast_operator` | `remote_running` | 用 learned temporal basis + hidden coefficients 构造 prefix-consistent forecast operator；`K=720` 可 dense-equivalent，较小 rank 测试 intrinsic rank | conditional_pass：anchor-free、非 residual、prefix-native，且直接修复 A5-B fixed-basis under-capacity；必须和 A6-DER ceiling 一起解释 | local smoke passed；`r256/r512` remote gate running | 返回后比较 LBF-r256/r512 vs A6-DER ceiling、A5-B 与 best stage controls | `docs/experiments/phase5-a6-capacity-native-unified-head-mechanisms.md`; `docs/code-explanation/phase5-a6-capacity-native-heads.md` |
...
| A6 capacity-native remote gate | Codex | A6 local smoke passed | `remote_running` | 已在 3090 启动 9-run gate：A6-DER、A6-LBF-r256、A6-LBF-r512 × Weather/ETTm1/ETTh2 |
