...
| `local_protocol_patch_authorized` | completed for Figure 5 read-only artifact analysis；no model/training patch |
...
2026-08-19 Figure 6按最新Efficiency证据重做为8-system bubble chart：y=`macro MSE`（轴标题=`MSE`），x=`four-horizon checkpoint storage (log)`，bubble area=`peak inference memory`。SimpleTM按作者明确要求只从Figure 6过滤，完整Table 3/audit不删除；DLinear/iTransformer/PatchTST/TimeMixer的architecture-equivalent resource role保留在caption，图内不使用$\dagger$或额外footnote，ISCF-BSCA以`ours`上标突出。该图支持accuracy--resource trade-off，不支持uniform superiority。Canonical=`paper-figures/figure_6_accuracy_system_cost.*`；training/formal test=`0/0`。
...
2026-08-16 Figure 5复用5个Full与5个`Fixed Scope (s=144)`冻结validation artifacts完成，不训练、不访问test。20/20 CHPC comparisons的maximum absolute CHPD均为0；scope-arm macro region-best随future region变化，最大excess MSE为6.123%。Learned utilization总体接近均匀（0.18258--0.21479），highest-utilization与lowest-MSE scope仅8/40 cells一致，因此只支持descriptive arm heterogeneity；Figure 5自身不建立accuracy attribution、successful routing或causal specialization。2026-08-17作者修正Table 4独立支持aggregate allocation accuracy direction。Weather row 77按完整1,280-row pool冻结规则选为illustrative trajectory。Canonical result=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/figure5_mechanism_diagnostics_20260816/result_and_decision.md`。Decision=`figure5_complete_mixed_mechanism_evidence`；current main-text experiment matrix closed。
...
Formal audit完成70/70 checkpoints与280/280 new cells；加入v1 BSCA reference后300/300 candidate cells完整。Dataset-level selector选择Weather/ETTm1/ETTm2/ETTh1/ETTh2=`p03/p11/p03/p08/p04`。相对v1 BSCA macro MSE/MAE改善`2.128%/1.719%`；相对Original则为`-0.505%/-0.750%`，虽赢4/5 dataset MSE means与12/20 MSE cells，但ETTh1仍退化`3.110%/3.376%`，overall gate FAIL。一个旧scope-invariant checker defect经exact protocol repair后断点恢复；checkpoint、model、test loader与selection rule未改变。Matched ISCF、extra HPO/seeds和table mutation不启动。Canonical result=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/decoder_transfer_itransformer_hpo_v2_20260816/formal_results/result_and_decision.md`。Decision=`itransformer_decoder_hpo_v2_improves_v1_bsca_but_fails_original_gate_close_rescue`。

2026-08-17作者提供Core-Ablation复跑后的dataset-level four-horizon means并要求替换固定表。Full macro MSE/MAE=`.305/.344`且12/12 metric columns为best；相对四个controls的display-precision macro MSE gains=`3.481%,1.613%,2.866%,3.175%`，aggregate directions为4/4 positive。新输入未附per-horizon raw files、selectors或checkpoint hashes；2026-08-14的100-cell/hash artifacts保留为historical audit，不冒充修正表直接provenance。Figure 5 internal health仍为near-uniform utilization与8/40 alignment，故只支持allocation accuracy gain，不支持reliable routing/specialization。Decision=`core_ablation_author_corrected_aggregate_all_four_controls_positive`；canonical report=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/core_ablation_20260814/formal_results/result_and_table_audit.md`。
...
2026-08-13用户随后要求利用ETTh1训练较快，将HPO matrix相对H5B扩大约50%。H5C冻结54个seed2021 profiles，分为18 local context/patch、10 winner-LR、7 winner-dropout、5 winner-WD、5 winner-rank、5 LR/dropout与4 context/dropout interactions；与此前61个ETTh1 effective jobs零重复。Capacity因历史负向不再扩张。Current Main II=`4/8` best，minimum=`5/8`、stretch=`6/8`；mean MSE/MAE双guard=`1.002×H5B`。Training test=0，54/54 manifest后一次完整216-row formal test conditional authorized；table mutation=false。Canonical prelaunch=`analysis/iscf_bsca_main_v1_hpo_20260731/h5c_etth1_refined_interaction_search_20260813/design_and_prelaunch_gate.md`。Decision=`H5C_frozen_authorized_remote_resource_smoke_next`。

H5C exact commit=`fe9ac10b`于11:40:12--11:42:32完成54/54 resource smoke：54 checkpoints/metrics/configs、54 unique hashes、failure=0、test=0，quota188G。Full train/validation于11:43:04在GPU0/1/2启动，PID=`843554`；首批L736/L726/L713 jobs均完成epoch2且validation finite，显存766--784 MiB。Canonical launch=`analysis/iscf_bsca_main_v1_hpo_20260731/h5c_etth1_refined_interaction_search_20260813/remote_launch.md`。Decision=`H5C_three_GPU_train_validation_active_test_zero`。
...
Remote精确清理五个旧resource-smoke目录后quota由189G降至186G；exact commit=`21df4c80`完成48/48 H5D resource smoke、48 unique hashes、failure=0、test=0。Full train/validation于14:43:23在GPU0/1/2启动，PID=`1092478`；首批batch16 jobs已完成epoch1，memory约561--562 MiB。Canonical launch=`analysis/iscf_bsca_main_v1_hpo_20260731/h5d_etth1_interaction_search_20260813/remote_launch.md`。Decision=`H5D_three_GPU_train_validation_active_test_zero`。

H5D train/validation于16:09:53完成48/48，training test=0、failure=0；48个checkpoint hashes唯一，best epoch范围1--3。Immutable manifest SHA256=`480180333de60c3f53d98c894b8854e4169401edcf7ca378d20f1b213e233a9e`。用户明确授权继续一次完整192-row formal test与冻结selector；H5E、extra seeds及table mutation仍false。Canonical gate=`analysis/iscf_bsca_main_v1_hpo_20260731/h5d_etth1_interaction_search_20260813/training_result_and_formal_test_gate.md`。Decision=`H5D_training_complete_48_checkpoint_manifest_frozen_formal_test_authorized`。

H5D formal test于16:28:05--16:30:50完成48/48 checkpoints与192/192 rows，errors/temp/checkpoint mutation均为0。只有3个profiles通过双guard；唯一达到5/8 Main II best的`h5d_bs16_lr3p6`因mean MSE `+0.675%`而被排除。Eligible winner `h5d_bs16_lr2p4`为4/8 best、8/8 top-2，未严格超过H5B。冻结selector保留`h5b_seq640_p20`；Main I/II不变。Canonical result=`analysis/iscf_bsca_main_v1_hpo_20260731/h5d_etth1_interaction_search_20260813/formal_test_result_and_selection.md`。Failure=`objective_tradeoff_and_search_space_performance_shortfall`，rollback=Step 6 retain H5B。Decision=`H5D_no_eligible_best_cell_improvement_retain_H5B_profile`。
