...
2026-08-16 Figure 5复用5个Full与5个`Fixed Scope (s=144)`冻结validation artifacts完成，不训练、不访问test。20/20 CHPC comparisons的maximum absolute CHPD均为0；scope-arm macro region-best随future region变化，最大excess MSE为6.123%。Learned utilization总体接近均匀（0.18258--0.21479），highest-utilization与lowest-MSE scope仅8/40 cells一致，因此只支持descriptive arm heterogeneity，不支持successful allocation或accuracy attribution。Weather row 77按完整1,280-row pool冻结规则选为illustrative trajectory。Canonical result=`analysis/iscf_bsca_paper_experiment_consolidation_20260731/figure5_mechanism_diagnostics_20260816/result_and_decision.md`。Decision=`figure5_complete_mixed_mechanism_evidence`；current main-text experiment matrix closed。
