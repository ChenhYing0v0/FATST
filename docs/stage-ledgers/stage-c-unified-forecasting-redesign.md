...
| `active_question` | post-CPSI rollback：哪类non-absorbable scope operator值得重新过Step4/5？ |
| `source_evidence` | historical/source-faithful `A6-LBF-r256` |
| `mechanism_control` | same-run end-to-end `A6-LBF-natural-baseline`；frozen A6只作reference/diagnostic |
| `active_candidates` | none；ISCF-v0/A6_FULL frozen references；LINEAR optimization evidence only |
| `future_validation_suite` | ETTh1, ETTh2, ETTm1, ETTm2, Weather；five profiles frozen |
| `paper_facing_scorecard` | validation/test H96,H192,H336,H720 MSE/MAE；dense默认diagnostic |
| `restart_handoff` | `docs/stage-ledgers/stage-c-post-d21-d22-restart-handoff-20260720.md` |
| `stage_exit` | 新problem先通过existence/narrative gate，再决定一项或两项可归因contributions |
| `stage_rollback` | CPSI material test fail；return Step4/5；no seed/rank/loss/router rescue；CTD paused |

## Decision Cursor

| Field | Content |
| --- | --- |
| `current_11_step` | CPSI Step9/10 complete；rollback Step4/5 |
| `current_candidate` | none；ISCF-v0 retained strong carrier |
| `latest_decision` | `cpsi_v1_exact_performance_fail_return_step4_5` |
| `next_required_action` | source-informed operator problem/function-class audit；no implementation |
| `method_training_authorized` | false；formal matrix complete；confirmation false |
| `rollback_point` | Step4/5；broader interaction unresolved，exact product closed |

## 11-Step Record

| Field | Current Record |
| --- | --- |
| `current_step` | ISCF-v1-CPSI Step9/10 complete；return Step4/5 |
...
| `design` | five production readouts；81/81 local；19/19 prelaunch；25 new / 35 effective matrix frozen |
| `narrative_gate` | Step6 pass；Step7A/7B pass；effectiveness pending |
| `effectiveness_gate` | fail：vs ISCF MSE/MAE -2.2128%/-1.6987%；internal pass；LINEAR tie only |
| `artifacts` | `analysis/stage_c_post_d21_unconstrained_reset_20260720/iscf_v1_cpsi_step9_10_20260721/step9_10_result_and_rollback.md` |
| `decision` | exact CPSI-v1 closed；active method none；return Step4/5 |
