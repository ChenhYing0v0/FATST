...
| `next_required_action` | 实现并验证 SC0 三臂 global carrier calibration；不得先实现 SC1/SC2 method |
...
| Implement SC0 config profile and runner | Codex | user continues StageC execution | `pending` | 添加独立 config、profile override、runner、analyzer |
| Verify SC0 local semantics | Codex | implementation complete | `pending` | shape、patch boundary、active params、prefix consistency、strict reload、dual checkpoint smoke |
