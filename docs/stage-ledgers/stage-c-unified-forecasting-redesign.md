...
| Field | Content |
| --- | --- |
| `current_11_step` | Step 4-6 completed；Step 7 local implementation/invariant gate next |
| `current_candidate` | `SC1-PMFO-RCT`、`SC2-MIPR`（narrative_ready；effectiveness pending） |
| `latest_decision` | external prior-art与numeric theory gate完成；SC1收紧为refinement-conservative future tree，SC2收紧为measure-induced block risk |
| `next_required_action` | 实现PMFO-RCT local module、shape/refinement/prefix/locality tests与matched control parameter audit |
...
| --- | --- |
| `current_step` | Step 4-6 complete；Step 7 next |
...
| `design` | local invariants -> SC1 matched controls -> SC2 raw/random-projector controls -> 2x2 -> full matrix |
...
| `artifacts` | D1-v2、Step4-6 prior-art/theory report、Step7 protocol、baseline report |
| `decision` | 两项成为`narrative_ready` candidates；只授权SC1 local implementation，不授权remote training |
...
| `SC1-PMFO-RCT` | `narrative_ready` | conservative future-tree refinement提供dense basis之外的scale identity与domain-local execution | pass；medium novelty confidence，禁止generic wavelet/arbitrary-H claim | D1 problem pass；algebra pass；performance未测 | Step7 local module/invariants，随后matched controls |
...
| PMFO-RCT Step7A local implementation | `pending` | 按`docs/experiments/stage-c-pmfo-rct-step7-protocol.md`实现；不训练 |
...
4. narrative gate已完成；Step7A通过前不得启动remote training或实现MIPR；
