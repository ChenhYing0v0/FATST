...
| `stage_rollback` | Step7A 81/81、Step7B 19/19 pass；mild diagnostics continue to frozen test；CTD paused |
...
| `ISCF-v1-CPSI` | `step7b_prelaunch_pass_step8_authorized` | common scope state应在native synthesis前非线性调制private deviation | test-first design pass；generic set/multiscale claim forbidden | implementation 81/81；prelaunch 19/19；no performance result | commit-pinned Step8 remote；25/25后single test |
