...
| `active_question` | Section 6 v0.2 author-refined discussion-and-limitations draft under author review；Section 5 v0.13 and Figure 5 v5.3 remain temporarily fixed usable；Section 7 prose pending |
...
| Field | Content |
| --- | --- |
| `current_11_step` | paper experiment matrix closed；Section 5 v0.13 author-review candidate；Section 6 v0.2 discussion-and-limitations draft under author review；Figure 5 v5.3 fixed |
| `current_candidate` | Sections 6--7 paper writing |
| `latest_decision` | `section6_v0p2_author_refinement_author_review_candidate` |
| `writing_latest_decision` | `section6_v0p2_author_refinement_author_review_candidate` |
...
Section 6 v0.2 writing amendment（2026-08-24）：P1改用Sections 5.2/5.3的直接指代并删除CHPC structural-contract句；P2改为“从输出端出发”的ISCF表述，统一使用`forecast generation`并将ISCF置于互补关系句后半部分；P3删除Scope Probabilities的uncertainty limitation，将区域限制解释为fixed-interval partitioning，将跨region限制解释为没有专门的region-to-region information passing mechanism。Section 7尚未起草。Decision=`section6_v0p2_author_refinement_author_review_candidate`。
