...
ARMS = ["dense_row_initialized_prefix_decoder_multiprefix", "nested_segment_decoder_multiprefix"]
...
def run_dir(root: Path, checkpoint_policy: str, arm: str, dataset: str, seed: int) -> Path:
    run_name = f"TimeAlignOfficialUnified720_A2_{arm}_{checkpoint_policy}"
...
def load_reference(path: Path, value_key: str = "mse", arm: str | None = None, loss_mode: str | None = None) -> dict[tuple[str, int], dict[str, float]]:
    ...
    for row in read_csv(path):
        ...
        ref[(row["dataset"], int(row["target_horizon"]))] = {
            "mse": float(row[value_key]),
            "mae": float(row["mae"] if value_key == "mse" else row.get("fixed_mae", row["mae"])),
        }
    return ref
...
def summarize(compare_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for dataset in DATASETS + ["ALL"]:
        ...
        for arm in ARMS:
            ...
            for name in ["h0_full", "h0b_stochastic_k2", "h1_target_set", "h1c_row_gated", "fixed"]:
...
def write_report(path: Path, summary_rows: list[dict[str, Any]], compare_rows: list[dict[str, Any]]) -> None:
    ...
    for row in summary_rows:
        lines.append(
            "| {dataset} | {arm} | {settings} | {h0} | {h1} | {wins_h1c} | {h1c} | {wins_fixed} | {fixed} |".format(
                dataset=row["dataset"],
                arm=row["arm"],
    ...
    for row in compare_rows:
        lines.append(
            "| {dataset} | {horizon} | {arm} | {mse:.6f} | {h1} | {h1c} | {fixed} |".format(
                dataset=row["dataset"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze Phase5 TimeAlign-HSS A2 interface gate.")
    parser.add_argument("--root", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument("--checkpoint-policy", choices=["official-last", "best-val"], default="official-last")
    parser.add_argument("--seed", type=int, default=2021)
    parser.add_argument("--h0-metrics-csv", type=Path, default=Path("analysis/phase5_timealign_hss_h0_prefix_gate_20260630/phase5_timealign_hss_h0_metrics.csv"))
    parser.add_argument("--h0b-metrics-csv", type=Path, default=Path("analysis/phase5_timealign_hss_h0b_schedule_gate_20260630/phase5_timealign_hss_h0b_metrics.csv"))
    ...
    references = {
        "h0_full": load_reference(args.h0_metrics_csv, loss_mode="full"),
        "h0b_stochastic_k2": load_reference(args.h0b_metrics_csv, arm="stochastic_prefix_k2"),
    ...
    write_csv(args.output_dir / "phase5_timealign_hss_a2_metrics.csv", metric_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_a2_comparison.csv", compare_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_a2_summary.csv", summary_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_a2_training.csv", training_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_a2_best_epoch.csv", best_rows)
    (args.output_dir / "phase5_timealign_hss_a2_summary.json").write_text(
        json.dumps({"metric_rows": len(metric_rows), "summary_rows": summary_rows}, indent=2, sort_keys=True) + "\n"
    )
    write_report(args.output_dir / "phase5_timealign_hss_a2_interface_gate_report.md", summary_rows, compare_rows)
    print(f"phase5_timealign_hss_a2_report={args.output_dir / 'phase5_timealign_hss_a2_interface_gate_report.md'}")
