...
ARMS = ["dense_initialized_nested_segment_decoder_multiprefix"]


def run_dir(root: Path, checkpoint_policy: str, arm: str, dataset: str, seed: int) -> Path:
    run_name = f"TimeAlignOfficialUnified720_A3_{arm}_{checkpoint_policy}"
...
def load_a2_nested(path: Path) -> dict[tuple[str, int], dict[str, float]]:
    return load_reference(path, arm="nested_segment_decoder_multiprefix")
...
def summarize(compare_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for dataset in DATASETS + ["ALL"]:
        ...
        for arm in ARMS:
            ...
            for name in ["a2_nested", "h1_target_set", "h1c_row_gated", "fixed"]:
...
def write_report(path: Path, summary_rows: list[dict[str, Any]], compare_rows: list[dict[str, Any]]) -> None:
    ...
    for row in summary_rows:
        lines.append(
            "| {dataset} | {arm} | {settings} | {wins_a2} | {a2} | {wins_h1} | {h1} | {wins_h1c} | {h1c} | {wins_fixed} | {fixed} |".format(
                dataset=row["dataset"],
                arm=row["arm"],
                settings=row["settings"],
                wins_a2=row.get("wins_vs_a2_nested", ""),
                a2=fmt_pct(row.get("mean_relative_mse_vs_a2_nested_pct", float("nan"))),
                wins_h1=row.get("wins_vs_h1_target_set", ""),
    ...
    for row in compare_rows:
        lines.append(
            "| {dataset} | {horizon} | {arm} | {mse:.6f} | {a2} | {h1} | {h1c} | {fixed} |".format(
                dataset=row["dataset"],
...
def add_capacity_repair_rows(summary_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for row in summary_rows:
        out = dict(row)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze Phase5 TimeAlign-HSS A3 interface repair.")
    ...
    references = {
        "a2_nested": load_a2_nested(args.a2_metrics_csv),
    ...
    decision_rows = add_capacity_repair_rows(summary_rows)
    best_rows = best_epoch_rows(training_rows)

    args.output_dir.mkdir(parents=True, exist_ok=True)
    write_csv(args.output_dir / "phase5_timealign_hss_a3_metrics.csv", metric_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_a3_comparison.csv", compare_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_a3_summary.csv", summary_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_a3_decision.csv", decision_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_a3_training.csv", training_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_a3_best_epoch.csv", best_rows)
    (args.output_dir / "phase5_timealign_hss_a3_summary.json").write_text(
        json.dumps({"metric_rows": len(metric_rows), "summary_rows": summary_rows, "decision_rows": decision_rows}, indent=2, sort_keys=True) + "\n"
    )
    write_report(args.output_dir / "phase5_timealign_hss_a3_interface_repair_report.md", summary_rows, compare_rows)
    print(f"phase5_timealign_hss_a3_report={args.output_dir / 'phase5_timealign_hss_a3_interface_repair_report.md'}")
