...
ARMS = ["target_conditioned_nested_residual_decoder_multiprefix"]


def run_dir(root: Path, checkpoint_policy: str, arm: str, dataset: str, seed: int) -> Path:
    run_name = f"TimeAlignOfficialUnified720_A3B_{arm}_{checkpoint_policy}"
...
def summarize(compare_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for dataset in DATASETS + ["ALL"]:
        ...
        for arm in ARMS:
            ...
            for name in ["a2_nested", "a3_shallow", "h1_target_set", "h1c_row_gated", "fixed"]:
...
def decision_rows(summary_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for row in summary_rows:
        out = dict(row)
        a2 = row.get("mean_relative_mse_vs_a2_nested_pct")
        a3 = row.get("mean_relative_mse_vs_a3_shallow_pct")
        h1 = row.get("mean_relative_mse_vs_h1_target_set_pct")
        h1c = row.get("mean_relative_mse_vs_h1c_row_gated_pct")
        fixed = row.get("mean_relative_mse_vs_fixed_pct")
        out["improves_a2_nested"] = a2 is not None and a2 < 0
        out["improves_a3_shallow"] = a3 is not None and a3 < 0
        out["passes_h1_gate"] = h1 is not None and h1 < 0
        out["passes_h1c_gate"] = h1c is not None and h1c < 0
        out["beats_fixed"] = fixed is not None and fixed < 0
        rows.append(out)
    return rows
...
def write_report(path: Path, summary_rows: list[dict[str, Any]], compare_rows: list[dict[str, Any]]) -> None:
    ...
    for row in summary_rows:
        lines.append(
            "| {dataset} | {arm} | {settings} | {a2} | {a3} | {h1} | {wins_h1c} | {h1c} | {wins_fixed} | {fixed} |".format(
                dataset=row["dataset"],
                arm=row["arm"],
                settings=row["settings"],
                a2=fmt_pct(row.get("mean_relative_mse_vs_a2_nested_pct", float("nan"))),
                a3=fmt_pct(row.get("mean_relative_mse_vs_a3_shallow_pct", float("nan"))),
    ...
    for row in compare_rows:
        lines.append(
            "| {dataset} | {horizon} | {arm} | {mse:.6f} | {a2} | {a3} | {h1} | {h1c} | {fixed} |".format(
                dataset=row["dataset"],
                horizon=row["target_horizon"],
                arm=row["arm"],
                mse=row["mse"],
                a2=fmt_pct(row.get("relative_mse_vs_a2_nested_pct", float("nan"))),
                a3=fmt_pct(row.get("relative_mse_vs_a3_shallow_pct", float("nan"))),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze Phase5 TimeAlign-HSS A3B nested residual gate.")
    ...
    parser.add_argument("--a3-metrics-csv", type=Path, default=Path("analysis/phase5_timealign_hss_a3_interface_repair_20260701/phase5_timealign_hss_a3_metrics.csv"))
    ...
    references = {
        ...
        "a3_shallow": load_reference(args.a3_metrics_csv, arm="dense_initialized_nested_segment_decoder_multiprefix"),
    ...
    decision = decision_rows(summary_rows)

    args.output_dir.mkdir(parents=True, exist_ok=True)
    write_csv(args.output_dir / "phase5_timealign_hss_a3b_metrics.csv", metric_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_a3b_comparison.csv", compare_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_a3b_summary.csv", summary_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_a3b_decision.csv", decision)
    write_csv(args.output_dir / "phase5_timealign_hss_a3b_training.csv", training_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_a3b_best_epoch.csv", best_rows)
    (args.output_dir / "phase5_timealign_hss_a3b_summary.json").write_text(
        json.dumps({"metric_rows": len(metric_rows), "summary_rows": summary_rows, "decision_rows": decision}, indent=2, sort_keys=True) + "\n"
    )
    write_report(args.output_dir / "phase5_timealign_hss_a3b_nested_residual_gate_report.md", summary_rows, compare_rows)
    print(f"phase5_timealign_hss_a3b_report={args.output_dir / 'phase5_timealign_hss_a3b_nested_residual_gate_report.md'}")
