...
def write_report(output_dir: Path, comparison: pd.DataFrame, summary: pd.DataFrame, training: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        (
            ...
            "它证明 learned-basis dense-capacity path 有效，但还需要 official-last training-trajectory 与 "
            "learned-basis structure 诊断来判断 ETTh2 与 long horizon 的剩余差距；`best-val` 只能作为 "
            "diagnostic-only upper-bound audit，不能作为主协议。"
        ),
        "",
        "## Arm Summary",
        "",
    ...
    lines.extend(
        [
            ...
            (
                ...
                "随后 last-val 变差；这应被记录为 official-last trajectory drift，而不是改变 main protocol 的理由。"
                "在未完成 trajectory/objective 诊断前，不能把 A6-LBF 升级为 paper-core pass。"
            ),
            "",
            "## Decision",
            "",
