...
ARMS = ["prefix_conditioned_stochastic_k2", "target_set_decoder_multiprefix"]
...
def run_dir(root: Path, checkpoint_policy: str, arm: str, dataset: str, seed: int) -> Path:
    run_name = f"TimeAlignOfficialUnified720_H1_{arm}_{checkpoint_policy}"
    return root / checkpoint_policy / run_name / dataset / "mixed_h96_h192_h336_h720" / f"seed{seed}"


def collect_h1_metrics(root: Path, checkpoint_policy: str, seed: int) -> list[dict[str, Any]]:
...
def collect_h1_training(root: Path, checkpoint_policy: str, seed: int) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for dataset in DATASETS:
        for arm in ARMS:
...
def load_h0_reference(path: Path) -> dict[tuple[str, str, int], dict[str, float]]:
    ref: dict[tuple[str, str, int], dict[str, float]] = {}
    for row in read_csv(path):
        mode = row["loss_mode"]
        if mode not in {"full", "multi-prefix"}:
            continue
        ref[(row["dataset"], mode, int(row["target_horizon"]))] = {
            "mse": float(row["mse"]),
            "mae": float(row["mae"]),
        }
    return ref
...
def load_h0b_reference(path: Path) -> dict[tuple[str, int], dict[str, float]]:
...
def compare(
    ...
    h0b_ref: dict[tuple[str, int], dict[str, float]],
    ...
    for row in metric_rows:
        ...
        h0b = h0b_ref.get((dataset, horizon))
        if h0b:
            out["relative_mse_vs_h0b_stochastic_k2_pct"] = relative_pct(row["mse"], h0b["mse"])
            out["beats_h0b_stochastic_k2"] = row["mse"] < h0b["mse"]
        fixed = fixed_ref.get((dataset, horizon))
        if fixed:
            out["fixed_mse"] = fixed["mse"]
            out["relative_mse_vs_fixed_pct"] = relative_pct(row["mse"], fixed["mse"])
            out["beats_fixed"] = row["mse"] < fixed["mse"]
        rows.append(out)
    return rows
...
def summarize(compare_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for dataset in DATASETS + ["ALL"]:
        ...
        for arm in ARMS:
            ...
            for name in ["full", "multi_prefix", "h0b_stochastic_k2", "fixed"]:
...
def write_report(path: Path, summary_rows: list[dict[str, Any]], compare_rows: list[dict[str, Any]]) -> None:
    ...
    for row in summary_rows:
        lines.append(
            "| {dataset} | {arm} | {settings} | {full} | {multi} | {h0b} | {wins_fixed} | {fixed} |".format(
                dataset=row["dataset"],
                arm=row["arm"],
    ...
    for row in compare_rows:
        lines.append(
            "| {dataset} | {horizon} | {arm} | {mse:.6f} | {full} | {multi} | {h0b} | {fixed} |".format(
                dataset=row["dataset"],
                horizon=row["target_horizon"],
                arm=row["arm"],
                mse=row["mse"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze Phase5 TimeAlign-HSS H1 readout gate.")
    ...
    metric_rows = collect_h1_metrics(args.root, args.checkpoint_policy, args.seed)
    training_rows = collect_h1_training(args.root, args.checkpoint_policy, args.seed)
    compare_rows = compare(
        metric_rows,
        load_h0_reference(args.h0_metrics_csv),
        load_h0b_reference(args.h0b_metrics_csv),
        load_fixed_reference(args.fixed_reference_csv),
    )
    summary_rows = summarize(compare_rows)
    best_rows = best_epoch_rows(training_rows)

    args.output_dir.mkdir(parents=True, exist_ok=True)
    write_csv(args.output_dir / "phase5_timealign_hss_h1_metrics.csv", metric_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_h1_comparison.csv", compare_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_h1_summary.csv", summary_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_h1_training.csv", training_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_h1_best_epoch.csv", best_rows)
    (args.output_dir / "phase5_timealign_hss_h1_summary.json").write_text(
        json.dumps(
            {
    ...
    write_report(args.output_dir / "phase5_timealign_hss_h1_readout_gate_report.md", summary_rows, compare_rows)
    print(f"phase5_timealign_hss_h1_report={args.output_dir / 'phase5_timealign_hss_h1_readout_gate_report.md'}")
