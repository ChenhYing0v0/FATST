...
ARMS = ["target_set_prefix_head_multiprefix", "prefix_token_decoder_multiprefix"]
...
def run_dir(root: Path, checkpoint_policy: str, arm: str, dataset: str, seed: int) -> Path:
    run_name = f"TimeAlignOfficialUnified720_H1B_{arm}_{checkpoint_policy}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze Phase5 TimeAlign-HSS H1B variable readout gate.")
    ...
    write_csv(args.output_dir / "phase5_timealign_hss_h1b_metrics.csv", metric_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_h1b_comparison.csv", compare_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_h1b_summary.csv", summary_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_h1b_training.csv", training_rows)
    write_csv(args.output_dir / "phase5_timealign_hss_h1b_best_epoch.csv", best_rows)
    (args.output_dir / "phase5_timealign_hss_h1b_summary.json").write_text(
        json.dumps(
            {
    ...
    write_report(args.output_dir / "phase5_timealign_hss_h1b_variable_readout_gate_report.md", summary_rows, compare_rows)
    print(f"phase5_timealign_hss_h1b_report={args.output_dir / 'phase5_timealign_hss_h1b_variable_readout_gate_report.md'}")
