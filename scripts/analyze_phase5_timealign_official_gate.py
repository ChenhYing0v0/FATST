...
def fixed_dir(root: Path, checkpoint_policy: str, dataset: str, horizon: int) -> Path:
    return (
        root
        / f"TimeAlignOfficialFixedH{horizon}_{checkpoint_policy}"
        / dataset
        / FIXED_LABEL[horizon]
        / f"seed{SEED}"
    )
...
def unified_dir(root: Path, checkpoint_policy: str, dataset: str) -> Path:
    return (
        root
        / f"TimeAlignOfficialUnified720_{checkpoint_policy}"
        / dataset
        / UNIFIED_LABEL
        / f"seed{SEED}"
    )
...
def load_main(
    ...
    for dataset in DATASETS:
        unified_path = unified_dir(root, checkpoint_policy, dataset) / "metrics_by_target_horizon.csv"
        for horizon in HORIZONS:
            fixed = load_metric_row(
                fixed_dir(root, checkpoint_policy, dataset, horizon) / "metrics_by_target_horizon.csv",
                horizon,
            )
            ...
            if fixed is not None:
                fixed_rows.append(
                    {
                        "model": f"TimeAlignOfficialFixedH{horizon}_{checkpoint_policy}",
                        "dataset": dataset,
            ...
            if unified is not None:
                unified_rows.append(
                    {
                        "model": f"TimeAlignOfficialUnified720_{checkpoint_policy}",
                        "dataset": dataset,
                        "target_horizon": horizon,
                        **unified,
                    }
...
def load_training_summary(root: Path, checkpoint_policy: str) -> list[dict[str, Any]]:
    rows = []
    runs: list[tuple[str, str, Path]] = []
    for dataset in DATASETS:
        for horizon in HORIZONS:
            run_name = f"TimeAlignOfficialFixedH{horizon}_{checkpoint_policy}"
            runs.append((run_name, dataset, fixed_dir(root, checkpoint_policy, dataset, horizon)))
        runs.append(
            (
                f"TimeAlignOfficialUnified720_{checkpoint_policy}",
                dataset,
                unified_dir(root, checkpoint_policy, dataset),
            )
        )
        ...
        rows.append(
            {
                ...
                "last_minus_best_val_mse_pct": pct(
                    float(logs[-1]["val_mean_mse"]),
...
def write_report(
    ...
    lines = [
        ...
        "",
        f"[Decision] `{decision}`.",
        f"[Fact] checkpoint_policy = `{checkpoint_policy}`.",
        "",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze Phase5 official TimeAlign artifacts.")
    parser.add_argument("--raw-root", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument("--checkpoint-policy", choices=["official-last", "best-val"], required=True)
...
def main() -> None:
    ...
    fixed_rows, unified_rows, gap_rows = load_main(args.raw_root, args.checkpoint_policy)
    summary_rows = summarize_gap(gap_rows)
    training_rows = load_training_summary(args.raw_root, args.checkpoint_policy)
    write_csv(args.output_dir / "phase5_timealign_official_fixed_metrics.csv", fixed_rows)
    write_csv(args.output_dir / "phase5_timealign_official_unified_metrics.csv", unified_rows)
    write_csv(args.output_dir / "phase5_timealign_official_unified_gap.csv", gap_rows)
    write_csv(args.output_dir / "phase5_timealign_official_unified_gap_summary.csv", summary_rows)
    write_csv(args.output_dir / "phase5_timealign_official_training_summary.csv", training_rows)
    write_report(args.output_dir, args.checkpoint_policy, fixed_rows, gap_rows, summary_rows, training_rows)
