#!/usr/bin/env python3
"""Analyze the ISCF-RSCC Step8 matched validation matrix."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/stage_c_iscf_rscc_step7b.json"),
    )
...
def arm_ids(config: dict[str, Any]) -> tuple[tuple[str, ...], tuple[str, ...]]:
    ...
    new_arms = tuple(
        str(arm["id"]) for arm in config["arms"] if arm["train"]
    )
    return all_arms, new_arms
...
def run_dir(
    ...
    root = parent_root if arm == PARENT_ARM else new_root / arm
    return root / dataset / "h720_full" / "seed2021"
...
def probe_dir(
    ...
    return run_dir(arm, dataset, new_root, new_root)
    ...
    for arm in all_arms:
        for dataset in config["datasets"]:
            ...
            diagnostic_directory = probe_dir(
                arm, dataset, new_root, parent_probe_root
            )
            missing = [
                name
            ...
            effective = json.loads(
                (directory / "effective_config.json").read_text(
                    encoding="utf-8"
                )
            )
            ...
            rows.append(
                {
                    ...
                    "objective_expected": objectives[arm],
                    "objective_observed": observed_objective,
                    "objective_match": observed_objective == objectives[arm],
                    ...
                    "uses_test_split": invariant.get(
                        "uses_test_split", False
                    ),
                    "invariant_pass": invariant.get(
                        "pass", arm == PARENT_ARM
                    ),
                }
            )
...
def comparison_rows(
    ...
    for comparison in config["comparisons"]:
        ...
        for dataset in config["datasets"]:
            for horizon in HORIZONS:
                candidate = indexed[
                    (comparison["candidate"], dataset, horizon)
                ]
                reference = indexed[
                    (comparison["reference"], dataset, horizon)
                ]
                row = {
        ...
        summaries.append(
            {
                ...
                "macro_mae_gain_percent": float(
                    np.mean(
                        [
                            row["mae_gain_percent"]
                            for row in comparison_cells
                        ]
                    )
                ),
                "cell_wins": sum(
                    row["mse_gain_percent"] > 0.0
                    for row in comparison_cells
                ),
...
def internal_health(
    ...
    for arm in all_arms:
        for dataset in config["datasets"]:
            ...
            delta = (
                np.abs(leave_one_out - target[:, None, :])
                - full_error[:, None, :]
            )
            ...
            rows.append(
                {
                    ...
                    "policy_entropy_median": float(
                        np.median(policy_entropy)
                    ),
                    "minimum_scope_usage": float(usage.min()),
                    "nonzero_usage_scope_count": int(
                        np.sum(usage > 1e-6)
                    ),
            ...
            rows.append(
                {
                    ...
                    "nonzero_scope_gradient_count": sum(
                        min(
                            float(
                                row[
                                    f"train_pcc_scope_s{scope}_mode_grad_norm"
                                ]
                            )
                            for row in log_rows
                        )
...
def median_internal(
    ...
    return float(
        np.median([row[field] for row in rows if row["arm"] == arm])
    )
...
def decision_summary(
    ...
    health = config["internal_health"]
    primary = comparisons[gates["primary_comparison"]]
    control_names = tuple(
        comparison["id"]
        for comparison in config["comparisons"]
        if comparison["id"] != gates["primary_comparison"]
    )
    controls = [comparisons[name] for name in control_names]
    expected_runs = config["matrix"]["effective_runs"]
    expected_cells = config["matrix"]["effective_validation_cells"]
    rscc_headroom = median_internal(
        internal_rows,
        "iscf_rscc",
        "coalition_oracle_headroom_l1_percent",
    )
    equal_headroom = median_internal(
        internal_rows,
        PARENT_ARM,
        "coalition_oracle_headroom_l1_percent",
    )
    rscc_alignment = median_internal(
        internal_rows,
        "iscf_rscc",
        "policy_credit_spearman_median",
    )
    equal_alignment = median_internal(
        internal_rows,
        PARENT_ARM,
        "policy_credit_spearman_median",
    )
    checks = {
        "artifact_matrix_complete": len(run_rows) == expected_runs
        and all(row["missing_artifact_count"] == 0 for row in run_rows),
        "validation_matrix_complete": len(metric_rows) == expected_cells,
        ...
        "numeric_and_gradient_health": all(
            row["nonzero_scope_gradient_count"]
            >= health["minimum_nonzero_gradient_scopes"]
        ...
        "policy_usage_health": all(
            row["nonzero_usage_scope_count"]
            >= health["minimum_nonzero_policy_usage_scopes"]
            for row in internal_rows
            if row["arm"] == "iscf_rscc"
        ),
        "coalition_headroom_positive": rscc_headroom > 0.0,
        "policy_alignment_improves_vs_equal": (
            rscc_alignment > equal_alignment
        ),
    }
    internal_pass = all(
        checks[name]
        for name in (
            "numeric_and_gradient_health",
            "policy_usage_health",
            "coalition_headroom_positive",
            "policy_alignment_improves_vs_equal",
        )
    )
    if checks["effectiveness_vs_equal"] and checks["matched_controls"]:
        if internal_pass:
            decision = "rscc_v1_validation_pass_request_formal_test_design"
            failure = "none"
        else:
            decision = "rscc_v1_internal_mechanism_fail_close_exact_route"
            failure = "intervention_point_wrong"
    elif checks["effectiveness_vs_equal"]:
        decision = "rscc_v1_control_attribution_fail_close_exact_route"
        failure = "capacity_control_explains"
    else:
        decision = "rscc_v1_effectiveness_fail_close_exact_route"
        failure = "hypothesis_false"
    return {
        "candidate_version": config["candidate_version"],
        "runs": len(run_rows),
        "validation_cells": len(metric_rows),
        "checks": checks,
        "primary_macro_mse_gain_percent": primary[
            "macro_mse_gain_percent"
        ],
        "primary_macro_mae_gain_percent": primary[
            "macro_mae_gain_percent"
        ],
        "primary_cell_wins": primary["cell_wins"],
        "primary_dataset_wins": primary["dataset_wins"],
        "primary_horizon_wins": primary["horizon_wins"],
        "matched_control_macro_mse_gains_percent": {
            row["comparison"]: row["macro_mse_gain_percent"]
            for row in controls
        },
        "equal_median_coalition_oracle_headroom_l1_percent": equal_headroom,
        "rscc_median_coalition_oracle_headroom_l1_percent": rscc_headroom,
        "equal_median_policy_credit_spearman": equal_alignment,
        "rscc_median_policy_credit_spearman": rscc_alignment,
...
def synthetic_smoke(config: dict[str, Any]) -> None:
    ...
    if (
        len(metrics) != config["matrix"]["effective_validation_cells"]
        or len(cells) != len(config["comparisons"]) * 20
        or len(summaries) != len(config["comparisons"])
    ):
        raise RuntimeError("synthetic RSCC Step9 smoke failed")
    print("iscf_rscc_step9_synthetic_smoke=pass")
...
def main() -> None:
    ...
    internal_rows = internal_health(
        config, args.new_root, args.parent_probe_root
    )
    ...
    print(
        f"iscf_rscc_step9=complete runs={decision['runs']} "
