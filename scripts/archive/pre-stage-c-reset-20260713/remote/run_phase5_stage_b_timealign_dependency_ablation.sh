...
echo "phase5_stage_b_timealign_dependency_ablation_start=$(date -Is)"
echo "cwd=$(pwd)"
...
run_one() {
  ...
  local w_align="$4"
  local gpu="$5"
  local run_name="TimeAlignOfficialUnified720_A6LBF_r256_${arm}_${CHECKPOINT_POLICY}"
  ...
  mkdir -p "${output_dir}"
  echo "run_start=$(date -Is) dataset=${dataset} arm=${arm} gpu=${gpu} w_recon=${w_recon} w_align=${w_align}"
  CUDA_VISIBLE_DEVICES="${gpu}" "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python baselines/timealign_official/train_repo.py \
      ...
      --w-recon "${w_recon}" \
      --w-align "${w_align}" 2>&1 | tee "${run_log}"
  echo "run_done=$(date -Is) dataset=${dataset} arm=${arm}"
}
...
for arm_spec in "${arms[@]}"; do
  IFS=":" read -r arm w_recon w_align <<< "${arm_spec}"
  for dataset in "${DATASETS[@]}"; do
    gpu="${GPU_IDS[$((idx % gpu_count))]}"
    run_one "${dataset}" "${arm}" "${w_recon}" "${w_align}" "${gpu}" &
    pids+=("$!")
    idx=$((idx + 1))
...
echo "phase5_stage_b_timealign_dependency_ablation_done=$(date -Is)"
