...
REMOTE_OUTPUT_ROOT="${REMOTE_OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase2_future_state_alignment_repair}"
RUN_NAME="${RUN_NAME:-PatchEncoderFutureStateAlignmentConfWeighted}"
ANALYSIS_ROOT="${ANALYSIS_ROOT:-analysis/phase2_future_state_alignment_repair_gate_20260623}"
OUTPUT_PREFIX="${OUTPUT_PREFIX:-phase2_future_state_alignment_repair}"
LOCAL_CONDA_ENV="${LOCAL_CONDA_ENV:-r2026-fsa}"
SKIP_ANALYSIS="${SKIP_ANALYSIS:-0}"

echo "sync_phase2_repair_start=$(date '+%Y-%m-%dT%H:%M:%S%z')"
...
conda run -n "${LOCAL_CONDA_ENV}" python scripts/analyze_phase2_future_state_alignment_repair_gate.py \
  --analysis-root "${ANALYSIS_ROOT}" \
  --model-name "${RUN_NAME}" \
  --output-prefix "${OUTPUT_PREFIX}"

echo "decision_report=${ANALYSIS_ROOT}/${OUTPUT_PREFIX}_decision_report.md"
echo "sync_phase2_repair_done=$(date '+%Y-%m-%dT%H:%M:%S%z')"
