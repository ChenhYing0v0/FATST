...
REMOTE_OUTPUT_ROOT="${REMOTE_OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase2_region_balanced_objective}"
RUN_NAME="${RUN_NAME:-PatchEncoderRegionBalanced}"
ANALYSIS_ROOT="${ANALYSIS_ROOT:-analysis/phase2_region_balanced_gate_20260623}"
OUTPUT_PREFIX="${OUTPUT_PREFIX:-phase2_region_balanced}"
LOCAL_CONDA_ENV="${LOCAL_CONDA_ENV:-r2026-fsa}"
SKIP_ANALYSIS="${SKIP_ANALYSIS:-0}"

echo "sync_phase2_region_balanced_start=$(date '+%Y-%m-%dT%H:%M:%S%z')"
...
conda run -n "${LOCAL_CONDA_ENV}" python scripts/analyze_phase2_region_balanced_gate.py \
  --analysis-root "${ANALYSIS_ROOT}" \
  --model-name "${RUN_NAME}" \
  --output-prefix "${OUTPUT_PREFIX}"

echo "decision_report=${ANALYSIS_ROOT}/${OUTPUT_PREFIX}_decision_report.md"
echo "sync_phase2_region_balanced_done=$(date '+%Y-%m-%dT%H:%M:%S%z')"
