...
REMOTE_OUTPUT_ROOT="${REMOTE_OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase5_stage_b_b9_fsn_scf_small_gate}"
CHECKPOINT_POLICY="${CHECKPOINT_POLICY:-official-last}"
ANALYSIS_ROOT="${ANALYSIS_ROOT:-analysis/phase5_stage_b_b9_fsn_scf_small_gate_$(date '+%Y%m%d')}"
SKIP_ANALYSIS="${SKIP_ANALYSIS:-0}"

echo "sync_phase5_stage_b_b9_fsn_scf_small_gate_start=$(date '+%Y-%m-%dT%H:%M:%S%z')"
...
if [[ "${SKIP_ANALYSIS}" == "1" ]]; then
  ...
  echo "sync_phase5_stage_b_b9_fsn_scf_small_gate_done=$(date '+%Y-%m-%dT%H:%M:%S%z')"
  exit 0
fi

python scripts/analyze_phase5_stage_b_b9_fsn_scf_small_gate.py \
