#!/usr/bin/env python3
"""Freeze the twenty-four H4K checkpoints before formal test access."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "iscf_bsca_main_v1_hpo/h4k/test_audit"
        ),
    )
...
def main() -> None:
    ...
    if len(ledger) != 24:
        raise ValueError(f"expected twenty-four H4K rows, found {len(ledger)}")
    if Counter(entry["dataset"] for entry in ledger) != Counter(
        EXPECTED_BY_DATASET
    ):
        raise ValueError("H4K dataset profile counts differ from the contract")
    ...
    if len({row["trial_id"] for row in rows}) != 24:
        raise ValueError("H4K trial IDs are not unique")
    if len({row["checkpoint_sha256_before_test"] for row in rows}) != 24:
        raise ValueError("H4K checkpoint hashes are not unique")
