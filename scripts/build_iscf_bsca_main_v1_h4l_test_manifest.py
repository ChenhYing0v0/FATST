#!/usr/bin/env python3
"""Freeze the forty-eight H4L checkpoints before formal-test access."""
...
EXPECTED_BY_DATASET = {"ETTm2": 24, "Weather": 24}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "iscf_bsca_main_v1_hpo/h4l/test_audit"
        ),
    )
...
def main() -> None:
    ...
    if len(ledger) != 48:
        raise ValueError(f"expected forty-eight H4L rows, found {len(ledger)}")
    if Counter(entry["dataset"] for entry in ledger) != Counter(
        EXPECTED_BY_DATASET
    ):
        raise ValueError("H4L dataset profile counts differ from the contract")
    ...
    if len({row["trial_id"] for row in rows}) != 48:
        raise ValueError("H4L trial IDs are not unique")
    if len({row["checkpoint_sha256_before_test"] for row in rows}) != 48:
        raise ValueError("H4L checkpoint hashes are not unique")
