#!/usr/bin/env python3
"""Freeze the forty Weather H4N checkpoints before formal-test access."""
...
EXPECTED_BY_DATASET = {"Weather": 40}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "iscf_bsca_main_v1_hpo/h4n/test_audit"
        ),
    )
...
def main() -> None:
    ...
    if len(ledger) != 40:
        raise ValueError(f"expected forty H4N rows, found {len(ledger)}")
    if Counter(entry["dataset"] for entry in ledger) != Counter(
        EXPECTED_BY_DATASET
    ):
        raise ValueError("H4N dataset profile counts differ from the contract")

    rows = []
    for entry in ledger:
        if entry["status"] != "validation_complete":
            raise ValueError(f"unexpected trial status: {entry['trial_id']}")
        if not entry["artifact_completeness_pass"] or not entry[
            "numeric_health_pass"
        ]:
    ...
    if len({row["trial_id"] for row in rows}) != 40:
        raise ValueError("H4N trial IDs are not unique")
    if len({row["checkpoint_sha256_before_test"] for row in rows}) != 40:
        raise ValueError("H4N checkpoint hashes are not unique")
