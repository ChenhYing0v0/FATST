#!/usr/bin/env python3
"""Freeze the forty-eight H5A checkpoints before official-test access."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "iscf_bsca_main_v1_hpo/h5a/test_audit"
        ),
    )
...
def main() -> None:
    ...
    if len(ledger) != 48:
        raise ValueError(f"expected 48 H5A rows, found {len(ledger)}")
    if Counter(entry["dataset"] for entry in ledger) != EXPECTED:
    ...
    if len({row["trial_id"] for row in rows}) != 48:
        raise ValueError("H5A trial IDs are not unique")
    if len({row["checkpoint_sha256_before_test"] for row in rows}) != 48:
        raise ValueError("H5A checkpoint hashes are not unique")
