#!/usr/bin/env python3
"""Audit all H4L train/validation artifacts before formal-test authorization."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs" / "iscf_bsca_main_v1_hpo_wide_h4l.json",
    )
...
def main() -> None:
    ...
    assert config["authorization"][
        "official_test_H4L_execution_authorized_after_complete_training"
    ] is False
    ...
    for job in jobs:
        ...
        mean_mse = sum(pair[0] for pair in metrics.values()) / len(HORIZONS)
