#!/usr/bin/env python3
"""Audit all H4M train/validation artifacts before formal-test access."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs" / "iscf_bsca_main_v1_hpo_targeted_h4m.json",
    )
...
def main() -> None:
    ...
    for job in jobs:
        ...
        assert math.isclose(
            mean_mse, float(best["val_mean_mse"]), rel_tol=0.0, abs_tol=1e-7
        )
        ...
        current = validation_best.get(job["dataset"])
        if current is None or mean_mse < current[1]:
            validation_best[job["dataset"]] = (trial_id, mean_mse)
    ...
    assert len(checkpoint_hashes) == 24
    assert not (args.output_root / "test_audit").exists()
    print(
        json.dumps(
            {
                "protocol_id": config["protocol_id"],
                "training_complete": "24/24",
                ...
                "validation_best_diagnostic_only": {
                    ...
                    for dataset, value in validation_best.items()
                },
