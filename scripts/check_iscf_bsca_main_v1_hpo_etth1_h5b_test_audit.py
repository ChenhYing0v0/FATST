#!/usr/bin/env python3
"""Check the frozen H5B formal-test contract before official-test access."""
...
CONFIG = ROOT / "configs" / "iscf_bsca_main_v1_hpo_etth1_h5b_test_audit.json"
MANIFEST = (
    ROOT
    / "analysis"
...
def main() -> None:
    ...
    assert config["matrix"]["expected_runs"] == 36
    assert config["matrix"]["expected_standard_horizon_cells"] == 144
    assert sha256(MANIFEST) == config["checkpoint_manifest"]["sha256"]
    assert len(rows) == 36
    assert len({row["trial_id"] for row in rows}) == 36
    assert len({row["checkpoint_sha256_before_test"] for row in rows}) == 36
    assert {row["dataset"] for row in rows} == {"ETTh1"}
    assert all(row["phase"] == "H5B" and row["seed"] == "2021" for row in rows)
    ...
    assert authorization["automatic_H5C_extension_authorized"] is False
    ...
    assert selection["eligibility_guard_mean_mse_relative_to_h5a_max"] == 1.003
    assert selection["eligibility_guard_mean_mae_relative_to_h5a_max"] == 1.003
    assert selection["minimum_Main_II_best_cells"] == 3
    ...
    result = subprocess.run(
        ["bash", "scripts/remote/run_iscf_bsca_main_v1_hpo_etth1_h5b_test_audit.sh"],
        cwd=ROOT,
    ...
    assert "jobs=36" in result and "test_cells=144" in result
    assert "authorized=true" in result
    print(
        json.dumps(
            {
                "candidate_version": config["candidate_version"],
                "training_checkpoints": 36,
                "standard_horizon_test_cells": 144,
