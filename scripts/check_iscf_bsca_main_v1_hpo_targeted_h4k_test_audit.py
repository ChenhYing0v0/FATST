...
CONFIG = (
    ROOT
    / "configs"
    / "iscf_bsca_main_v1_hpo_targeted_h4k_test_audit.json"
)
MANIFEST = (
    ROOT
    / "analysis"
    / "iscf_bsca_main_v1_hpo_20260731"
    / "h4k_checkpoint_manifest.csv"
)
...
def main() -> None:
    ...
    assert all(row["phase"] == "H4K" for row in rows)
    ...
    assert authorization["automatic_H4L_extension_authorized"] is False
    ...
    result = subprocess.run(
        [
            "bash",
            "scripts/remote/run_iscf_bsca_main_v1_hpo_targeted_h4k_test_audit.sh",
        ],
        cwd=ROOT,
