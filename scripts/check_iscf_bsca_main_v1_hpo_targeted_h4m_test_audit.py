#!/usr/bin/env python3
"""Check the authorized frozen H4M formal-test contract."""
...
CONFIG = ROOT / "configs" / "iscf_bsca_main_v1_hpo_targeted_h4m_test_audit.json"
MANIFEST = (
    ROOT
    / "analysis"
    / "iscf_bsca_main_v1_hpo_20260731"
    / "h4m_checkpoint_manifest.csv"
)
...
def main() -> None:
    ...
    assert len({row["checkpoint_sha256_before_test"] for row in rows}) == 24
    assert Counter(row["dataset"] for row in rows) == Counter(expected)
    assert all(row["phase"] == "H4M" for row in rows)
    assert all(row["seed"] == "2021" for row in rows)

    authorization = config["authorization"]
    assert authorization["user_authorized"] is True
    assert authorization["authorization_date"] == "2026-08-05"
    ...
    assert authorization["automatic_H4N_extension_authorized"] is False

    selection = config["hyperparameter_selection"]
    assert selection["all_189_trials_in_final_selector"] is True
    ...
    result = subprocess.run(
        [
            "bash",
            "scripts/remote/run_iscf_bsca_main_v1_hpo_targeted_h4m_test_audit.sh",
        ],
        cwd=ROOT,
    ...
    assert "jobs=24" in result and "test_cells=96" in result
    assert "authorized=true" in result
    print(
        json.dumps(
            {
                "candidate_version": config["candidate_version"],
                "training_checkpoints": 24,
