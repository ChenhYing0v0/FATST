#!/usr/bin/env python3
"""Check the authorized frozen H4L formal-test contract."""
...
CONFIG = ROOT / "configs" / "iscf_bsca_main_v1_hpo_wide_h4l_test_audit.json"
MANIFEST = (
    ROOT
    / "analysis"
    / "iscf_bsca_main_v1_hpo_20260731"
    / "h4l_checkpoint_manifest.csv"
)
...
def main() -> None:
    ...
    assert all(row["phase"] == "H4L" for row in rows)
    ...
    assert authorization["automatic_H4M_extension_authorized"] is False
    ...
    result = subprocess.run(
        [
            "bash",
            "scripts/remote/run_iscf_bsca_main_v1_hpo_wide_h4l_test_audit.sh",
        ],
        cwd=ROOT,
    ...
    assert "test_cells=192" in result
