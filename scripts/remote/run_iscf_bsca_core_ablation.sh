...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/iscf_bsca_core_ablation_20260814}"
DATASET_ROOT="${DATASET_ROOT:-/home/yingch/dataset}"
CONFIG="${CONFIG:-configs/iscf_bsca_core_ablation_protocol.json}"
...
PROTOCOL_PROFILE="${PROTOCOL_PROFILE:-iscf_bsca_core_ablation_20260814}"
...
EXPECTED_PROFILE_HASH="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["profiles"]["sha256"])' "${CONFIG}")"
...
for dataset,arm_id in c["launch_order"]:
 ...
 print("\t".join(map(str,(dataset,arm_id,arm["readout_mode"],arm["policy_mode"],arm["objective_mode"],arm["fixed_scale"],arm["partition"],arm["partition_seed"],rank,profile["patch_num"],profile["d_model"],profile["d_ff"]))))
' "${CONFIG}"
)
...
is_complete() {
  ...
  is_training_core_complete "$1" \
    ...
    && python3 -c 'import json,sys; assert json.load(open(sys.argv[1]))["pass"] is True' \
      "${d}/trained_invariants.json" 2>/dev/null
}
...
if [[ "${STATUS_ONLY}" == "1" ]]; then
  ...
  echo "core_ablation_status=$(date -Is) training=${trained}/${#LINES[@]} test=${tested}/${#LINES[@]}"
...
if [[ "${DRY_RUN}" == "1" ]]; then
  printf '%s\n' "${LINES[@]}"
  echo "core_ablation_dry_run=pass jobs=${#LINES[@]} config_hash=${CONFIG_HASH} profile_hash=${PROFILE_HASH} remote_authorized=${REMOTE_AUTHORIZED} test_authorized=${TEST_AUTHORIZED}"
  exit 0
fi
...
if [[ "${FORMAL_TEST_ONLY}" == "1" ]]; then
  ...
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/check_iscf_bsca_core_ablation_training_artifacts.py \
      --config "${CONFIG}" --output-root "${OUTPUT_ROOT}" \
      --manifest "${MANIFEST}" --verify-manifest
  read -r trained _tested <<< "$(status_counts)"
  [[ "${trained}" -eq "${#LINES[@]}" ]] || { echo "formal test blocked until 20/20 training: ${trained}/${#LINES[@]}" >&2; exit 4; }
fi

run_training_command() {
  local line="$1" gpu="$2" output_dir="$3" log="$4" smoke="$5"
  local dataset arm readout policy objective fixed_scale partition partition_seed rank patch_num d_model d_ff
  local final_args=()
  IFS=$'\t' read -r dataset arm readout policy objective fixed_scale partition partition_seed rank patch_num d_model d_ff <<< "${line}"
  if [[ "${smoke}" == "1" ]]; then
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python baselines/timealign_official/train_repo.py \
      ...
      --run-name "ISCF_BSCA_CORE_ABLATION_${arm}" --output-dir "${output_dir}" --device cuda \
      ...
      --pcsd-policy-hidden-dim 64 --pcsd-policy-mode "${policy}" --pcsd-fixed-scale "${fixed_scale}" \
...
if [[ "${RESOURCE_SMOKE}" == "1" ]]; then
  smoke_indices=(0 1 2 6)
  nvidia-smi --query-gpu=index,name,memory.total,memory.used,memory.free,utilization.gpu --format=csv,noheader,nounits
  for smoke_index in "${!smoke_indices[@]}"; do
    line="${LINES[${smoke_indices[${smoke_index}]}]}"
    gpu="${GPU_IDS[$((smoke_index % ${#GPU_IDS[@]}))]}"
    IFS=$'\t' read -r dataset arm _rest <<< "${line}"
    d="${OUTPUT_ROOT}/_resource_smoke/${arm}_${dataset}_seed${SEED}"
    mkdir -p "${d}"
    ...
    echo "core_ablation_resource_smoke_arm_done=$(date -Is) arm=${arm} output=${d}"
  done
  echo "core_ablation_resource_smoke=pass arms=${#smoke_indices[@]}"
  exit 0
fi
...
{
  echo "core_ablation_start=$(date -Is)"; echo "commit=$(git rev-parse HEAD)"; echo "output_root=${OUTPUT_ROOT}"
...
run_one() {
  ...
  if [[ "${FORMAL_TEST_ONLY}" == "1" ]]; then
    ...
    if is_training_core_complete "${line}"; then
      echo "validation_replay_start=$(date -Is) job=$((index + 1))/${#LINES[@]} arm=${arm} dataset=${dataset} gpu=${gpu}"
    else
...
[[ "${status}" == "0" ]]; echo "core_ablation_${MODE}_done=$(date -Is)"
