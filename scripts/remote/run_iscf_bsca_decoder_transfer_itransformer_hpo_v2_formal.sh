...
CONFIG="${CONFIG:-configs/iscf_bsca_decoder_transfer_itransformer_hpo_v2_formal.json}"
...
if len(rows) != 70:
    raise SystemExit(f"manifest row count mismatch: {len(rows)}/70")
if len({row["checkpoint_sha256"] for row in rows}) != 70:
    raise SystemExit("manifest checkpoint hashes are not unique")
for row in rows:
    print("\t".join((
        row["dataset"], row["profile_id"], row["checkpoint"],
...
verify_checkpoints() {
  local line dataset profile checkpoint expected actual count=0
  for line in "${ROWS[@]}"; do
    IFS=$'\t' read -r dataset profile checkpoint expected <<< "${line}"
    [[ -s "${checkpoint}" ]]
...
if [[ "${STATUS_ONLY}" == 1 ]]; then
  ...
  echo "itransformer_hpo_v2_formal_status=$(date -Is) formal_test=${complete}/70 cells=$((complete * 4))/280"
  exit 0
fi
...
if [[ "${DRY_RUN}" == 1 ]]; then
  verify_checkpoints
  printf '%s\n' "${ROWS[@]}"
  echo "itransformer_hpo_v2_formal_dry_run=pass jobs=70 cells=280"
  exit 0
fi
...
run_one() {
  ...
  local dataset profile checkpoint expected run_dir out log before after
  IFS=$'\t' read -r dataset profile checkpoint expected <<< "${line}"
  ...
  [[ "${before}" == "${expected}" ]]
  echo "formal_start=$(date -Is) job=$((index + 1))/70 dataset=${dataset} profile=${profile} gpu=${gpu}"
  ...
  [[ "${before}" == "${after}" ]]
  test_complete "${line}"
  echo "formal_done=$(date -Is) job=$((index + 1))/70 dataset=${dataset} profile=${profile} gpu=${gpu}"
}
...
[[ "${complete}" == 70 ]]
echo "itransformer_hpo_v2_formal_done=$(date -Is) jobs=70/70 cells=280/280"
