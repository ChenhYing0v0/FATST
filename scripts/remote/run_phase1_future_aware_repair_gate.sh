...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase1_future_aware_repair}"
LOG_ROOT="${LOG_ROOT:-${OUTPUT_ROOT}/_logs/phase1_future_aware_repair_gate}"
...
models=(
  ...
  "PatchEncoderFutureAwareAlignOnly:baselines/patch_encoder_future_aware_adapter/train.py:--run-name PatchEncoderFutureAwareAlignOnly --align-weight ${ALIGN_WEIGHT} --recon-weight 0.0 --recon-normalization none"
  "PatchEncoderFutureAwareScaleNorm:baselines/patch_encoder_future_aware_adapter/train.py:--run-name PatchEncoderFutureAwareScaleNorm --align-weight ${ALIGN_WEIGHT} --recon-weight ${RECON_WEIGHT} --recon-normalization target_energy"
)
datasets=("ETTh2" "ETTm1" "Weather")
horizons=("96" "192" "336" "720")
read -r -a gpu_ids <<< "${GPU_IDS}"

echo "phase1_future_aware_repair_gate_start=$(date -Is)"
echo "cwd=$(pwd)"
...
wait
echo "phase1_future_aware_repair_gate_done=$(date -Is)"
