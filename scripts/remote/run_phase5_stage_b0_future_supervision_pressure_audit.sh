...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase5_stage_b0_future_supervision_pressure_audit}"
...
ARMS="${ARMS:-b0_no_recon b0_no_align b0_no_future_pressure}"
SEQ_LEN="${SEQ_LEN:-720}"
...
arm_config() {
  ...
  case "${arm}" in
    b0_no_recon)
      echo "learned-basis-forecast-operator 256 0.0 preset"
      ;;
    b0_no_align)
      echo "learned-basis-forecast-operator 256 1.0 0.0"
      ;;
    b0_no_future_pressure)
      echo "learned-basis-forecast-operator 256 0.0 0.0"
      ;;
    *)
      echo "Unknown B0 arm: ${arm}" >&2
      exit 1
      ;;
  esac
}

echo "phase5_stage_b0_future_supervision_pressure_audit_start=$(date -Is)"
echo "cwd=$(pwd)"
...
run_job() {
  ...
  local arm="$2"
  local gpu="$3"
  local readout_mode basis_rank w_recon w_align_override
  read -r readout_mode basis_rank w_recon w_align_override <<< "$(arm_config "${arm}")"
  local run_name run_dir run_log
  run_name="TimeAlignOfficialUnified720_B0_${arm}_${CHECKPOINT_POLICY}"
  ...
  echo "run_start=$(date -Is) run_name=${run_name} dataset=${dataset} arm=${arm} readout_mode=${readout_mode} w_recon=${w_recon} w_align_override=${w_align_override} gpu=${gpu}"
  local extra_args=()
  if [[ "${w_align_override}" != "preset" ]]; then
    extra_args+=(--w-align-override "${w_align_override}")
  fi
...
echo "phase5_stage_b0_future_supervision_pressure_audit_done=$(date -Is)"
