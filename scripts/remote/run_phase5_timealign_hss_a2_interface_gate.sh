...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase5_timealign_hss_a2_interface_gate}"
...
ARMS="${ARMS:-dense_row_initialized_prefix_decoder_multiprefix nested_segment_decoder_multiprefix}"
SEQ_LEN="${SEQ_LEN:-720}"
...
arm_config() {
  ...
  case "${arm}" in
    dense_row_initialized_prefix_decoder_multiprefix)
      echo "dense-row-initialized-prefix-decoder multi-prefix 1 32 32"
      ;;
    nested_segment_decoder_multiprefix)
      echo "nested-segment-decoder multi-prefix 1 32 32"
      ;;
    *)
      echo "Unknown A2 arm: ${arm}" >&2
      exit 1
      ;;
  esac
}

echo "phase5_timealign_hss_a2_interface_gate_start=$(date -Is)"
echo "cwd=$(pwd)"
...
run_job() {
  ...
  run_name="TimeAlignOfficialUnified720_A2_${arm}_${CHECKPOINT_POLICY}"
...
echo "phase5_timealign_hss_a2_interface_gate_done=$(date -Is)"
