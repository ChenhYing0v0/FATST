...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase5_timealign_hss_a3_interface_repair}"
...
ARMS="${ARMS:-dense_initialized_nested_segment_decoder_multiprefix}"
SEQ_LEN="${SEQ_LEN:-720}"
...
arm_config() {
  ...
  case "${arm}" in
    dense_initialized_nested_segment_decoder_multiprefix)
      echo "dense-initialized-nested-segment-decoder multi-prefix 1 32 32"
      ;;
    *)
      echo "Unknown A3 arm: ${arm}" >&2
      exit 1
      ;;
  esac
}

echo "phase5_timealign_hss_a3_interface_repair_start=$(date -Is)"
echo "cwd=$(pwd)"
...
run_job() {
  ...
  run_name="TimeAlignOfficialUnified720_A3_${arm}_${CHECKPOINT_POLICY}"
...
echo "phase5_timealign_hss_a3_interface_repair_done=$(date -Is)"
