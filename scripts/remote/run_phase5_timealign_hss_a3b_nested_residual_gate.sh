...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase5_timealign_hss_a3b_nested_residual_gate}"
...
ARMS="${ARMS:-target_conditioned_nested_residual_decoder_multiprefix}"
SEQ_LEN="${SEQ_LEN:-720}"
...
arm_config() {
  ...
  case "${arm}" in
    target_conditioned_nested_residual_decoder_multiprefix)
      echo "target-conditioned-nested-residual-decoder multi-prefix 1 32 32"
      ;;
    *)
      echo "Unknown A3B arm: ${arm}" >&2
      exit 1
      ;;
  esac
}

echo "phase5_timealign_hss_a3b_nested_residual_gate_start=$(date -Is)"
echo "cwd=$(pwd)"
...
run_job() {
  ...
  run_name="TimeAlignOfficialUnified720_A3B_${arm}_${CHECKPOINT_POLICY}"
...
echo "phase5_timealign_hss_a3b_nested_residual_gate_done=$(date -Is)"
