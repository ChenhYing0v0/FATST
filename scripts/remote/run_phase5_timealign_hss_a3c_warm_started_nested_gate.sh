...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase5_timealign_hss_a3c_warm_started_nested_gate}"
...
ARMS="${ARMS:-checkpoint_initialized_nested_segment_decoder_multiprefix}"
SEQ_LEN="${SEQ_LEN:-720}"
...
arm_config() {
  ...
  case "${arm}" in
    checkpoint_initialized_nested_segment_decoder_multiprefix)
      echo "checkpoint-initialized-nested-segment-decoder multi-prefix 1 32 32"
      ;;
    *)
      echo "Unknown A3C arm: ${arm}" >&2
      exit 1
      ;;
  esac
}
...
echo "phase5_timealign_hss_a3c_warm_started_nested_gate_start=$(date -Is)"
echo "cwd=$(pwd)"
...
run_job() {
  ...
  run_name="TimeAlignOfficialUnified720_A3C_${arm}_${CHECKPOINT_POLICY}"
  ...
  echo "run_start=$(date -Is) run_name=${run_name} dataset=${dataset} arm=${arm} readout_mode=${readout_mode} loss_mode=${loss_mode} checkpoint=${checkpoint_path} gpu=${gpu}"
  CUDA_VISIBLE_DEVICES="${gpu}" PYTHONUNBUFFERED=1 "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python baselines/timealign_official/train_repo.py \
      ...
      --warm-start-checkpoint "${checkpoint_path}" 2>&1 | tee "${run_log}"
...
echo "phase5_timealign_hss_a3c_warm_started_nested_gate_done=$(date -Is)"
