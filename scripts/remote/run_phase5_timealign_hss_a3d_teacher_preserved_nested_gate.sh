...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase5_timealign_hss_a3d_teacher_preserved_nested_gate}"
...
ARMS="${ARMS:-teacher_preserved_nested_w03 teacher_preserved_nested_w10}"
SEQ_LEN="${SEQ_LEN:-720}"
...
arm_config() {
  ...
  case "${arm}" in
    teacher_preserved_nested_w03)
      echo "checkpoint-initialized-nested-segment-decoder multi-prefix 1 32 32 0.3"
      ;;
    teacher_preserved_nested_w10)
      echo "checkpoint-initialized-nested-segment-decoder multi-prefix 1 32 32 1.0"
      ;;
    *)
      echo "Unknown A3D arm: ${arm}" >&2
      exit 1
      ;;
  esac
}
...
echo "phase5_timealign_hss_a3d_teacher_preserved_nested_gate_start=$(date -Is)"
echo "cwd=$(pwd)"
...
run_job() {
  ...
  local arm="$2"
  local gpu="$3"
  local readout_mode loss_mode prefix_samples continuous_min_prefix continuous_prefix_step teacher_loss_weight
  read -r readout_mode loss_mode prefix_samples continuous_min_prefix continuous_prefix_step teacher_loss_weight <<< "$(arm_config "${arm}")"
  ...
  run_name="TimeAlignOfficialUnified720_A3D_${arm}_${CHECKPOINT_POLICY}"
  ...
  echo "run_start=$(date -Is) run_name=${run_name} dataset=${dataset} arm=${arm} readout_mode=${readout_mode} loss_mode=${loss_mode} teacher_loss_weight=${teacher_loss_weight} checkpoint=${checkpoint_path} gpu=${gpu}"
  CUDA_VISIBLE_DEVICES="${gpu}" PYTHONUNBUFFERED=1 "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python baselines/timealign_official/train_repo.py \
      ...
      --warm-start-checkpoint "${checkpoint_path}" \
      --teacher-checkpoint "${checkpoint_path}" \
      --teacher-readout-mode target-set-decoder \
      --teacher-loss-weight "${teacher_loss_weight}" 2>&1 | tee "${run_log}"
...
echo "phase5_timealign_hss_a3d_teacher_preserved_nested_gate_done=$(date -Is)"
