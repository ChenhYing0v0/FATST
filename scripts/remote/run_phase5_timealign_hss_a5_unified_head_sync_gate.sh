...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase5_timealign_hss_a5_unified_head_sync_gate}"
...
ARMS="${ARMS:-a5b_r64 a5b_r128 a5q_seg48_small a5q_seg24_wide}"
SEQ_LEN="${SEQ_LEN:-720}"
...
arm_config() {
  ...
  case "${arm}" in
    a5b_r64)
      echo "continuous-forecast-basis-operator multi-prefix 1 32 32 64 48 256"
      ;;
    a5b_r128)
      echo "continuous-forecast-basis-operator multi-prefix 1 32 32 128 48 256"
      ;;
    a5q_seg48_small)
      echo "elastic-causal-target-query-decoder multi-prefix 1 32 32 64 48 128"
      ;;
    a5q_seg24_wide)
      echo "elastic-causal-target-query-decoder multi-prefix 1 32 32 64 24 512"
      ;;
    *)
      echo "Unknown A5 arm: ${arm}" >&2
      exit 1
      ;;
  esac
}

echo "phase5_timealign_hss_a5_unified_head_sync_gate_start=$(date -Is)"
echo "cwd=$(pwd)"
...
run_job() {
  ...
  local arm="$2"
  local gpu="$3"
  local readout_mode loss_mode prefix_samples continuous_min_prefix continuous_prefix_step basis_rank target_query_segment_len target_query_ff
  read -r readout_mode loss_mode prefix_samples continuous_min_prefix continuous_prefix_step basis_rank target_query_segment_len target_query_ff <<< "$(arm_config "${arm}")"
  local run_name run_dir run_log
  run_name="TimeAlignOfficialUnified720_A5_${arm}_${CHECKPOINT_POLICY}"
  ...
  echo "run_start=$(date -Is) run_name=${run_name} dataset=${dataset} arm=${arm} readout_mode=${readout_mode} loss_mode=${loss_mode} basis_rank=${basis_rank} target_query_segment_len=${target_query_segment_len} target_query_ff=${target_query_ff} gpu=${gpu}"
  CUDA_VISIBLE_DEVICES="${gpu}" PYTHONUNBUFFERED=1 "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python baselines/timealign_official/train_repo.py \
      ...
      --target-query-heads 4 \
      --target-query-ff "${target_query_ff}" 2>&1 | tee "${run_log}"
...
echo "phase5_timealign_hss_a5_unified_head_sync_gate_done=$(date -Is)"
