...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase5_timealign_hss_a6_capacity_native_gate}"
...
ARMS="${ARMS:-a6_der a6_lbf_r256 a6_lbf_r512}"
SEQ_LEN="${SEQ_LEN:-720}"
...
arm_config() {
  ...
  case "${arm}" in
    a6_der)
      echo "prefix-native-dense-equivalent-row-bank multi-prefix 1 32 32 64"
      ;;
    a6_lbf_r256)
      echo "learned-basis-forecast-operator multi-prefix 1 32 32 256"
      ;;
    a6_lbf_r512)
      echo "learned-basis-forecast-operator multi-prefix 1 32 32 512"
      ;;
    *)
      echo "Unknown A6 arm: ${arm}" >&2
      exit 1
      ;;
  esac
}

echo "phase5_timealign_hss_a6_capacity_native_gate_start=$(date -Is)"
echo "cwd=$(pwd)"
...
run_job() {
  ...
  read -r readout_mode loss_mode prefix_samples continuous_min_prefix continuous_prefix_step basis_rank <<< "$(arm_config "${arm}")"
  local run_name run_dir run_log
  run_name="TimeAlignOfficialUnified720_A6_${arm}_${CHECKPOINT_POLICY}"
  ...
  echo "run_start=$(date -Is) run_name=${run_name} dataset=${dataset} arm=${arm} readout_mode=${readout_mode} basis_rank=${basis_rank} gpu=${gpu}"
...
echo "phase5_timealign_hss_a6_capacity_native_gate_done=$(date -Is)"
