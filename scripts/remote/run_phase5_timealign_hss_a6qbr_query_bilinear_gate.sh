...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase5_timealign_hss_a6qbr_query_bilinear_gate}"
...
ARMS="${ARMS:-a6qbr_r256 a6qbr_r512}"
SEQ_LEN="${SEQ_LEN:-720}"
...
arm_config() {
  ...
  case "${arm}" in
    a6qbr_r256)
      echo "query-bilinear-readout multi-prefix 1 32 32 256"
      ;;
    a6qbr_r512)
      echo "query-bilinear-readout multi-prefix 1 32 32 512"
      ;;
    *)
      echo "Unknown A6-QBR arm: ${arm}" >&2
      exit 1
      ;;
  esac
}

echo "phase5_timealign_hss_a6qbr_query_bilinear_gate_start=$(date -Is)"
echo "cwd=$(pwd)"
...
run_job() {
  ...
  run_name="TimeAlignOfficialUnified720_A6QBR_${arm}_${CHECKPOINT_POLICY}"
...
echo "phase5_timealign_hss_a6qbr_query_bilinear_gate_done=$(date -Is)"
