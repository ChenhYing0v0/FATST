...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase5_timealign_hss_a6s2_stability_calibration_gate}"
...
VARIANTS="${VARIANTS:-lbf_r256_ema0995 lbf_r256_ema0999 lbf_r256_smooth10 lbf_r256_smooth100 lbf_r256_ema0995_smooth10}"
SEQ_LEN="${SEQ_LEN:-720}"
...
if [[ "${CHECKPOINT_POLICY}" != "official-last" ]]; then
  echo "A6S2 calibration gate must use CHECKPOINT_POLICY=official-last." >&2
  exit 1
fi
...
variant_config() {
  ...
  case "${variant}" in
    lbf_r256_ema0995)
      echo "learned-basis-forecast-operator 256 1 0.995 0.0"
      ;;
    lbf_r256_ema0999)
      echo "learned-basis-forecast-operator 256 1 0.999 0.0"
      ;;
    lbf_r256_smooth10)
      echo "learned-basis-forecast-operator 256 0 0.0 10.0"
      ;;
    lbf_r256_smooth100)
      echo "learned-basis-forecast-operator 256 0 0.0 100.0"
      ;;
    lbf_r256_ema0995_smooth10)
      echo "learned-basis-forecast-operator 256 1 0.995 10.0"
      ;;
    *)
      echo "Unknown A6S2 calibration variant: ${variant}" >&2
      exit 1
      ;;
  esac
}

echo "phase5_timealign_hss_a6s2_stability_calibration_gate_start=$(date -Is)"
echo "cwd=$(pwd)"
...
run_job() {
  ...
  local variant="$2"
  local gpu="$3"
  local readout_mode basis_rank ema_eval ema_decay smooth_weight
  read -r readout_mode basis_rank ema_eval ema_decay smooth_weight <<< "$(variant_config "${variant}")"
  ...
  echo "run_start=$(date -Is) run_name=${run_name} dataset=${dataset} variant=${variant} readout_mode=${readout_mode} basis_rank=${basis_rank} ema_decay=${ema_decay} smooth_weight=${smooth_weight} gpu=${gpu}"
  local ema_args=()
  if [[ "${ema_eval}" == "1" ]]; then
    ema_args=(--ema-decay "${ema_decay}" --ema-eval)
  fi
  local smooth_args=()
  if [[ "${smooth_weight}" != "0.0" ]]; then
    smooth_args=(--basis-operator-smoothness-weight "${smooth_weight}")
  fi
...
echo "phase5_timealign_hss_a6s2_stability_calibration_gate_done=$(date -Is)"
