...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase5_timealign_hss_a6s_stability_gate}"
...
VARIANTS="${VARIANTS:-lbf_r256_base lbf_r256_ema099 lbf_r256_smooth1e3 lbf_r256_ema099_smooth1e3 der_ema099}"
SEQ_LEN="${SEQ_LEN:-720}"
...
if [[ "${CHECKPOINT_POLICY}" != "official-last" ]]; then
  echo "A6S stability gate must use CHECKPOINT_POLICY=official-last." >&2
  exit 1
fi
...
variant_config() {
  ...
  case "${variant}" in
    lbf_r256_base)
      echo "learned-basis-forecast-operator 256 0 0.0 0.0"
      ;;
    lbf_r256_ema099)
      echo "learned-basis-forecast-operator 256 1 0.99 0.0"
      ;;
    lbf_r256_smooth1e3)
      echo "learned-basis-forecast-operator 256 0 0.0 0.001"
      ;;
    lbf_r256_ema099_smooth1e3)
      echo "learned-basis-forecast-operator 256 1 0.99 0.001"
      ;;
    der_ema099)
      echo "prefix-native-dense-equivalent-row-bank 64 1 0.99 0.0"
      ;;
    *)
      echo "Unknown A6S stability variant: ${variant}" >&2
      exit 1
      ;;
  esac
}

echo "phase5_timealign_hss_a6s_stability_gate_start=$(date -Is)"
echo "cwd=$(pwd)"
...
echo "phase5_timealign_hss_a6s_stability_gate_done=$(date -Is)"
