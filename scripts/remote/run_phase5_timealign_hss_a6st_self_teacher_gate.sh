...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase5_timealign_hss_a6st_self_teacher_gate}"
...
VARIANTS="${VARIANTS:-a6st_w005_d0999_wu1 a6st_w01_d0999_wu1 a6st_w02_d0999_wu1 a6st_w01_d0999_wu3 a6st_w01_d0995_wu1}"
SEQ_LEN="${SEQ_LEN:-720}"
...
if [[ "${CHECKPOINT_POLICY}" != "official-last" ]]; then
  echo "A6ST self-teacher gate must use CHECKPOINT_POLICY=official-last." >&2
  exit 1
fi
...
variant_config() {
  ...
  case "${variant}" in
    ...
    a6st_w01_d0995_wu1)
      ...
      ;;
    *)
      echo "Unknown A6ST self-teacher variant: ${variant}" >&2
      exit 1
      ;;
  esac
}

echo "phase5_timealign_hss_a6st_self_teacher_gate_start=$(date -Is)"
echo "cwd=$(pwd)"
...
run_job() {
  ...
  local variant="$2"
  local gpu="$3"
  local self_teacher_decay self_teacher_weight self_teacher_warmup
  read -r self_teacher_decay self_teacher_weight self_teacher_warmup <<< "$(variant_config "${variant}")"
  ...
  echo "run_start=$(date -Is) run_name=${run_name} dataset=${dataset} variant=${variant} self_teacher_decay=${self_teacher_decay} self_teacher_weight=${self_teacher_weight} self_teacher_warmup=${self_teacher_warmup} gpu=${gpu}"
  CUDA_VISIBLE_DEVICES="${gpu}" PYTHONUNBUFFERED=1 "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python baselines/timealign_official/train_repo.py \
      ...
      --self-teacher-warmup-epochs "${self_teacher_warmup}" 2>&1 | tee "${run_log}"
...
echo "phase5_timealign_hss_a6st_self_teacher_gate_done=$(date -Is)"
