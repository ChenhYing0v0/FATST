...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase5_timealign_hss_a8tag_teacher_advantage_gate}"
...
VARIANTS="${VARIANTS:-a8tag_advbin_w02_d0999_wu1 a8tag_advbin_w05_d0999_wu1 a8tag_advratio_w10_d0999_wu1}"
SEQ_LEN="${SEQ_LEN:-720}"
...
if [[ "${CHECKPOINT_POLICY}" != "official-last" ]]; then
  echo "A8TAG teacher-advantage gate must use CHECKPOINT_POLICY=official-last." >&2
  exit 1
fi
...
variant_config() {
  local variant="$1"
  case "${variant}" in
    a8tag_advbin_w02_d0999_wu1)
      echo "0.999 0.20 1 teacher-advantage-binary"
      ;;
    a8tag_advbin_w05_d0999_wu1)
      echo "0.999 0.50 1 teacher-advantage-binary"
      ;;
    a8tag_advratio_w10_d0999_wu1)
      echo "0.999 1.00 1 teacher-advantage-ratio"
      ;;
    *)
      echo "Unknown A8TAG teacher-advantage variant: ${variant}" >&2
      exit 1
      ;;
  esac
}

echo "phase5_timealign_hss_a8tag_teacher_advantage_gate_start=$(date -Is)"
echo "cwd=$(pwd)"
...
run_job() {
  ...
  local variant="$2"
  local gpu="$3"
  local self_teacher_decay self_teacher_weight self_teacher_warmup gate_mode
  read -r self_teacher_decay self_teacher_weight self_teacher_warmup gate_mode <<< "$(variant_config "${variant}")"
  ...
  echo "run_start=$(date -Is) run_name=${run_name} dataset=${dataset} variant=${variant} self_teacher_decay=${self_teacher_decay} self_teacher_weight=${self_teacher_weight} self_teacher_warmup=${self_teacher_warmup} gate_mode=${gate_mode} gpu=${gpu}"
  CUDA_VISIBLE_DEVICES="${gpu}" PYTHONUNBUFFERED=1 "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python baselines/timealign_official/train_repo.py \
      ...
      --self-teacher-gate-mode "${gate_mode}" 2>&1 | tee "${run_log}"
...
echo "phase5_timealign_hss_a8tag_teacher_advantage_gate_done=$(date -Is)"
