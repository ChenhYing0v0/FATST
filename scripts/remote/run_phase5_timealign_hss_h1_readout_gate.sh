...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase5_timealign_hss_h1_readout_gate}"
...
ARMS="${ARMS:-prefix_conditioned_stochastic_k2 target_set_decoder_multiprefix}"
SEQ_LEN="${SEQ_LEN:-720}"
...
arm_config() {
  local arm="$1"
  case "${arm}" in
    prefix_conditioned_stochastic_k2)
      echo "prefix-conditioned-head stochastic-prefix 2 32 32"
      ;;
    target_set_decoder_multiprefix)
      echo "target-set-decoder multi-prefix 1 32 32"
      ;;
    *)
      echo "Unknown H1 arm: ${arm}" >&2
      exit 1
      ;;
  esac
}

echo "phase5_timealign_hss_h1_readout_gate_start=$(date -Is)"
echo "cwd=$(pwd)"
...
run_job() {
  ...
  run_name="TimeAlignOfficialUnified720_H1_${arm}_${CHECKPOINT_POLICY}"
...
echo "phase5_timealign_hss_h1_readout_gate_done=$(date -Is)"
