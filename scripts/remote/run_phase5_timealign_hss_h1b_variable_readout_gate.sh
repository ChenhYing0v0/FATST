...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase5_timealign_hss_h1b_variable_readout_gate}"
...
ARMS="${ARMS:-target_set_prefix_head_multiprefix prefix_token_decoder_multiprefix}"
SEQ_LEN="${SEQ_LEN:-720}"
...
arm_config() {
  local arm="$1"
  case "${arm}" in
    target_set_prefix_head_multiprefix)
      echo "target-set-prefix-head multi-prefix 1 32 32"
      ;;
    prefix_token_decoder_multiprefix)
      echo "prefix-token-decoder multi-prefix 1 32 32"
      ;;
    *)
      echo "Unknown H1B arm: ${arm}" >&2
      exit 1
      ;;
  esac
}

echo "phase5_timealign_hss_h1b_variable_readout_gate_start=$(date -Is)"
echo "cwd=$(pwd)"
...
run_job() {
  ...
  run_name="TimeAlignOfficialUnified720_H1B_${arm}_${CHECKPOINT_POLICY}"
...
echo "phase5_timealign_hss_h1b_variable_readout_gate_done=$(date -Is)"
