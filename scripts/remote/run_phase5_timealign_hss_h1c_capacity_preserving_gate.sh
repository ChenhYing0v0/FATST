...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase5_timealign_hss_h1c_capacity_preserving_gate}"
...
ARMS="${ARMS:-dense_prefix_residual_adapter_multiprefix row_gated_dense_head_multiprefix prefix_adapter_shared_dense_multiprefix}"
SEQ_LEN="${SEQ_LEN:-720}"
...
arm_config() {
  local arm="$1"
  case "${arm}" in
    dense_prefix_residual_adapter_multiprefix)
      echo "dense-prefix-residual-adapter multi-prefix 1 32 32"
      ;;
    row_gated_dense_head_multiprefix)
      echo "row-gated-dense-head multi-prefix 1 32 32"
      ;;
    prefix_adapter_shared_dense_multiprefix)
      echo "prefix-adapter-shared-dense multi-prefix 1 32 32"
      ;;
    *)
      echo "Unknown H1C arm: ${arm}" >&2
      exit 1
      ;;
  esac
}

echo "phase5_timealign_hss_h1c_capacity_preserving_gate_start=$(date -Is)"
echo "cwd=$(pwd)"
...
run_job() {
  ...
  local arm="$2"
  local gpu="$3"
  local readout_mode loss_mode prefix_samples continuous_min_prefix continuous_prefix_step
  read -r readout_mode loss_mode prefix_samples continuous_min_prefix continuous_prefix_step <<< "$(arm_config "${arm}")"
  local run_name run_dir run_log
  run_name="TimeAlignOfficialUnified720_H1C_${arm}_${CHECKPOINT_POLICY}"
  ...
  echo "run_start=$(date -Is) run_name=${run_name} dataset=${dataset} arm=${arm} readout_mode=${readout_mode} loss_mode=${loss_mode} prefix_samples=${prefix_samples} gpu=${gpu}"
  CUDA_VISIBLE_DEVICES="${gpu}" PYTHONUNBUFFERED=1 "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python baselines/timealign_official/train_repo.py \
      ...
      --continuous-prefix-step "${continuous_prefix_step}" 2>&1 | tee "${run_log}"
...
echo "phase5_timealign_hss_h1c_capacity_preserving_gate_done=$(date -Is)"
