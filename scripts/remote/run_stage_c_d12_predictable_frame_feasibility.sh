...
if [[ "${DRY_RUN}" == "1" ]]; then
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/run_stage_c_d12_predictable_frame_feasibility.py \
      --synthetic-smoke
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/analyze_stage_c_d12_predictable_frame_feasibility.py \
      --synthetic-smoke
  echo "stage_c_d12_dry_run=pass"
  exit 0
fi

if [[ "${#GPU_IDS[@]}" -lt 3 ]]; then
  echo "D12-A requires three GPU ids" >&2
  exit 2
fi

mkdir -p "${OUTPUT_ROOT}/_logs"
{
  echo "stage_c_d12_start=$(date -Is)"
...
run_dataset() {
  ...
  if [[ -s "${OUTPUT_ROOT}/${dataset}/dataset_summary.json" ]]; then
