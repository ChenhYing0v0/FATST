...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/stage_c_d19_if_control_v1_1}"
...
CONFIG="${CONFIG:-configs/stage_c_d19_if_control_step7b.json}"
...
PROTOCOL_PROFILE="stage_c_d19_if_control_v1_1"
...
arms={arm["id"]: arm for arm in config["arms"] if arm["training_new"]}
contracts=config["profile_contracts"]
for dataset,arm_id in config["launch_order"]:
    arm=arms[arm_id]
    profile=profiles[dataset]
    direct_width=contracts[profile["profile"]]["direct_hidden_width"]
    print("\t".join(map(str,(
        dataset,arm_id,arm["readout_mode"],arm["objective_mode"],
        profile["profile"],profile["patch_num"],profile["d_model"],
        profile["d_ff"],direct_width,
    ))))
' "${CONFIG}"
)
...
if [[ "${STATUS_ONLY}" == "1" ]]; then
  ...
  echo "d19_status=$(date -Is) completed=${completed}/${#LINES[@]}"
...
if [[ "${DRY_RUN}" == "1" ]]; then
  printf '%s\n' "${LINES[@]}"
  echo "d19_dry_run=pass jobs=${#LINES[@]} config_hash=${CONFIG_HASH} profile_hash=${PROFILE_HASH} remote_authorized=${REMOTE_AUTHORIZED} test_authorized=${TEST_AUTHORIZED}"
  exit 0
fi

if [[ "${RESOURCE_SMOKE}" == "1" ]]; then
  if [[ "${REMOTE_AUTHORIZED}" != "true" ]]; then
    echo "D19 remote resource smoke is not authorized by ${CONFIG}" >&2
    exit 3
  fi
elif [[ "${REMOTE_AUTHORIZED}" != "true" || "${TEST_AUTHORIZED}" != "true" ]]; then
  echo "D19 remote/test launch is not authorized by ${CONFIG}" >&2
  exit 3
fi

run_training_command() {
  local line="$1" gpu="$2" output_dir="$3" run_log="$4" smoke="$5"
  local dataset arm readout objective profile patch_num d_model d_ff direct_width
  local run_args=()
  IFS=$'\t' read -r dataset arm readout objective profile patch_num d_model d_ff direct_width <<< "${line}"
  if [[ "${smoke}" == "1" ]]; then
    run_args=(
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python baselines/timealign_official/train_repo.py \
      ...
      --num-workers 0 --run-name "D19_${arm}" \
      ...
      --if-hidden-width 2048 --if-direct-hidden-width "${direct_width}" \
      --if-head-dropout 0.1 --if-fourier-norm ortho \
...
if [[ "${RESOURCE_SMOKE}" == "1" ]]; then
  smoke_lines=()
  for line in "${LINES[@]}"; do
    if [[ "${line}" == $'Weather\tif_measure\t'* \
      || "${line}" == $'ETTm2\tdirect_nonlinear_matched_measure\t'* ]]; then
      smoke_lines+=("${line}")
    fi
  done
...
mkdir -p "${LOG_ROOT}" "${ANALYSIS_ROOT}"
{
  ...
  echo "artifact_units=20"
  echo "official_test_cells=80"
...
if [[ "${status}" != "0" ]]; then
  echo "d19_worker_failure=$(date -Is)" >&2
  exit 1
fi

"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
  python scripts/analyze_stage_c_d19_if_control.py \
    --raw-root "${OUTPUT_ROOT}" --control-root "${CONTROL_ROOT}" \
    --output-dir "${ANALYSIS_ROOT}" --config "${CONFIG}" --seed "${SEED}"
echo "d19_done=$(date -Is)"
