...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/stage_c_d20_cst_v1}"
DATASET_ROOT="${DATASET_ROOT:-/home/yingch/dataset}"
CONFIG="${CONFIG:-configs/stage_c_d20_cst_step7b.json}"
...
PROTOCOL_PROFILE="stage_c_d20_cst_v1"
...
for dataset,arm_id in config["launch_order"]:
    ...
    print("\t".join(map(str,(
        dataset,arm_id,arm["readout_mode"],arm["history_statistic_mode"],
...
if [[ "${STATUS_ONLY}" == "1" ]]; then
  ...
  echo "d20_status=$(date -Is) completed=${completed}/${#LINES[@]}"
...
if [[ "${DRY_RUN}" == "1" ]]; then
  printf '%s\n' "${LINES[@]}"
  echo "d20_dry_run=pass jobs=${#LINES[@]} config_hash=${CONFIG_HASH} profile_hash=${PROFILE_HASH} remote_authorized=${REMOTE_AUTHORIZED} test_authorized=${TEST_AUTHORIZED}"
  exit 0
fi

if [[ "${RESOURCE_SMOKE}" == "1" ]]; then
  if [[ "${REMOTE_AUTHORIZED}" != "true" ]]; then
    echo "D20 remote resource smoke is not authorized by ${CONFIG}" >&2
    exit 3
  fi
elif [[ "${REMOTE_AUTHORIZED}" != "true" || "${TEST_AUTHORIZED}" != "true" ]]; then
  echo "D20 remote/test launch is not authorized by ${CONFIG}" >&2
  exit 3
fi

run_training_command() {
  local line="$1" gpu="$2" output_dir="$3" run_log="$4" smoke="$5"
  local dataset arm readout statistic objective profile patch_num d_model d_ff
  local run_args=()
  IFS=$'\t' read -r dataset arm readout statistic objective profile patch_num d_model d_ff <<< "${line}"
  if [[ "${smoke}" == "1" ]]; then
    run_args=(
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python baselines/timealign_official/train_repo.py \
      ...
      --num-workers 0 --run-name "D20_${arm}" \
      ...
      --history-statistic-mode "${statistic}" --history-statistic-dim 64 \
      --history-statistic-random-seed 20260719 \
...
if [[ "${RESOURCE_SMOKE}" == "1" ]]; then
  smoke_lines=()
  for line in "${LINES[@]}"; do
    if [[ "${line}" == $'Weather\tA6_CST_SPEC\t'* \
      || "${line}" == $'ETTm2\tA6_CST_RANDOM\t'* ]]; then
      smoke_lines+=("${line}")
    fi
  done
  ...
  echo "d20_resource_smoke_done=$(date -Is) output=${smoke_root}"
  exit 0
fi
...
if [[ "${status}" != "0" ]]; then
  echo "d20_worker_failure=$(date -Is)" >&2
  exit 1
fi

"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
  python scripts/analyze_stage_c_d20_cst.py \
    --raw-root "${OUTPUT_ROOT}" --output-dir "${ANALYSIS_ROOT}" \
    --config "${CONFIG}" --seed "${SEED}"
echo "d20_done=$(date -Is)"
