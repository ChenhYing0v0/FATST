...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/stage_c_d23_fcmi_v1}"
DATASET_ROOT="${DATASET_ROOT:-/home/yingch/dataset}"
CONFIG="${CONFIG:-configs/stage_c_d23_fcmi_step7b.json}"
...
PROTOCOL_PROFILE="stage_c_d23_fcmi_v1"
...
for dataset,arm_id in config["launch_order"]:
    ...
    dense_rank=dense_ranks[dataset] if arm_id=="DENSE_DUAL_MATCHED" else 0
    print("\t".join(map(str,(
        dataset,arm_id,arm["readout_mode"],arm["evaluation_role"],
        arm["objective_mode"],profile["profile"],profile["patch_num"],
        profile["d_model"],profile["d_ff"],dense_rank,
    ))))
' "${CONFIG}"
)
...
is_complete() {
  local line="$1" output_dir evaluation_role
  output_dir="$(run_dir_for_line "${line}")"
  evaluation_role="$(cut -f4 <<< "${line}")"
  local common_complete=1
  [[ -s "${output_dir}/metrics_by_target_horizon.csv" \
    ...
    || common_complete=0
  if [[ "${common_complete}" != "1" ]]; then return 1; fi
  if [[ "${evaluation_role}" == "validation_only" ]]; then return 0; fi
...
if [[ "${STATUS_ONLY}" == "1" ]]; then
  ...
  echo "d23_status=$(date -Is) completed=${completed}/${#LINES[@]}"
...
if [[ "${DRY_RUN}" == "1" ]]; then
  printf '%s\n' "${LINES[@]}"
  echo "d23_dry_run=pass jobs=${#LINES[@]} config_hash=${CONFIG_HASH} profile_hash=${PROFILE_HASH} remote_authorized=${REMOTE_AUTHORIZED} test_authorized=${TEST_AUTHORIZED}"
  exit 0
fi

if [[ "${REMOTE_AUTHORIZED}" != "true" ]]; then
  echo "D23 remote launch is not authorized by ${CONFIG}" >&2
  exit 3
fi
if [[ "${RESOURCE_SMOKE}" != "1" && "${TEST_AUTHORIZED}" != "true" ]]; then
  echo "D23 formal test launch is not authorized by ${CONFIG}" >&2
  exit 3
fi

run_training_command() {
  local line="$1" gpu="$2" output_dir="$3" run_log="$4" smoke="$5"
  local dataset arm readout evaluation_role objective profile patch_num
  local d_model d_ff dense_rank
  local run_args=()
  IFS=$'\t' read -r dataset arm readout evaluation_role objective profile \
    patch_num d_model d_ff dense_rank <<< "${line}"
  if [[ "${smoke}" == "1" ]]; then
    run_args=(
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${CONDA_BIN}" run --no-capture-output \
    -n "${CONDA_ENV}" python baselines/timealign_official/train_repo.py \
      ...
      --num-workers 0 --run-name "D23_${arm}" \
      ...
      --fcmi-n-heads 8 --fcmi-dropout 0 \
      --fcmi-permutation-seed 20260720 \
      --fcmi-dense-rank "${dense_rank}" \
...
if [[ "${RESOURCE_SMOKE}" == "1" ]]; then
  smoke_lines=()
  for line in "${LINES[@]}"; do
    if [[ "${line}" == $'Weather\tFCMI\t'* \
      || "${line}" == $'ETTm2\tDENSE_DUAL_MATCHED\t'* ]]; then
      smoke_lines+=("${line}")
    fi
  done
  ...
  echo "d23_resource_smoke_done=$(date -Is) output=${smoke_root}"
  exit 0
fi
...
{
  echo "d23_start=$(date -Is)"
  echo "commit=$(git rev-parse HEAD)"
  echo "output_root=${OUTPUT_ROOT}"
  echo "config_hash=${CONFIG_HASH}"
  echo "profile_hash=${PROFILE_HASH}"
  echo "gpu_ids=${GPU_IDS[*]}"
  echo "training_runs=${#LINES[@]}"
  echo "validation_cells=160"
  echo "official_test_cells=160"
  echo "checkpoint_selection=best_val_mean_mse_h96_h192_h336_h720"
  echo "test_informed=true"
  echo "role=paper_core_candidate_formal_screen"
  nvidia-smi --query-gpu=index,name,memory.total,memory.used,memory.free,utilization.gpu \
    --format=csv,noheader,nounits
...
run_one() {
  ...
  local dataset arm readout evaluation_role objective profile patch_num
  local d_model d_ff dense_rank output_dir run_log checkpoint_before
  ...
  mkdir -p "${output_dir}"
  echo "train_start=$(date -Is) job=$((index + 1))/${#LINES[@]} arm=${arm} dataset=${dataset} gpu=${gpu} profile=${profile}"
  ...
  if [[ "${evaluation_role}" == "formal_test" ]]; then
    CUDA_VISIBLE_DEVICES="${gpu}" "${CONDA_BIN}" run --no-capture-output \
      -n "${CONDA_ENV}" \
...
if [[ "${status}" != "0" ]]; then
  echo "d23_worker_failure=$(date -Is)" >&2
  exit 1
fi

"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
  python scripts/analyze_stage_c_d23_fcmi.py \
    --raw-root "${OUTPUT_ROOT}" --output-dir "${ANALYSIS_ROOT}" \
    --config "${CONFIG}" --seed "${SEED}"
echo "d23_done=$(date -Is)"
