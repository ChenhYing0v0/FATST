...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/stage_c_fair_reaudit_v1}"
DATASET_ROOT="${DATASET_ROOT:-/home/yingch/dataset}"
CONFIG="${CONFIG:-configs/stage_c_fair_reaudit_v1.json}"
...
PROTOCOL_PROFILE="stage_c_fair_reaudit_v1"
...
if [[ "${STATUS_ONLY}" == "1" ]]; then
  ...
  echo "fair_reaudit_status=$(date -Is) completed=${completed}/${#LINES[@]}"
...
if [[ "${DRY_RUN}" == "1" ]]; then
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/check_stage_c_fair_reaudit_v1.py >/dev/null
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/evaluate_stage_c_pcsd_cf_checkpoint.py \
      --synthetic-smoke >/dev/null
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/analyze_stage_c_fair_reaudit_v1.py \
      --synthetic-smoke >/dev/null
  printf '%s\n' "${LINES[@]}"
  echo "fair_reaudit_dry_run=pass jobs=${#LINES[@]} config_hash=${CONFIG_HASH} profile_hash=${PROFILE_HASH} test=true"
  exit 0
fi
...
run_training_command() {
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python baselines/timealign_official/train_repo.py \
      ...
      --num-workers 0 --run-name "FAIR_REAUDIT_${arm}" \
...
if [[ "${RESOURCE_SMOKE}" == "1" ]]; then
  smoke_line=""
  for line in "${LINES[@]}"; do
    if [[ "${line}" == $'Weather\tsiff_pcc\t'* ]]; then
      smoke_line="${line}"
      break
    fi
  done
  test -n "${smoke_line}"
  smoke_root="${OUTPUT_ROOT}/_resource_smoke/siff_pcc_weather_seed${SEED}"
  mkdir -p "${smoke_root}"
...
{
  echo "fair_reaudit_start=$(date -Is)"
  ...
  echo "test_role=primary_mechanism_effectiveness_and_paper_benchmark"
...
if [[ "${status}" != "0" ]]; then
  echo "fair_reaudit_worker_failure=$(date -Is)" >&2
  exit 1
fi
...
echo "fair_reaudit_done=$(date -Is)"
