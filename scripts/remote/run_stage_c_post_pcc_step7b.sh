...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/stage_c_post_pcc_step7b}"
DATASET_ROOT="${DATASET_ROOT:-/home/yingch/dataset}"
CONFIG="${CONFIG:-configs/stage_c_post_pcc_step7b.json}"
...
PROTOCOL_PROFILE="stage_c_post_pcc_step7b_validation_screen_v1"
...
is_coupling_line() {
  local line="$1" dataset arm readout rest
  IFS=$'\t' read -r dataset arm readout rest <<< "${line}"
  [[ "${readout}" != "siff-dense-nonlinear-matched" ]]
}
...
is_complete() {
  ...
  [[ -s "${output_dir}/metrics_by_target_horizon.csv" \
    ...
    && -s "${output_dir}/pcsd_validation_diagnostics.npz" ]] || return 1
  if is_coupling_line "${line}"; then
...
if [[ "${STATUS_ONLY}" == "1" ]]; then
  ...
  echo "post_pcc_step7b_status=$(date -Is) completed=${completed}/${#LINES[@]}"
...
if [[ "${DRY_RUN}" == "1" ]]; then
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/check_stage_c_post_pcc_step7b.py >/dev/null
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
  ...
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/evaluate_stage_c_sc2_pcc_gradient.py \
      --synthetic-smoke >/dev/null
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/analyze_stage_c_post_pcc_step7b.py \
      --synthetic-smoke >/dev/null
  printf '%s\n' "${LINES[@]}"
  echo "post_pcc_step7b_dry_run=pass jobs=${#LINES[@]} config_hash=${CONFIG_HASH} profile_hash=${PROFILE_HASH} test=false"
  exit 0
fi
...
run_training_command() {
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python baselines/timealign_official/train_repo.py \
      ...
      --num-workers 0 --run-name "POST_PCC_STEP7B_${arm}" --output-dir "${output_dir}" \
...
if [[ "${RESOURCE_SMOKE}" == "1" ]]; then
  smoke_line=""
  for line in "${LINES[@]}"; do
    if [[ "${line}" == $'Weather\tsiff_mcca\t'* ]]; then
      smoke_line="${line}"
      break
    fi
  done
  test -n "${smoke_line}"
  smoke_root="${OUTPUT_ROOT}/_resource_smoke/siff_mcca_weather_seed${SEED}"
  mkdir -p "${smoke_root}"
...
{
  echo "post_pcc_step7b_start=$(date -Is)"
...
run_one() {
  ...
  local dataset arm readout objective rank profile patch_num d_model d_ff output_dir run_log
  IFS=$'\t' read -r dataset arm readout objective rank profile patch_num d_model d_ff <<< "${line}"
  output_dir="$(run_dir_for_line "${line}")"
  ...
  mkdir -p "${output_dir}"
  echo "run_start=$(date -Is) job=$((index + 1))/${#LINES[@]} arm=${arm} dataset=${dataset} gpu=${gpu} profile=${profile} rank=${rank}"
  run_training_command "${line}" "${gpu}" "${output_dir}" "${run_log}" 0
  CUDA_VISIBLE_DEVICES="${gpu}" "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/evaluate_stage_c_pcsd_cf_checkpoint.py \
      --run-dir "${output_dir}" --design "${CONFIG}" --device cuda \
      >>"${run_log}" 2>&1
  if is_coupling_line "${line}"; then
    CUDA_VISIBLE_DEVICES="${gpu}" "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
      python scripts/evaluate_stage_c_sc2_pcc_gradient.py \
...
if [[ "${status}" != "0" ]]; then
  echo "post_pcc_step7b_worker_failure=$(date -Is)" >&2
  exit 1
fi
...
"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
  python scripts/analyze_stage_c_post_pcc_step7b.py \
    ...
    --pcsd-reference-root "$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["references"]["pcsd_cf_root"])' "${CONFIG}")" \
    --pcc-reference-root "$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["references"]["pcc_root"])' "${CONFIG}")" \
    --output-dir "${ANALYSIS_ROOT}" --config "${CONFIG}" --seed "${SEED}"
echo "post_pcc_step7b_done=$(date -Is)"
