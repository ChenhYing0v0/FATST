...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/stage_c_sc1_d11_future_component_responsibility}"
CONTRACT="${CONTRACT:-configs/stage_c_five_dataset_natural_profiles.json}"
DESIGN="${DESIGN:-configs/stage_c_sc1_d11_future_component_responsibility.json}"
...
if [[ "${DRY_RUN}" == "1" ]]; then
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/run_stage_c_sc1_d11_future_component_responsibility.py \
      --synthetic-smoke
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/analyze_stage_c_sc1_d11_future_component_responsibility.py \
      --synthetic-smoke
  echo "stage_c_sc1_d11_dry_run=pass"
  exit 0
fi

if [[ "${#GPU_IDS[@]}" -lt 3 ]]; then
  echo "D11 requires three GPU ids" >&2
  exit 2
fi

mkdir -p "${OUTPUT_ROOT}/_logs"
{
  echo "stage_c_sc1_d11_start=$(date -Is)"
  echo "commit=$(git rev-parse HEAD)"
  echo "cwd=$(pwd)"
  echo "output_root=${OUTPUT_ROOT}"
  echo "role=diagnostic_only"
  echo "uses_test_split=false"
  echo "trains_forecast_model=false"
