...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/stage_c_sc1_d2_core3_precheck}"
...
if [[ "${DRY_RUN}" == "1" ]]; then
  ...
  echo "stage_c_sc1_d2_dry_run=pass datasets=${#DATASETS[@]}"
  exit 0
fi

mkdir -p "${OUTPUT_ROOT}/_logs"
echo "stage_c_sc1_d2_start=$(date -Is) commit=$(git rev-parse HEAD)"
pids=()
...
"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
  python scripts/analyze_stage_c_sc1_d2_diagnostic.py \
    ...
    --output-dir "${OUTPUT_ROOT}" \
    --expected-suite core3
echo "stage_c_sc1_d2_done=$(date -Is)"
