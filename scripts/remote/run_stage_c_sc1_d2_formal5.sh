...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/stage_c_sc1_d2_formal5}"
...
if [[ "${#GPU_IDS[@]}" -lt 3 ]]; then
  echo "formal5 requires three GPU ids" >&2
  exit 2
fi
...
if [[ "${DRY_RUN}" == "1" ]]; then
  ...
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/run_stage_c_sc1_d2_diagnostic.py --synthetic-smoke
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/analyze_stage_c_sc1_d2_diagnostic.py --synthetic-smoke
  echo "stage_c_sc1_d2_formal5_dry_run=pass datasets=5 arms=165"
  exit 0
fi

mkdir -p "${OUTPUT_ROOT}/_logs"
echo "stage_c_sc1_d2_formal5_start=$(date -Is) commit=$(git rev-parse HEAD)"
nvidia-smi --query-gpu=index,name,memory.used,memory.free,utilization.gpu \
  --format=csv,noheader,nounits

run_dataset() {
  local dataset="$1" gpu="$2"
  if [[ -s "${OUTPUT_ROOT}/${dataset}/d2_probe_metrics.csv" ]]; then
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/run_stage_c_sc1_d2_diagnostic.py \
      ...
      --contract "${CONTRACT}" --output-dir "${OUTPUT_ROOT}" --dataset "${dataset}" \
      --device cuda --diagnostic-suite formal5 --train-batches "${TRAIN_BATCHES}" \
      --val-batches "${VAL_BATCHES}" --max-epochs "${MAX_EPOCHS}" \
      --patience "${PATIENCE}" \
...
echo "stage_c_sc1_d2_formal5_done=$(date -Is)"
