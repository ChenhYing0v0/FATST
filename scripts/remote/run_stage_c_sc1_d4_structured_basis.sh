...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/stage_c_sc1_d4_structured_basis}"
...
if [[ "${#GPU_IDS[@]}" -lt 3 ]]; then
  echo "SC1-D4 requires three GPU ids" >&2
  exit 2
fi
...
if [[ "${DRY_RUN}" == "1" ]]; then
  ...
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/analyze_stage_c_sc1_d4_structured_basis.py --synthetic-smoke
  echo "stage_c_sc1_d4_dry_run=pass datasets=5 fits=315"
  exit 0
fi

mkdir -p "${OUTPUT_ROOT}/_logs"
echo "stage_c_sc1_d4_start=$(date -Is) commit=$(git rev-parse HEAD)"
...
run_dataset() {
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/run_stage_c_sc1_d4_structured_basis.py \
      ...
      --contract "${CONTRACT}" --d4-config "${D4_CONFIG}" \
...
"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
  python scripts/analyze_stage_c_sc1_d4_structured_basis.py \
    --input-root "${OUTPUT_ROOT}" --d4-config "${D4_CONFIG}" \
    --output-dir "${OUTPUT_ROOT}"
echo "stage_c_sc1_d4_done=$(date -Is)"
