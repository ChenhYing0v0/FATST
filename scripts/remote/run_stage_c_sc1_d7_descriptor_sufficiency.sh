...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/stage_c_sc1_d7_descriptor_sufficiency}"
...
if [[ "${#GPU_IDS[@]}" -lt 3 ]]; then
  echo "SC1-D7 requires three GPU ids" >&2
  exit 2
fi
...
if [[ "${DRY_RUN}" == "1" ]]; then
  ...
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/run_stage_c_sc1_d7_descriptor_sufficiency.py --synthetic-smoke
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/analyze_stage_c_sc1_d7_descriptor_sufficiency.py --synthetic-smoke
  echo "stage_c_sc1_d7_dry_run=pass datasets=5 fits=105"
  exit 0
fi

mkdir -p "${OUTPUT_ROOT}/_logs"
echo "stage_c_sc1_d7_start=$(date -Is) commit=$(git rev-parse HEAD)"
nvidia-smi --query-gpu=index,name,memory.used,memory.free,utilization.gpu \
  --format=csv,noheader,nounits

run_dataset() {
  local dataset="$1" gpu="$2"
  if [[ -s "${OUTPUT_ROOT}/${dataset}/d7_probe_metrics.csv" ]]; then
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/run_stage_c_sc1_d7_descriptor_sufficiency.py \
      ...
      --contract "${CONTRACT}" --d7-config "${D7_CONFIG}" \
...
worker "${GPU_IDS[0]}" Weather & pid0="$!"
worker "${GPU_IDS[1]}" ETTm1 ETTh2 & pid1="$!"
worker "${GPU_IDS[2]}" ETTm2 ETTh1 & pid2="$!"
wait "${pid0}"
wait "${pid1}"
wait "${pid2}"
...
"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
  python scripts/analyze_stage_c_sc1_d7_descriptor_sufficiency.py \
    --input-root "${OUTPUT_ROOT}" --d7-config "${D7_CONFIG}" \
    --output-dir "${OUTPUT_ROOT}"
echo "stage_c_sc1_d7_done=$(date -Is)"
