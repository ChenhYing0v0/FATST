...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/stage_c_sc1_d8_e2e}"
...
PROTOCOL_PROFILE="stage_c_sc1_d8_e2e_v1"
...
# Arm-major ordering spreads Weather and ETTm1 across workers early.
...
if [[ "${STATUS_ONLY}" == "1" ]]; then
  ...
  for line in "${LINES[@]}"; do
    ...
    [[ -s "${output_dir}/patch_diagnostics.json" ]] \
      && patch_complete=$((patch_complete + 1))
  done
  echo "sc1_d8_status=$(date -Is) completed=${completed}/${#LINES[@]} invariants=${invariant_complete}/${#LINES[@]} patch=${patch_complete}/${#LINES[@]}"
...
if [[ "${DRY_RUN}" == "1" ]]; then
  ...
  for readout in \
    ...
    plgo-paf-geo-m694 \
    plgo-paf-perm-m694 \
    plgo-paf-random-m694; do
    "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
      python scripts/check_stage_c_sc1_d8_checkpoint_invariants.py \
      --synthetic-readout "${readout}" --seed "${SEED}" >/dev/null
  done
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/analyze_stage_c_sc1_d8_e2e.py --synthetic-smoke >/dev/null
  printf '%s\n' "${LINES[@]}"
  echo "stage_c_sc1_d8_dry_run=pass jobs=${#LINES[@]} profile_hash=${PROFILE_HASH} final_split=val"
  exit 0
fi

mkdir -p "${OUTPUT_ROOT}/_logs" "${OUTPUT_ROOT}/_analysis"
{
  echo "stage_c_sc1_d8_start=$(date -Is)"
...
run_one() {
  ...
  if [[ -s "${output_dir}/metrics_by_target_horizon.csv" \
    && -s "${output_dir}/trained_invariants.json" \
    && -s "${output_dir}/patch_diagnostics.json" ]]; then
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python baselines/timealign_official/train_repo.py \
      ...
      --run-name "SC1_D8_${arm}" --output-dir "${output_dir}" --device cuda \
      ...
      --patch-diagnostic-batches 8 --pred-loss-mode full \
...
if [[ "${status}" != "0" ]]; then
  echo "stage_c_sc1_d8_worker_failure=$(date -Is)" >&2
  exit 1
fi

"${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
  python scripts/analyze_stage_c_sc1_d8_e2e.py \
    --raw-root "${OUTPUT_ROOT}" --output-dir "${OUTPUT_ROOT}/_analysis"
echo "stage_c_sc1_d8_done=$(date -Is)"
