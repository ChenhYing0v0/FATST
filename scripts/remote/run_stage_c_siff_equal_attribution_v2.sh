...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/stage_c_siff_equal_attribution_v2}"
DATASET_ROOT="${DATASET_ROOT:-/home/yingch/dataset}"
CONFIG="${CONFIG:-configs/stage_c_siff_equal_attribution_v2.json}"
...
PROTOCOL_PROFILE="stage_c_siff_equal_attribution_v2"
...
if [[ "${STATUS_ONLY}" == "1" ]]; then
  ...
  echo "siff_equal_attribution_status=$(date -Is) completed=${completed}/${#LINES[@]}"
...
if [[ "${DRY_RUN}" == "1" ]]; then
  ...
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/analyze_stage_c_siff_equal_attribution_v2.py \
      --config "${CONFIG}" --synthetic-smoke >/dev/null
  printf '%s\n' "${LINES[@]}"
  echo "siff_equal_attribution_dry_run=pass jobs=${#LINES[@]} config_hash=${CONFIG_HASH} profile_hash=${PROFILE_HASH} remote_authorized=${REMOTE_AUTHORIZED} test_authorized=${TEST_AUTHORIZED}"
  exit 0
fi

if [[ "${RESOURCE_SMOKE}" == "1" ]]; then
  if [[ "${REMOTE_AUTHORIZED}" != "true" ]]; then
    echo "remote resource smoke is not authorized by ${CONFIG}; run Step7B prelaunch first" >&2
    exit 3
  fi
elif [[ "${REMOTE_AUTHORIZED}" != "true" || "${TEST_AUTHORIZED}" != "true" ]]; then
  echo "remote/test launch is not authorized by ${CONFIG}; run Step7B prelaunch first" >&2
  exit 3
fi
...
run_training_command() {
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python baselines/timealign_official/train_repo.py \
      ...
      --num-workers 0 --run-name "SIFF_EQ_ATTR_${arm}" \
...
if [[ "${RESOURCE_SMOKE}" == "1" ]]; then
  smoke_line=""
  for line in "${LINES[@]}"; do
    if [[ "${line}" == $'Weather\tsiff_equal\t'* ]]; then
      smoke_line="${line}"
      break
    fi
  done
  test -n "${smoke_line}"
  smoke_root="${OUTPUT_ROOT}/_resource_smoke/siff_equal_weather_seed${SEED}"
  mkdir -p "${smoke_root}"
...
{
  echo "siff_equal_attribution_start=$(date -Is)"
...
if [[ "${status}" != "0" ]]; then
  echo "siff_equal_attribution_worker_failure=$(date -Is)" >&2
  exit 1
fi
...
echo "siff_equal_attribution_done=$(date -Is)"
