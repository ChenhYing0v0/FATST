...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/stage_c_siff_v2_fcc_v1}"
HISTORICAL_ROOT="${HISTORICAL_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/stage_c_siff_equal_attribution_v2}"
DATASET_ROOT="${DATASET_ROOT:-/home/yingch/dataset}"
CONFIG="${CONFIG:-configs/stage_c_siff_v2_fcc_v1.json}"
...
PROTOCOL_PROFILE="stage_c_siff_v2_fcc_v1"
...
for seed,dataset,arm_id in config["launch_order"]:
    ...
    print("\t".join(map(str,(
        ...
        arm["objective_mode"],rank,profile["profile"],profile["patch_num"],
...
if [[ "${STATUS_ONLY}" == "1" ]]; then
  ...
  echo "siff_fcc_status=$(date -Is) training=${completed}/${#LINES[@]} test=${test_completed}/${#LINES[@]}"
...
if [[ "${DRY_RUN}" == "1" ]]; then
  "${PYTHON_BIN}" scripts/analyze_stage_c_siff_v2_fcc.py \
    --config "${CONFIG}" --synthetic-smoke >/dev/null
  printf '%s\n' "${LINES[@]}"
  echo "siff_fcc_dry_run=pass jobs=${#LINES[@]} config_hash=${CONFIG_HASH} profile_hash=${PROFILE_HASH} remote_authorized=${REMOTE_AUTHORIZED} test_authorized=${TEST_AUTHORIZED}"
  exit 0
fi

if [[ "${REMOTE_AUTHORIZED}" != "true" ]]; then
  echo "FCC remote launch is not authorized by ${CONFIG}" >&2
  exit 3
fi
if [[ "${FORMAL_TEST_ONLY}" == "1" && "${TEST_AUTHORIZED}" != "true" ]]; then
  echo "FCC formal test is not authorized by ${CONFIG}" >&2
  exit 3
fi
...
run_training_command() {
  ...
  local seed dataset arm readout policy objective rank profile patch_num d_model d_ff
  local run_args=()
  IFS=$'\t' read -r seed dataset arm readout policy objective rank profile \
  ...
  PYTHONHASHSEED="${seed}" CUDA_VISIBLE_DEVICES="${gpu}" \
    ...
    python baselines/timealign_official/train_repo.py \
      ...
      --num-workers 0 --run-name "SIFF_FCC_${arm}_seed${seed}" \
...
if [[ "${RESOURCE_SMOKE}" == "1" ]]; then
  smoke_lines=()
  for line in "${LINES[@]}"; do
    if [[ "${line}" == $'2022\tWeather\tsiff_equal\t'* \
      || "${line}" == $'2023\tETTm2\tsiff_independent_equal\t'* ]]; then
      smoke_lines+=("${line}")
    fi
  done
  ...
  echo "siff_fcc_resource_smoke_done=$(date -Is) output=${smoke_root}"
  exit 0
fi
...
if [[ "${FORMAL_TEST_ONLY}" == "1" ]]; then
  ...
  if [[ "${training_complete}" -ne "${#LINES[@]}" ]]; then
    echo "FCC formal test requires complete training: ${training_complete}/${#LINES[@]}" >&2
    exit 4
  fi
fi
...
mkdir -p "${LOG_ROOT}"
if [[ "${FORMAL_TEST_ONLY}" == "1" ]]; then
  LAUNCH_RECORD="${OUTPUT_ROOT}/formal_test_launch_record.txt"
  JOB_RECORD="${OUTPUT_ROOT}/formal_test_jobs.tsv"
else
  LAUNCH_RECORD="${OUTPUT_ROOT}/launch_record.txt"
  JOB_RECORD="${OUTPUT_ROOT}/jobs.tsv"
fi
{
  echo "siff_fcc_start=$(date -Is)"
...
run_one() {
  ...
  if [[ "${FORMAL_TEST_ONLY}" == "1" ]]; then
    if is_test_complete "${line}"; then
      echo "skip_existing_test=$(date -Is) job=$((index + 1))/${#LINES[@]} arm=${arm} dataset=${dataset} seed=${seed} gpu=${gpu}"
      return 0
    fi
    ...
    if [[ "${checkpoint_before}" != "${checkpoint_after}" ]]; then
      echo "checkpoint_mutated arm=${arm} dataset=${dataset} seed=${seed}" >&2
      return 1
    fi
  ...
  if is_complete "${line}"; then
    echo "skip_existing=$(date -Is) job=$((index + 1))/${#LINES[@]} arm=${arm} dataset=${dataset} seed=${seed} gpu=${gpu}"
    return 0
  fi
  mkdir -p "${output_dir}"
  echo "train_start=$(date -Is) job=$((index + 1))/${#LINES[@]} arm=${arm} dataset=${dataset} seed=${seed} gpu=${gpu} profile=${profile} rank=${rank}"
  run_training_command "${line}" "${gpu}" "${output_dir}" "${run_log}" 0
  echo "run_done=$(date -Is) job=$((index + 1))/${#LINES[@]} arm=${arm} dataset=${dataset} seed=${seed} gpu=${gpu}"
}
...
if [[ "${status}" != "0" ]]; then
  echo "siff_fcc_worker_failure=$(date -Is)" >&2
  exit 1
fi
...
if [[ "${FORMAL_TEST_ONLY}" == "1" ]]; then
  ...
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/analyze_stage_c_siff_v2_fcc.py \
      --config "${CONFIG}" --new-root "${OUTPUT_ROOT}" \
      --historical-root "${HISTORICAL_ROOT}" \
      --output-dir "${analysis_root}"
  echo "siff_fcc_formal_test_done=$(date -Is) analysis=${analysis_root}"
else
  echo "siff_fcc_training_done=$(date -Is) formal_test_executed=false"
fi
