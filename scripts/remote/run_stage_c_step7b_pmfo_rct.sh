...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/stage_c_step7b_pmfo_rct}"
...
PROTOCOL_PROFILE="stage_c_pmfo_rct_step7b_v1"
SEGMENT_HORIZONS="48,96,192,336,720"
DENSE_HORIZONS="$(seq -s, 1 720)"
PROFILE_HASH="$(sha256sum "${CONTRACT}" | awk '{print $1}')"
...
for dataset in ("Weather", "ETTm1", "ETTh2"):
...
if [[ "${STATUS_ONLY}" == "1" ]]; then
  ...
  for line in "${LINES[@]}"; do
    ...
    if [[ -s "${output_dir}/trained_invariants.json" ]]; then
      invariant_complete=$((invariant_complete + 1))
    fi
  done
  echo "step7b_status=$(date -Is) completed=${completed}/${#LINES[@]} invariants=${invariant_complete}/${#LINES[@]}"
...
if [[ "${DRY_RUN}" == "1" ]]; then
  ...
  for readout in \
    ...
    "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
      python scripts/check_stage_c_step7b_checkpoint_invariants.py \
      --synthetic-readout "${readout}" --seed "${SEED}" >/dev/null
  done
  printf '%s\n' "${LINES[@]}"
  echo "stage_c_step7b_dry_run=pass jobs=${#LINES[@]} profile_hash=${PROFILE_HASH}"
  exit 0
fi

mkdir -p "${OUTPUT_ROOT}/_logs" "${OUTPUT_ROOT}/_analysis"
{
  echo "stage_c_step7b_start=$(date -Is)"
  ...
  echo "evaluation=dense_h1_h720_full_crop_mse_mae"
...
run_one() {
  local line="$1" gpu="$2"
  local dataset arm readout profile patch_num d_model d_ff output_dir run_name run_log
  IFS=$'\t' read -r dataset arm readout profile patch_num d_model d_ff <<< "${line}"
  output_dir="$(run_dir_for_line "${line}")"
  run_name="SC1_STEP7B_${arm}"
  ...
  if [[ -s "${output_dir}/metrics_by_target_horizon.csv" \
    ...
    echo "skip_existing=$(date -Is) arm=${arm} dataset=${dataset} gpu=${gpu}"
    return 0
  fi

  mkdir -p "${output_dir}"
  echo "run_start=$(date -Is) arm=${arm} dataset=${dataset} gpu=${gpu} profile=${profile}"
  CUDA_VISIBLE_DEVICES="${gpu}" "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python baselines/timealign_official/train_repo.py \
      ...
      --final-evaluation-split test --protocol-class method_screening \
      ...
      --pmfo-state-dim 32 --pmfo-dense-hidden-dim 144 --pred-loss-mode full \
      --no-save-predictions >"${run_log}" 2>&1
  "${CONDA_BIN}" run --no-capture-output -n "${CONDA_ENV}" \
    python scripts/check_stage_c_step7b_checkpoint_invariants.py \
...
if [[ "${status}" != "0" ]]; then
  echo "stage_c_step7b_worker_failure=$(date -Is)" >&2
  exit 1
fi
...
echo "stage_c_step7b_done=$(date -Is)"
