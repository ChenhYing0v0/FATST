...
REMOTE_OUTPUT_ROOT="${REMOTE_OUTPUT_ROOT:-/home/yingch/exp_outputs/r-2026-fatst/phase5_timealign_hss_h1b_variable_readout_gate}"
CHECKPOINT_POLICY="${CHECKPOINT_POLICY:-official-last}"
ANALYSIS_ROOT="${ANALYSIS_ROOT:-analysis/phase5_timealign_hss_h1b_variable_readout_gate_$(date '+%Y%m%d')}"
SEED="${SEED:-2021}"
...
echo "sync_phase5_timealign_hss_h1b_start=$(date '+%Y-%m-%dT%H:%M:%S%z')"
...
python scripts/analyze_phase5_timealign_hss_h1b_variable_readout_gate.py \
...
echo "sync_phase5_timealign_hss_h1b_done=$(date '+%Y-%m-%dT%H:%M:%S%z')"
